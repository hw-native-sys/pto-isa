...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--case", dest="case_name", help="Generate only the specified case name (e.g. case_float_H_128_S0_512_S1_2048)"
    )
    parser.add_argument(
        "--cases",
        action="append",
        help="Explicit case entry HEAD_SIZE,S0,S1[,CUBE_S0[,TILE_S1]] (repeatable; CUBE_S1 fixed at 128)",
    )
    parser.add_argument(
        "--cases-json",
        dest="cases_json",
        default=None,
        help="Path to generated_cases.json (defaults to ../generated_cases.json if present)",
    )
    ...
    if args.cases:
        ...
        cases.append(
            (
                ...
                (args.s0, args.head_size, args.s1, 128, TILE_S1_DEFAULT),
            )
        )
