...
AICORE inline void PtoLoadNdGmToNzL1(uint64_t dstL1Offset, __gm__ Element *src, uint32_t rows, uint32_t cols,
                                     uint32_t leadingDim)
{
...
template <typename Element, int Rows, int Cols>
AICORE inline void PtoLoadPackedWeightGmToL1(uint64_t dstL1Offset, __gm__ Element *src, uint32_t validRows,
                                             uint32_t validCols, uint32_t fullRows)
{
...
AICORE inline void PtoStoreQuantAccToGm(__gm__ ElementDst *dst, uint64_t accOffset, uint64_t scaleOffset,
                                        uint32_t validRow, uint32_t validCol, uint32_t leadingDim)
{
    ...
    DstStride dstStride(static_cast<int64_t>(validRow) * leadingDim, static_cast<int64_t>(validRow) * leadingDim,
...
template <int Cols>
AICORE inline void StagePerChannelScale(uint64_t l1SOffset, uint64_t fixpipeOffset, __gm__ uint64_t *gmBlockS,
                                        uint32_t cols)
{
...
template <uint32_t PRELOAD_STAGES_, uint32_t L1_STAGES_, uint32_t L0A_STAGES_, uint32_t L0B_STAGES_,
          uint32_t L0C_STAGES_, bool ENABLE_SHUFFLE_K_, uint32_t L1_M_, uint32_t L1_N_, uint32_t L1_K_, uint32_t L0_M_,
          uint32_t L0_N_, uint32_t L0_K_, typename ElementA_, typename ElementB_, typename ElementC_>
...
public:
    ...
    static_assert((L1A_TILE_SIZE + L1B_TILE_SIZE + L1S_TILE_SIZE) * L1_STAGES <= ArchTag::L1_SIZE,
    ...
    AICORE inline void RunTile(const GmmBlockParams &params)
    {
        ...
        for (uint32_t kLoopIdx = 0; kLoopIdx < kTileCount; ++kLoopIdx) {
            ...
            PtoLoadNdGmToNzL1<ElementA, L1_M, L1_K>(l1AOffsetList[l1ListId], gmTileA, params.actualM, kActual,
                                                    params.aLeadingDim);
            set_flag(PIPE_MTE2, PIPE_MTE1, l1AEventList[l1ListId]);

            wait_flag(PIPE_MTE1, PIPE_MTE2, l1BEventList[l1ListId]);
            PtoLoadPackedWeightGmToL1<ElementB, L1_K, L1_N>(l1BOffsetList[l1ListId], gmTileB, kActual, params.actualN,
                                                            params.bFullRows);
...
private:
    ...
    using APanelTile = pto::Tile<pto::TileType::Mat, ElementA, L1_M, L1_K, pto::BLayout::ColMajor, pto::DYNAMIC,
                                 pto::DYNAMIC, pto::SLayout::RowMajor>;
    using BPanelTile = pto::Tile<pto::TileType::Mat, ElementB, L1_K, L1_N, pto::BLayout::ColMajor, pto::DYNAMIC,
                                 pto::DYNAMIC, pto::SLayout::RowMajor>;
    ...
    AICORE inline void AssignL1Panels(const L1TileGmmParams &params, APanelTile &aPanel, BPanelTile &bPanel,
                                      AccPanelTile &accPanel)
    {
    ...
    AICORE inline void StoreL1TileResult(const L1TileGmmParams &params)
    {
        ...
        PtoStoreQuantAccToGm<ElementC, L1_M, L1_N>(params.gmBlockC, l0COffsetList[l0CListId], fixpipeBaseOffset,
                                                   params.actualM, params.actualN, params.cLeadingDim);
