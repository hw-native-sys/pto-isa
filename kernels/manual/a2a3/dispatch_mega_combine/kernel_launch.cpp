...
AICORE inline void RunMegaMoeSpecialized(GM_ADDR x, GM_ADDR w1, GM_ADDR w2, GM_ADDR expertId, GM_ADDR scale1,
                                         GM_ADDR scale2, GM_ADDR probs, GM_ADDR c, GM_ADDR expertTokenNums,
...
extern "C" __global__ __aicore__ void dispatch_mega_combine_kernel(GM_ADDR fftsAddr, GM_ADDR x, GM_ADDR w1, GM_ADDR w2,
                                                                   GM_ADDR expertId, GM_ADDR scale1, GM_ADDR scale2,
                                                                   GM_ADDR probs, GM_ADDR c, GM_ADDR expertTokenNums,
                                                                   GM_ADDR workspaceGM, GM_ADDR tilingGM,
                                                                   GM_ADDR profileGM, uint32_t startSyncDebug)
{
#ifdef _DEBUG
    cce::printf("MegaMoe kernel enter block=%d sub=%d DAV_VEC=%d workspace=%d tiling=%d\n", int(get_block_idx()),
    ...
    if (workspaceGM != nullptr && tilingGM != nullptr) {
...
#ifdef _DEBUG
        cce::printf("MegaMoe kernel tiling block=%d sub=%d stage=%d M=%d topK=%d expertNum=%d\n", int(get_block_idx()),
        ...
        if (tilingData->megaMoeInfo.expertPerRank == 8U) {
            RunMegaMoeSpecialized<8U>(x, w1, w2, expertId, scale1, scale2, probs, c, expertTokenNums, workspaceGM,
                                      tilingData);
        } else {
            RunMegaMoeSpecialized<16U>(x, w1, w2, expertId, scale1, scale2, probs, c, expertTokenNums, workspaceGM,
                                       tilingData);
        }
    }
