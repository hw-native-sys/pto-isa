...
// Pure 1D N-cut 32-cell WSE FFN emulation host driver -- TPUSH ReduceSum variant.
//
// Verifies the GridPipe TPUSH/TPOP unicast primitives on a real distributed-FFN
// ReduceSum: identical compute and topology to the TREDUCE ReduceSum variant, but
// the EAST(8-way)+SOUTH(4-way) reduction is driven by explicit TPUSH/TPOP instead
// of the fused TREDUCE collective (the documented A3 lowering).  The output is
// compared bit-exact against the same SwiGLU golden.
//
...
static bool VerifyOutput(DeviceResources& r)
{
    ...
    std::cout << "[INFO] ResultCmp 32-cell N-cut TPUSH ReduceSum output vs golden:" << std::endl;
    ...
    DeviceResources& r, int phase, int rowStart, int colStart, int waveCols, int blockCount, const char* tag)
{
    ...
    launchDistributedFfnGridTpushReduceSumMixedKernel(
...
static bool RunSingleDevice(int physCores)
{
    ...
    for (int rs = 0; rs < FFN_NCUT_ROWS; rs += rowsPerWave) {
        ...
        if (!LaunchWave(r, /*phase=*/1, rs, 0, FFN_NCUT_COLS, waveRows * FFN_NCUT_COLS, "B EAST row reduce (TPUSH)")) {
            Cleanup(r);
            return false;
        }
    }
    ...
    if (!LaunchWave(r, /*phase=*/2, 0, FFN_NCUT_COLS - 1, 1, FFN_NCUT_ROWS, "C SOUTH col reduce (TPUSH)")) {
        Cleanup(r);
        return false;
    }
    ...
    std::cout << "[INFO] emulated 32-cell N-cut TPUSH ReduceSum on " << r.physCores
...
int main(int argc, char** argv)
{
    ...
    std::cout << "\n================================================================" << std::endl;
    std::cout << "  Pure 1D N-cut 32-cell WSE FFN emulation -- TPUSH ReduceSum" << std::endl;
    ...
    std::cout << "  per-cell I_shard=" << FFN_NCUT_I_SHARD
              << "  TPUSH EAST(8-way)+SOUTH(4-way), H-chunk=" << FFN_RS_REDUCE_H_BASE << std::endl;
    std::cout << "================================================================" << std::endl;

    bool ok = RunSingleDevice(physCores);
    std::cout << (ok ? "[SUCCESS] 32-cell N-cut FFN GridPipe TPUSH ReduceSum PASS." :
                       "[FAILED] 32-cell N-cut FFN GridPipe TPUSH ReduceSum FAILED.")
              << std::endl;
