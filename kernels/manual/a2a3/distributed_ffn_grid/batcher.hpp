...
struct FfnBatcher {
    ...
    bool Allocate(size_t gridRows, size_t gridCols, const std::string& splitMode)
    {
        ...
        w_gate_shards_dev =
            MallocOrDie(static_cast<size_t>(FFN_BATCHER_W_GATE_SHARD_REGION_BYTES), "batcher.w_gate_shards");
        w_up_shards_dev = MallocOrDie(static_cast<size_t>(FFN_BATCHER_W_UP_SHARD_REGION_BYTES), "batcher.w_up_shards");
        w_down_shards_dev =
            MallocOrDie(static_cast<size_t>(FFN_BATCHER_W_DOWN_SHARD_REGION_BYTES), "batcher.w_down_shards");
    ...
    static void GatherColShard(
        const uint8_t* srcHalf, int rows, int cols, int colStart, int shardCols, uint8_t* dstHalf)
    {
    ...
    bool DistributeWeights()
    {
        ...
        for (size_t col = 0; col < cols; ++col) {
            ...
            if (aclrtMemcpy(
                    static_cast<uint8_t*>(w_gate_shards_dev) + col * gateShardBytes, gateShardBytes, hostShard.data(),
                    gateShardBytes, ACL_MEMCPY_HOST_TO_DEVICE) != ACL_SUCCESS) {
            ...
            if (aclrtMemcpy(
                    static_cast<uint8_t*>(w_up_shards_dev) + col * gateShardBytes, gateShardBytes, hostShard.data(),
                    gateShardBytes, ACL_MEMCPY_HOST_TO_DEVICE) != ACL_SUCCESS) {
            ...
            if (split_mode == "allgather") {
                ...
                if (aclrtMemcpy(
                        static_cast<uint8_t*>(w_down_shards_dev) + col * downShardBytes, downShardBytes,
                ...
                if (aclrtMemcpy(
                        static_cast<uint8_t*>(w_down_shards_dev) + col * downShardBytes, downShardBytes,
                        host_w_down_full.data() + byteOff, downShardBytes, ACL_MEMCPY_HOST_TO_DEVICE) != ACL_SUCCESS) {
    ...
    bool BroadcastInput(const std::string& dataDir)
    {
        ...
        if (aclrtMemcpy(x_full_dev, hostX.size(), hostX.data(), hostX.size(), ACL_MEMCPY_HOST_TO_DEVICE) !=
            ACL_SUCCESS) {
    ...
    bool AllocateNcut(size_t gridRows, size_t gridCols)
    {
        ...
        ncut_w_gate_full_dev =
            MallocOrDie(static_cast<size_t>(FFN_NCUT_H * FFN_NCUT_I * FFN_HALF_ELEM_BYTES), "batcher.ncut_w_gate_full");
        ncut_w_up_full_dev =
            MallocOrDie(static_cast<size_t>(FFN_NCUT_H * FFN_NCUT_I * FFN_HALF_ELEM_BYTES), "batcher.ncut_w_up_full");
        ncut_w_down_full_dev =
            MallocOrDie(static_cast<size_t>(FFN_NCUT_I * FFN_NCUT_H * FFN_HALF_ELEM_BYTES), "batcher.ncut_w_down_full");
        ...
        ncut_w_down_shards_dev =
            MallocOrDie(ncut_cells * static_cast<size_t>(FFN_NCUT_W_DOWN_SHARD_BYTES), "batcher.ncut_w_down_shards");
    ...
    bool LoadAndDistributeNcut(const std::string& dataDir)
    {
        ...
        auto loadFull = [&](const char* name, void* dev, std::vector<uint8_t>& host, size_t bytes) -> bool {
            ...
            if (!PtoTestCommon::ReadFile(path, fileSize, host.data(), bytes) || fileSize != bytes) {
                std::cerr << "[ERROR] ncut load mismatch: " << path << " (got " << fileSize << " expected " << bytes
                          << ")" << std::endl;
                return false;
            }
        ...
        for (size_t c = 0; c < ncut_cells; ++c) {
            ...
            if (aclrtMemcpy(
                    static_cast<uint8_t*>(ncut_w_gate_shards_dev) + c * FFN_NCUT_W_GATE_SHARD_BYTES,
            ...
            if (aclrtMemcpy(
                    static_cast<uint8_t*>(ncut_w_up_shards_dev) + c * FFN_NCUT_W_UP_SHARD_BYTES,
            ...
            if (aclrtMemcpy(
                    static_cast<uint8_t*>(ncut_w_down_shards_dev) + c * FFN_NCUT_W_DOWN_SHARD_BYTES,
    ...
    bool AllocateNcutReduce(size_t gridRows, size_t gridCols)
    {
        ...
        ncut_x_full_dev = MallocOrDie(static_cast<size_t>(FFN_NCUT_X_BYTES), "batcher.ncut_x_full");
        ncut_w_gate_full_dev =
            MallocOrDie(static_cast<size_t>(FFN_NCUT_H * FFN_NCUT_I) * elem, "batcher.ncut_w_gate_full");
        ncut_w_up_full_dev = MallocOrDie(static_cast<size_t>(FFN_NCUT_H * FFN_NCUT_I) * elem, "batcher.ncut_w_up_full");
        ncut_w_down_full_dev =
            MallocOrDie(static_cast<size_t>(FFN_NCUT_I * FFN_NCUT_H) * elem, "batcher.ncut_w_down_full");
        ncut_w_gate_shards_dev =
            MallocOrDie(ncut_cells * static_cast<size_t>(FFN_NCUT_W_GATE_SHARD_BYTES), "batcher.ncut_w_gate_shards");
        ncut_w_up_shards_dev =
            MallocOrDie(ncut_cells * static_cast<size_t>(FFN_NCUT_W_UP_SHARD_BYTES), "batcher.ncut_w_up_shards");
        ncut_reduce_w_down_shards_dev = MallocOrDie(
            ncut_cells * static_cast<size_t>(FFN_RS_W_DOWN_SHARD_BYTES), "batcher.ncut_reduce_w_down_shards");
        ncut_y_full_dev = MallocOrDie(static_cast<size_t>(FFN_NCUT_T * FFN_NCUT_H * 4), "batcher.ncut_y_full");
        if (!ncut_x_full_dev || !ncut_w_gate_full_dev || !ncut_w_up_full_dev || !ncut_w_down_full_dev ||
    ...
    bool LoadAndDistributeNcutReduce(const std::string& dataDir)
    {
        ...
        for (size_t c = 0; c < ncut_cells; ++c) {
            ...
            if (aclrtMemcpy(
                    static_cast<uint8_t*>(ncut_reduce_w_down_shards_dev) + c * FFN_RS_W_DOWN_SHARD_BYTES,
