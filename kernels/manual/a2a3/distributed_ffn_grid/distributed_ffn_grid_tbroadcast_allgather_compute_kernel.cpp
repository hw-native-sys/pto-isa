...
// The gather is spelled with the PTO instructions TBROADCAST<Group> /
// TPOP<Group>(pipe, tile, srcBlockId), not the A2/A3 backend's GRID_TRY_*_IMPL entry
// points, so this demo exercises the ISA surface itself (the GridGroup-vs-
// GridDirection overload selection against the unicast TPUSH/TPOP, the
// target-profile guard) and not just the lowering.  The instructions carry no
// spin bound -- they block like the hardware WAIT_SPR -- so a handshake mis-wire
// shows up as a hang rather than a kFaultWaitReadyTimeout sentinel.  That is safe
// here because the host waves each group wholly into one launch (see
// main_tbroadcast_allgather).  To bound the spin while debugging, build with
// -DPTO_GRID_MOCK_WFE_MAX_SPINS=N and call GRID_TRY_TBROADCAST_IMPL /
// GRID_TRY_TBPOP_IMPL directly.
//
// The collective rides the same rings and scoreboards a TPUSH would: a receiver
// takes each source's shard in its EAST ring (sources behind it) or WEST ring
// (sources ahead), and the sources SHARE that scoreboard over time, each storing
// its place in the shared sequence.  So the sources must publish one at a time in
// ascending rank order -- both gather phases interleave broadcast and drain in
// ONE ascending walk over the group, which is what establishes that.
...
// group spans: EAST|WEST for the ROW gather, NORTH|SOUTH for the COL gather.
constexpr int kRowGroupDirMask = pto::GridDirBit(GridDirection::EAST) | pto::GridDirBit(GridDirection::WEST);
constexpr int kColGroupDirMask = pto::GridDirBit(GridDirection::NORTH) | pto::GridDirBit(GridDirection::SOUTH);
using FfnGatherPipeP1 = GridPipe<HiddenShardTile, FFN_NCUT_SLOT_BYTES_P1, FFN_NCUT_GROUP_SLOTS_P1, kRowGroupDirMask>;
using FfnGatherPipeP2 = GridPipe<RowBlockTile, FFN_NCUT_SLOT_BYTES_P2, FFN_NCUT_GROUP_SLOTS_P2, kColGroupDirMask>;

// TAKING THE TURN.  The 8 (resp. 4) members share one ring slot per direction,
// so exactly one of them may be inside TBROADCAST at a time.  The caller-side
// obligation is one call on each side of the publish: TBWAIT<Group> blocks until
// the previous source's back-pressure is satisfied and writes nothing, and
// TBNOTIFY<Group>(pipe, dstBlockId) forwards that same verdict to the next
// source once TBROADCAST's own drain wait has proved the slot is clear.
//
// The SCHEDULE is this kernel's, not the instructions': an ascending walk with
// one publish per cell per launch.  Because TBWAIT has no exemption of its own
// (one wait consumes exactly one notification), the two ENDS of that walk are
// the kernel's to leave open -- index 0 does not wait, the last index does not
// notify.  Skipping the closing notify is not tidiness: consIndex is re-zeroed
// by the pipe init while the GM scoreboard is not, so a token minted for index 0
// here would satisfy the first TBWAIT of the next launch that reuses the window.
// No second pipe, no token window, no packet, and nothing extra for the
// receivers to drain -- the baton is one increment on the scoreboard of the axis
// the group does not span (NORTH for the ROW gather, EAST for the COL one).
...
// hazards), matching this kernel's simulation role.  cTile is allocated by the
// caller (via TASSIGN); the helper fills it and leaves it ready for the C2V push
// or TSTORE.
...
AICORE inline void FfnCubeGemmFill(TileAcc<float, kBaseM, N, kT, N>& cTile, __gm__ half* aGM, __gm__ half* bGM)
{
    ...
    using GlobalA =
        GlobalTensor<half, Shape<1, 1, 1, kT, kBaseK>, Stride<kT * Kfull, kT * Kfull, kT * Kfull, Kfull, 1>>;
    using GlobalB = GlobalTensor<half, Shape<1, 1, 1, kBaseK, N>, Stride<kBaseK * N, kBaseK * N, kBaseK * N, N, 1>>;
...
__global__ AICORE void DistributedFfnGridTbroadcastAllGatherMixedKernel(
    ...
    __gm__ uint8_t* wGateShards, __gm__ uint8_t* wUpShards, __gm__ uint8_t* wDownShards, __gm__ uint8_t* hiddenShardBuf,
    __gm__ uint8_t* rowBlockBuf, __gm__ uint8_t* hiddenFullBuf, __gm__ uint8_t* gatePartialBuf,
    __gm__ uint8_t* upPartialBuf, __gm__ uint8_t* yFull, __gm__ uint8_t* p1CtxRaw, __gm__ uint8_t* p2CtxRaw, int phase,
    int rowStart, int colStart, int waveCols, int gridRows, int gridCols)
{
    ...
    if (phase == 0) {
        if constexpr (DAV_CUBE) {
            ...
            FfnCubeGemmFill<kHfull, kIShard>(
                cGate, reinterpret_cast<__gm__ half*>(xBlock), reinterpret_cast<__gm__ half*>(wGateBlock));
#ifndef __PTO_AUTO__
            ...
            // up = x @ W_up
            FfnCubeGemmFill<kHfull, kIShard>(
                cGate, reinterpret_cast<__gm__ half*>(xBlock), reinterpret_cast<__gm__ half*>(wUpBlock));
#ifndef __PTO_AUTO__
            set_flag(PIPE_M, PIPE_FIX, EVENT_ID0);
            wait_flag(PIPE_M, PIPE_FIX, EVENT_ID0);
#endif
        ...
        if constexpr (DAV_VEC) {
            ...
            using GShard = GlobalTensor<
                half, Shape<1, 1, 1, kT, kIShard>, Stride<kT * kIShard, kT * kIShard, kT * kIShard, kIShard, 1>>;
    ...
    if (phase == 1) {
        if constexpr (DAV_VEC) {
            ...
            // SERIALIZED group broadcast: the 8 row-mates broadcast one at a
            // time, ascending srcCol, and each drains the shards of every other
            // source in the same walk.  Consumption is this loop's TPOP and
            // nothing else; the two rules that make it safe are
            //   (1) TBROADCAST returns only once ALL 7 receivers have TPOPed it,
            //   (2) TBNOTIFY then hands that verdict to the next source, which is
            //       blocked in TBWAIT until exactly that,
            // so no source ever writes a ring slot whose previous tile is still
            // undrained -- the slot is shared by every source on that side, and
            // the ready_scb likewise counts arrivals from all of them.
            for (int srcCol = 0; srcCol < gridCols; ++srcCol) {
                if (srcCol == col) {
                    // Wait for the slot (the previous source's shard has been
                    // TPOPed by the whole row); TBWAIT writes nothing.  col 0
                    // opens the walk, so nobody notifies it and it must not wait
                    // -- the instruction has no exemption of its own.  The own
                    // shard is already TINSERTed above; the publish reaches the
                    // other 7 row-mates and returns once all 7 have TPOPed.
                    if (col != 0) {
                        TBWAIT<GridGroup::ROW>(gatherPipe);
                    }
#ifndef __PTO_AUTO__
                    pipe_barrier(PIPE_ALL);
#endif
                    TBROADCAST<GridGroup::ROW>(gatherPipe, shardOwn);
                    // Ascending walk: the next source is the row-mate one column
                    // east, named by BLOCK ID like every peer operand.  The walk
                    // is finite (one publish per cell per launch), so the last
                    // column releases nobody -- a token minted there would sit in
                    // col 0's scoreboard and satisfy a later launch's first wait.
                    if (col + 1 < gridCols) {
                        TBNOTIFY<GridGroup::ROW>(gatherPipe, row * gridCols + col + 1);
                    }
#ifndef __PTO_AUTO__
                    pipe_barrier(PIPE_ALL);
#endif
                    ...
                    // Drain source srcCol's shard and insert it at its matching
                    // column slot, rebuilding the [8,768] row block.  The source is
                    // addressed by its logical block id -- row-mate srcCol is block
                    // row*gridCols + srcCol -- not by its position in the row.
                    TPOP<GridGroup::ROW>(gatherPipe, shardRecv, row * gridCols + srcCol);
#ifndef __PTO_AUTO__
                    set_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
                    wait_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
#endif
            ...
            using GBlock = GlobalTensor<
                half, Shape<1, 1, 1, kT, kRowBlock>,
    ...
    if (phase == 2) {
        if constexpr (DAV_VEC) {
            ...
            pipe_barrier(PIPE_V);
#endif
            // SERIALIZED group broadcast, ascending srcRow -- same discipline as
            // phase B (see the comment there): TBROADCAST returns only when all 3
            // receivers have TPOPed, and TBNOTIFY passes that on, so only one of
            // the 4 col-mates is inside TBROADCAST at a time.
            for (int srcRow = 0; srcRow < gridRows; ++srcRow) {
                if (srcRow == row) {
                    // Our turn; the own block is already TINSERTed above.  Same
                    // TBWAIT / TBNOTIFY discipline as phase B, successor one row
                    // south: row 0 opens the walk and does not wait, the last row
                    // closes it and notifies nobody.
                    if (row != 0) {
                        TBWAIT<GridGroup::COL>(gatherPipe);
                    }
#ifndef __PTO_AUTO__
                    pipe_barrier(PIPE_ALL);
#endif
                    TBROADCAST<GridGroup::COL>(gatherPipe, blockOwn);
                    if (row + 1 < gridRows) {
                        TBNOTIFY<GridGroup::COL>(gatherPipe, (row + 1) * gridCols + col);
                    }
#ifndef __PTO_AUTO__
                    pipe_barrier(PIPE_ALL);
#endif
                    ...
                    // Drain source srcRow's row block and insert it at its row slot,
                    // rebuilding the full hidden [8,3072].  Col-mate srcRow is block
                    // srcRow*gridCols + col.
                    TPOP<GridGroup::COL>(gatherPipe, blockRecv, srcRow * gridCols + col);
#ifndef __PTO_AUTO__
                    set_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
                    wait_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
#endif
            ...
            using GFull = GlobalTensor<
                half, Shape<1, 1, 1, kT, kIfull>, Stride<kT * kIfull, kT * kIfull, kT * kIfull, kIfull, 1>>;
    ...
    if (phase == 3) {
        if constexpr (DAV_CUBE) {
            ...
            FfnCubeGemmFill<kIfull, kHShard>(
                cDown, reinterpret_cast<__gm__ half*>(hiddenFullBlock), reinterpret_cast<__gm__ half*>(wDownBlock));
#ifndef __PTO_AUTO__
            set_flag(PIPE_M, PIPE_FIX, EVENT_ID0);
            wait_flag(PIPE_M, PIPE_FIX, EVENT_ID0);
#endif
            ...
            using GY = GlobalTensor<
                float, Shape<1, 1, 1, kT, kHShard>, Stride<kT * kHShard, kT * kHShard, kT * kHShard, kHfull, 1>>;
