...
// ChanCount = 1: both phases move their payload through the RESERVED BROADCAST
// CHANNEL's ring (TBROADCAST + TPOP<GridGroup>), never unicast TPUSH/TPOP, so
// that single ring is the whole payload region.  Its depth is the group width,
// which is what lets every member publish before anybody drains.  GroupMax sizes
// the group mailbox and the per-member tables.
using FfnGatherPipeP1 = GridPipe<
    HiddenShardTile, FFN_NCUT_SLOT_BYTES_P1, FFN_NCUT_SLOT_COUNT_P1, FFN_NCUT_GROUP_P1,
    /*ChanCount=*/FFN_NCUT_BCAST_CHAN_COUNT>;
using FfnGatherPipeP2 = GridPipe<
    RowBlockTile, FFN_NCUT_SLOT_BYTES_P2, FFN_NCUT_SLOT_COUNT_P2, FFN_NCUT_GROUP_P2,
    /*ChanCount=*/FFN_NCUT_BCAST_CHAN_COUNT>;
static_assert(
    ...
    int rowStart, int colStart, int waveCols, int gridRows, int gridCols)
{
    ...
    if (phase == 1) {
        if constexpr (DAV_VEC) {
            ...
            // Bound every wait this pipe performs, so a mis-wired handshake surfaces
            // as a fault sentinel instead of a hang -- and the collective below can
            // then be the plain PTO instruction rather than its TRY backend.
            ...
            // 真·同时 MPSC: BROADCAST FIRST, DRAIN AFTERWARDS.  The row group is 8
            // cells and the scoreboard file holds kGridChanCount = 4 channels, so
            // the row is wider than the doorbell pool -- but the publishers now
            // ask for a TICKET on the reserved broadcast channel instead of taking
            // a derived turn, and a receiver keeps granting while it is itself
            // blocked inside TBROADCAST.  The interleaved "drain the earlier turn,
            // then publish" loop this used to need is gone, and so is any ordering
            // requirement on the drains below.
            //
            // basek = this cell's column, i.e. its rank in the row group: ONE round,
            // so the group's sequence numbers are exactly 0..7 -- unique, dense, and
            // (with a ring slot per member) all grantable at once.
            TBROADCAST<GridGroup::ROW>(gatherPipe, shardOwn, static_cast<uint32_t>(col));
#ifndef __PTO_AUTO__
            pipe_barrier(PIPE_ALL);
#endif
            dsb(DSB_DDR);
            // Drain every row-mate and insert each into its matching column slot,
            // rebuilding the [8,768] row block.
            for (int srcCol = 0; srcCol < gridCols; ++srcCol) {
                if (srcCol == col) {
                    continue; // own shard already placed
                }
                TPOP<GridGroup::ROW>(gatherPipe, shardRecv, srcCol);
#ifndef __PTO_AUTO__
                set_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
                wait_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
#endif
    ...
    if (phase == 2) {
        if constexpr (DAV_VEC) {
            ...
            pipe_barrier(PIPE_V);
#endif
            // Same shape as phase B: broadcast, then drain in any order.  basek =
            // this cell's row, its rank in the column group, over a ring that is 4
            // slots deep -- so all four publishers are inside the grant window and
            // nobody waits.
            TBROADCAST<GridGroup::COL>(gatherPipe, blockOwn, static_cast<uint32_t>(row));
#ifndef __PTO_AUTO__
            pipe_barrier(PIPE_ALL);
#endif
            dsb(DSB_DDR);
            // Drain every col-mate and insert each at its row slot, rebuilding the
            // full hidden [8,3072].
            for (int srcRow = 0; srcRow < gridRows; ++srcRow) {
                if (srcRow == row) {
                    continue; // own block already placed
                }
                TPOP<GridGroup::COL>(gatherPipe, blockRecv, srcRow);
#ifndef __PTO_AUTO__
                set_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
                wait_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
#endif
