...
# Defaults = real DeepSeek-v4 Pro FFN on a 4x8 = 32-cell mesh: the intermediate I
# is split across all 32 cells (ffn-tile = per-cell I_shard = 96), x is broadcast,
# and the 32 down partials are reduced EAST(8) then SOUTH(4).  ffn-tile here is the
# per-cell I_shard; full I = ffn-tile * grid_rows * grid_cols.
...
echo "=== Pure 1D N-cut 32-cell WSE FFN emulation -- ReduceSum (Option B) ==="
...
# the AllGather variant: x[8,7168], w_gate/up[7168,3072], w_down[3072,7168]).
...
cmake -DRUN_MODE=${RUN_MODE} -DSOC_VERSION=${SOC_VERSION} \
      ...
      ..
make -j16 distributed_ffn_grid_treduce_reducesum
...
echo ""
echo "=== Running 32-cell N-cut FFN GridPipe ReduceSum (emulated on ${PHYS_CORES} phys AICores) ==="
export N_RANKS=${N_RANKS}
export FFN_GRID_DATA_DIR="${SCRIPT_DIR}/out"
./distributed_ffn_grid_treduce_reducesum --device-id "${DEVICE_ID}" --phys-cores "${PHYS_CORES}"
