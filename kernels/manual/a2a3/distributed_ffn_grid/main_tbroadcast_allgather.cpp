...
struct DeviceResources {
    ...
    // No separate turn arena: the publish baton rides the gather pipe's own
    // orthogonal scoreboard (TBNOTIFY / TBWAIT, GridTBroadcast.hpp).
...
// Scan one arena's per-cell flag header (first FFN_NCUT_GRID_FLAGS_BYTES = the
// cache-line-strided unicast ready/free scbs +
...
// DEBUG: dump each cell's per-direction READY scoreboards for the first
// `dumpCells` cells of one arena, so a stuck gather shows how far the serialized
// broadcast got.  The group collectives own no scoreboards -- a broadcast
// doorbell lands on the receiver's ready_scb[dir] for the direction its source's
// edge was attributed to -- so a cell's counts read as "the highest-ranked source
// that reached me from each side", and the cell waiting on source k shows k as
// the missing value in that source's direction.  A ROW gather populates
// EAST/WEST, a COL gather NORTH/SOUTH.  Offsets mirror InitGridPipeFromWindow
// (flag header, one cache line per scoreboard).
static void DumpArenaGroupScbs(void* arenaDev, int winBytes, int dumpCells, const char* arenaName)
{
    // Index order matches the mesh-edge index (GridDirection minus SOURCE).
    static const char* kEdgeName[FFN_NCUT_GRID_EDGE_COUNT] = {"N", "E", "W", "S"};
    std::cout << "[DEBUG] " << arenaName << " ready scoreboards per direction:" << std::endl;
    for (int cell = 0; cell < dumpCells; ++cell) {
        ...
        for (int d = 0; d < FFN_NCUT_GRID_EDGE_COUNT; ++d) {
            uint32_t ready = 0;
            aclrtMemcpy(
                &ready, sizeof(uint32_t), base + FFN_NCUT_READY_SCB_OFF(d), sizeof(uint32_t),
                ACL_MEMCPY_DEVICE_TO_HOST);
            std::cout << " " << kEdgeName[d] << "=" << ready;
        }
        std::cout << std::endl;
    }
}
...
static bool RunSingleDevice(int physCores)
{
    ...
    // row group (8 cells) is wholly present in its wave -- mandatory, because the
    // 8 sources hand the turn along inside the wave and a missing member stalls
    // the chain.  Wave size is bounded only by physCores (no oversubscription),
    ...
    for (int rs = 0; rs < FFN_NCUT_ROWS; rs += rowsPerWave) {
        ...
        if (!LaunchWave(r, /*phase=*/1, rs, 0, FFN_NCUT_COLS, waveRows * FFN_NCUT_COLS, "B AG-P1 row gather")) {
            DumpArenaGroupScbs(r.p1_windows_dev, FFN_NCUT_WIN_P1, FFN_NCUT_CELLS, "P1");
            Cleanup(r);
            return false;
        }
    }
    ...
    for (int cs = 0; cs < FFN_NCUT_COLS; cs += colsPerWave) {
        ...
        if (!LaunchWave(r, /*phase=*/2, 0, cs, waveCols, FFN_NCUT_ROWS * waveCols, "C AG-P2 col gather")) {
            DumpArenaGroupScbs(r.p2_windows_dev, FFN_NCUT_WIN_P2, FFN_NCUT_CELLS, "P2");
            Cleanup(r);
            return false;
        }
    }
