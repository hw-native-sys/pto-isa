...
static bool InitGridPipeContext(void* arenaDev, int winBytes, void*& ctxDev)
{
    ...
    if (aclrtMemcpy(
            ctxDev, sizeof(CommDeviceContext), &hostCtx, sizeof(CommDeviceContext), ACL_MEMCPY_HOST_TO_DEVICE) !=
        ACL_SUCCESS) {
...
static bool AllocateResources(DeviceResources& r, int physCores)
{
    ...
    r.hidden_shard_dev =
        MallocBufOrDie(static_cast<size_t>(FFN_NCUT_CELLS) * FFN_NCUT_HIDDEN_SHARD_BYTES, "hidden_shard");
    ...
    r.gate_partial_dev =
        MallocBufOrDie(static_cast<size_t>(FFN_NCUT_CELLS) * FFN_NCUT_GATE_PARTIAL_BYTES, "gate_partial");
...
static bool LoadBatcherData(DeviceResources& r) { return r.batcher.LoadAndDistributeNcut(r.dataDir); }
...
// DEBUG: dump the group-collective channel scoreboards (one channel per group
// member: ready/free/close absolute counts) for the first `dumpCells` cells of one
// arena, so a stuck gather shows exactly which member's doorbell never fired.
// Offsets mirror the fixed header in grid_pipe_runtime.hpp: scoreboard c of each
// kind owns one whole cache line, ready first, then free, then close.
static void DumpArenaChannelScbs(void* arenaDev, int winBytes, int groupMax, int dumpCells, const char* arenaName)
{
    constexpr int kLine = FFN_GRID_SCB_LINE_STRIDE;
    const int readyOff = 0;
    const int freeOff = FFN_GRID_CHAN_MAX * kLine;
    const int closeOff = 2 * FFN_GRID_CHAN_MAX * kLine;
    std::cout << "[DEBUG] " << arenaName << " group channels (ready/free/close, one line each, stride=" << kLine
              << "):" << std::endl;
    for (int cell = 0; cell < dumpCells; ++cell) {
        ...
        std::vector<uint32_t> closeV(groupMax, 0);
        for (int c = 0; c < groupMax; ++c) {
            aclrtMemcpy(
                &ready[c], sizeof(uint32_t), base + readyOff + c * kLine, sizeof(uint32_t), ACL_MEMCPY_DEVICE_TO_HOST);
            aclrtMemcpy(
                &freeV[c], sizeof(uint32_t), base + freeOff + c * kLine, sizeof(uint32_t), ACL_MEMCPY_DEVICE_TO_HOST);
            aclrtMemcpy(
                &closeV[c], sizeof(uint32_t), base + closeOff + c * kLine, sizeof(uint32_t), ACL_MEMCPY_DEVICE_TO_HOST);
        }
        int row = cell / FFN_NCUT_COLS;
        int col = cell - row * FFN_NCUT_COLS;
    ...
    DeviceResources& r, int phase, int rowStart, int colStart, int waveCols, int blockCount, const char* tag)
{
    ...
    launchDistributedFfnGridTbroadcastAllGatherMixedKernel(
        ...
        reinterpret_cast<uint8_t*>(r.p2_hccl_ctx_dev), phase, rowStart, colStart, waveCols, FFN_NCUT_ROWS,
        FFN_NCUT_COLS, blockCount, r.stream);
...
static bool RunSingleDevice(int physCores)
{
    ...
    // row group (8 cells) is wholly present in its wave.  Each member's doorbells
    // are its own channel's scoreboards, one whole cache line each, so concurrent
    // producers never clobber each other's word -- a wave scales to as many cells
    // as fit on the device.  Wave size is bounded only by physCores (no
    // oversubscription), not by the old ~8-poller thrash cap (kCommWaveCap, removed).
    ...
    for (int rs = 0; rs < FFN_NCUT_ROWS; rs += rowsPerWave) {
        ...
        if (!LaunchWave(r, /*phase=*/1, rs, 0, FFN_NCUT_COLS, waveRows * FFN_NCUT_COLS, "B AG-P1 row gather")) {
            DumpArenaChannelScbs(r.p1_windows_dev, FFN_NCUT_WIN_P1, FFN_NCUT_GROUP_P1, FFN_NCUT_CELLS, "P1");
            Cleanup(r);
            return false;
        }
    }
    ...
    for (int cs = 0; cs < FFN_NCUT_COLS; cs += colsPerWave) {
        ...
        if (!LaunchWave(r, /*phase=*/2, 0, cs, waveCols, FFN_NCUT_ROWS * waveCols, "C AG-P2 col gather")) {
            DumpArenaChannelScbs(r.p2_windows_dev, FFN_NCUT_WIN_P2, FFN_NCUT_GROUP_P2, FFN_NCUT_CELLS, "P2");
            Cleanup(r);
            return false;
        }
    }
    ...
    std::cout << "[INFO] emulated 32-cell N-cut FFN on " << r.physCores << " phys AICores, " << totalLaunches
              ...
              << " us, ~60% throughput fidelity" << std::endl;
