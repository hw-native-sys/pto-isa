...
//   - TPOP(prodId)  the transiting partial from its upstream peer,
//   - TADD       folds it into its own running partial, then
//   - TPUSH(consId) forwards the running partial to its downstream peer.
...
using FfnReducePipe =
    GridPipe<ReduceSegTile, FFN_RS_REDUCE_TILE_BYTES, FFN_RS_REDUCE_SLOT_COUNT, /*GroupMax=*/0, kFfnReduceChanCount>;
static_assert(
