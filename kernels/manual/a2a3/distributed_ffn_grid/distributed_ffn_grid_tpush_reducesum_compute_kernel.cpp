...
// A3 lowering of TREDUCE<Dir, Sum> documented in GridTReduce.hpp
// (GRID_TREDUCE_IMPL): a fan-in-1 chain where each cell
//   - TPOP<Dir>  the transiting partial from its upstream neighbor,
//   - TADD       folds it into its own running partial, then
//   - TPUSH<Dir> forwards the running partial to its downstream neighbor.
...
using FfnReducePipe = GridPipe<ReduceSegTile, FFN_RS_REDUCE_TILE_BYTES, FFN_RS_REDUCE_SLOT_COUNT, kFfnReduceDirMask>;
...
// is the landing tile for the transiting partial.  One hop = the adjacent cell
// along Dir; the whole-row reduction is the chain of such hops.
...
AICORE inline void TpushReduceHop(Pipe& pipe, pto::GridShape shape, pto::GridCoord coord, TileAcc& seg, TileRecv& recv)
{
    ...
    if (pto::CanPush(Dir, coord, shape)) {
#ifndef __PTO_AUTO__
        pipe_barrier(PIPE_ALL);
#endif
        dsb(DSB_DDR);
