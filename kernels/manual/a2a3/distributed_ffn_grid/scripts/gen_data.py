...
def gen_data_pure_ncut(cfg: FfnDataConfig) -> None:
    ...
    if act == "silu":
        print(
            f"[INFO] SwiGLU: silu(clamp(gate,+/-{SILU_CLAMP})) * up (must match kernel "
...
def main() -> None:
    ...
    parser.add_argument(
        "--grid-rows", type=int, default=None, help="Grid rows (2D layout).  If omitted, falls back to --n-ranks (1xN)."
    )
    parser.add_argument(
        "--grid-cols", type=int, default=None, help="Grid cols (2D layout).  If omitted, falls back to --n-ranks (1xN)."
    )
    parser.add_argument(
        "--n-ranks",
        type=int,
