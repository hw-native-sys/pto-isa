...
// This variant reduces through the group fan-in overload
// TREDUCE<Group, Op, T>(pipe, acc, scratch, base, ...) (lowering to
// GRID_TREDUCE_GROUP_IMPL), whose sink reads each member's contribution in place
// out of partialBuf (bytes and memberStride are independent runtime operands).
// No payload ever travels through the pipe, so the type carries NO ring at all:
// DirMask = kGridDirNone and BcastSlotCount = 0 leave the window as just the flag
// header.  The pipe is still required -- the fan-in handshake notifies through
// its per-edge scoreboards, the same eight TPUSH uses, picking the edge each
// contributor's link to the sink attributes to.  SlotCount must be positive, so
// 1 stands in unused.
using FfnReducePipe = GridPipe<ReduceSegTile, FFN_RS_REDUCE_TILE_BYTES, 1, pto::kGridDirNone>;
    ...
    __gm__ uint8_t* hcclCtxRaw, int phase, int rowStart, int colStart, int waveCols, int gridRows, int gridCols)
{
    ...
    // Group fan-in (mov_ubuf_group, op=SUM) with the SPR handshake.  Each cell
    // wrote its full-H partial to partialBuf in phase A; the row SINK (col ==
    // gridCols-1) folds all 8 row-mates' segment-h partials directly out of
    // partialBuf with one N->1 reduce intrinsic.  Member k == cell (row, k), so
    // members are visited col0..col7 -- the SAME order the EAST relay accumulated
    // in, hence the FP sum is bit-identical (IEEE-754 add is commutative).
    //
    // EVERY row member calls TREDUCE, not just the sink: the fan-in is notified
    // through the pipe's group ready_scb / free_scb -- the same scoreboard pair
    // TPUSH uses -- and a scoreboard has ONE external writer, so the 7
    // contributors must ring the sink in turn rather than all at once.  Their
    // half is the handshake alone (the data is already in GM); the topology
    // picks who is sink, exactly as it does for the directional relay.  Each of
    // the 7 H-segments is one round, kept apart by the pipe's own round counter.
    if (phase == 1) {
        if constexpr (DAV_VEC) {
            ReduceSegTile seg;     // reduce accumulator / result (sink only)
            ReduceSegTile scratch; // in-core combine scratch     (sink only)
            TASSIGN(seg, 0x0000);
            TASSIGN(scratch, 0x8000);

            // Phase-B sub-window (phase C takes the other).  All 7 contributors
            // ring the SINK's one EAST scoreboard, each storing its place in that
            // scoreboard's shared sequence; the two phases need separate windows
            // only because they would otherwise inherit each other's counts.
            // ONE definition of who collects: the same value every row member
            // hands TREDUCE, and the test this cell uses to decide whether it
            // owns the result.  Any rank would do -- the relay's left-to-right
            // accumulation is reproduced by the arena walk, not by the sink's
            // position -- and the last one keeps the row result where phase C
            // expects it.
            const int rowSinkBlockId = row * gridCols + (gridCols - 1);
            const bool isSink = (cell == rowSinkBlockId);
            FfnReducePipe reducePipe;
            GridShape shape{gridRows, gridCols};
            GridCoord coord{row, col};
            a2a3_grid::InitGridPipeFromWindow(
                reducePipe, shape, coord, reduceWindow + cell * FFN_RS_GROUP_WIN_TOTAL,
            ...
            for (int h = 0; h < kHSegs; ++h) {
                ...
                // (Contributors ignore it -- only the sink reads the arena.)
                ...
                TREDUCE<GridGroup::ROW, ReduceOp::Sum, float>(
                    reducePipe, seg, scratch, base, FFN_RS_REDUCE_TILE_BYTES, gridCols, rowSinkBlockId,
                    FFN_RS_PARTIAL_BYTES);
                if (!isSink) {
                    continue; // contributor: the handshake was the whole job
                }
#ifndef __PTO_AUTO__
                set_flag(PIPE_V, PIPE_MTE3, EVENT_ID0);
                wait_flag(PIPE_V, PIPE_MTE3, EVENT_ID0);
    ...
    // Group fan-in (mov_ubuf_group, op=SUM) with the SPR handshake, same shape as
    // phase B one axis over.  Phase B left one row partial per row in
    // rowPartialBuf -- written by these very cells, which were the row sinks --
    // and the column SINK (row == gridRows-1) folds all 4 rows' segment-h
    // partials out of it with one N->1 reduce intrinsic.  Member k == row k at
    // rowPartialBuf + k*RPPB (uniform stride RPPB); the row0..row3 order matches
    // the SOUTH relay accumulation (bit-identical).  All 4 column members call
    // TREDUCE; the 3 contributors run the handshake half only.
    if (phase == 2) {
        if constexpr (DAV_VEC) {
            ReduceSegTile seg;
            ...
            // Phase-C sub-window (second half of the pair): phase B's counts must
            // not satisfy phase C's thresholds, and these cells ran phase B as its
            // sinks.  Phase C rings the SOUTH scoreboard, phase B the EAST one.
            // One definition, as in phase B -- and the collector is named by
            // BLOCK ID, so the same integer serves the operand and the test.
            const int colSinkBlockId = (gridRows - 1) * gridCols + col;
            const bool isSink = (cell == colSinkBlockId);
            FfnReducePipe reducePipe;
            GridShape shape{gridRows, gridCols};
            GridCoord coord{row, col};
            a2a3_grid::InitGridPipeFromWindow(
                reducePipe, shape, coord, reduceWindow + cell * FFN_RS_GROUP_WIN_TOTAL + FFN_RS_GROUP_WIN,
                reinterpret_cast<__gm__ void*>(hcclCtxRaw), /*pipeId=*/0);
            ...
            for (int h = 0; h < kHSegs; ++h) {
                ...
                TREDUCE<GridGroup::COL, ReduceOp::Sum, float>(
                    reducePipe, seg, scratch, base, FFN_RS_REDUCE_TILE_BYTES, gridRows, colSinkBlockId,
