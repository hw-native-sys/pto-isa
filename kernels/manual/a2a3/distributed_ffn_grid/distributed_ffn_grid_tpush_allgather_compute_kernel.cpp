...
// The relay is spelled with the PTO instructions TPUSH<Dir> / TPOP<Dir>, not the
// A2/A3 backend's GRID_TRY_*_IMPL entry points, so this demo exercises the ISA
// surface itself (overload resolution, the SOURCE static_assert, the
...
// here because the host waves each relay group wholly into one launch (see
// main_tpush_allgather).  To bound the spin while debugging, build with
// -DPTO_GRID_MOCK_WFE_MAX_SPINS=N and call GRID_TRY_TPUSH_IMPL /
// GRID_TRY_TPOP_IMPL directly.
...
using FfnGatherPipeP1 = GridPipe<RowBlockTile, FFN_NCUT_TPUSH_SLOT_BYTES_P1, FFN_NCUT_TPUSH_SLOT_COUNT, kFfnP1DirMask>;
using FfnGatherPipeP2 =
    GridPipe<HiddenFullTile, FFN_NCUT_TPUSH_SLOT_BYTES_P2, FFN_NCUT_TPUSH_SLOT_COUNT, kFfnP2DirMask>;
...
{
    const bool isFwdSource = !pto::CanPop(fDir, coord, shape); // no upstream -> relay start
    const bool isFwdSink = !pto::CanPush(fDir, coord, shape);  // no downstream -> relay end
    const bool isBwdSource = isFwdSink;                        // fwd sink starts the backward scatter
    const bool isBwdSink = !pto::CanPush(bDir, coord, shape);  // no downstream on bDir -> scatter end
    ...
    if (isFwdSource) {
        ...
        TPOP<fDir>(pipe, recvTile);             // MTE2 -> recvTile
    ...
    if (!isFwdSink) {
        ...
        TPUSH<fDir>(pipe, relayTile);         // MTE3 <- relayTile
        FfnRelayDrain();
    }
    ...
    if (!isBwdSource) {             // not the fwd sink -> receive the complete tile
        TPOP<bDir>(pipe, recvTile); // MTE2 -> recvTile
        FfnRelayDrain();
        TINSERT(relayTile, recvTile, 0, 0); // V: recvTile -> relayTile
    }
    ...
    if (!isBwdSink) {                 // not the scatter end -> forward it on
        ...
        TPUSH<bDir>(pipe, relayTile); // MTE3 <- relayTile
        FfnRelayDrain();
    }
