...
# Verifies the GridPipe TPUSH/TPOP unicast primitives on a distributed-FFN ReduceSum:
# the EAST(8-way)+SOUTH(4-way) reduction is driven by explicit TPUSH/TPOP (the A3
# lowering of TREDUCE) instead of the fused TREDUCE collective, and the output is
# compared bit-exact against the same SwiGLU golden.  Defaults = real DeepSeek-v4
# Pro FFN on a 4x8 = 32-cell mesh (ffn-tile = per-cell I_shard = 96).
...
echo "=== Pure 1D N-cut 32-cell WSE FFN emulation -- TPUSH ReduceSum ==="
...
# Regenerate flat full-tensor Batcher data + SwiGLU golden (same flat tensors as
# the other variants: x[8,7168], w_gate/up[7168,3072], w_down[3072,7168]).
...
cmake -DRUN_MODE=${RUN_MODE} -DSOC_VERSION=${SOC_VERSION} \
      ...
      ..
make -j16 distributed_ffn_grid_tpush_reducesum
...
echo ""
echo "=== Running 32-cell N-cut FFN GridPipe TPUSH ReduceSum (emulated on ${PHYS_CORES} phys AICores) ==="
export N_RANKS=${N_RANKS}
export FFN_GRID_DATA_DIR="${SCRIPT_DIR}/out"
./distributed_ffn_grid_tpush_reducesum --device-id "${DEVICE_ID}" --phys-cores "${PHYS_CORES}"
