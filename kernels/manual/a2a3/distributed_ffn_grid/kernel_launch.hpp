...
// --- AllGather pattern: TBROADCAST variant (MPSC group broadcast).  Each member
//     rings its OWN channel's ready/free/close scoreboards, one whole cache line
//     each, so the MPSC gather no longer needs a cube keep-alive (doneFlags
//     removed). ---
