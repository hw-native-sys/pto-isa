...
//   - TBROADCAST : the TBROADCAST<GridGroup> collective (every cell broadcasts, in turn).
...
// --- AllGather pattern: TBROADCAST variant (serialized group broadcast).  The
//     group notifies through one ready_scb/free_scb pair, so the 8 (resp. 4)
//     sources publish in rank order rather than at once; a wave must therefore
//     carry a WHOLE group.  No cube keep-alive is needed (doneFlags removed). ---
