...
//   receive layout = kSlotRegionOffset + ChanCount * SlotCount * SlotBytes,
// where the fixed header is
//   kFlagsBytes       = 3 * kGridChanCount * kScbLineStride  (ready/free/close,
//                       full cache line each -- a different remote writer per word)
//   kSlotRegionOffset = kFlagsBytes + 2 control lines + kRecordBytes  (the LOCAL pipe record: the
//                       binding table + consumer history + dual channel maps/states,
//                       which no peer ever writes, rounded up to a cache line)
// The AllGather variant additionally inserts the group mailbox before the rings:
//   + 2 * GroupMax * kScbLineStride  (request + response queue, one line per
//                                     RANK-IN-GROUP, not per core in the mesh)
// Its PAYLOAD needs no region of its own: each cell broadcasts its own hidden
// shard concurrently (真·同时 MPSC) into the RESERVED BROADCAST CHANNEL's ordinary
// ring, addressed by the caller's basek % SlotCount, and its doorbells are that
// channel's ready/free scoreboards in the fixed header.
// Every layout finally appends one SlotBytes producer L1 staging slot, after all
// receive-side regions, so outbound source data cannot alias a payload ring.
//
// kGridChanCount stays 4 (3*4 = 12 of the 16 native IPC_SCB slots), and a group
// wider than that -- the row group here is 8 cells -- does NOT need one channel
// each: every publisher raises the ONE reserved broadcast channel's count with an
// atomic add against a ticket, so the channel budget is independent of the group
// width (see grid_intrinsic.hpp's GROUP CHANNEL MAP).
constexpr int FFN_GRID_CHAN_MAX = 4;          // pto::kGridChanCount
constexpr int FFN_GRID_CONS_HIST_MAX = 8;     // pto::kGridConsHistMax
constexpr int FFN_GRID_SCB_LINE_STRIDE = 64;  // pto::grid_mock::kScbLineStride
constexpr int FFN_GRID_BIND_QUEUE_DEPTH = 32; // pto::kGridBindQueueDepth (>= cells)
constexpr int FFN_ACTIVE_VECTOR_SUBBLOCK_ID = 0;
constexpr int FFN_GRID_FLAGS_BYTES = 3 * FFN_GRID_CHAN_MAX * FFN_GRID_SCB_LINE_STRIDE; // 768
// Bind mailbox = two QUEUES of one cache line per peer (request in the consumer's
// window, response in the producer's), plus one line for the scheme-C group-reduce
// epoch.  One line per peer is what lets K producers hand a request to the same
// consumer at once without overwriting each other.
constexpr int FFN_GRID_CONTROL_BYTES = (2 * FFN_GRID_BIND_QUEUE_DEPTH + 1) * FFN_GRID_SCB_LINE_STRIDE; // 4160
// pto::kGridRecordWords = header + 8 channel arrays + 4 consumer-history arrays + current producer channel +
// scratch + the group credit baseline.
constexpr int FFN_GRID_RECORD_WORDS = 4 + 10 * FFN_GRID_CHAN_MAX + 4 * FFN_GRID_CONS_HIST_MAX + 3; // 79
constexpr int FFN_GRID_RECORD_BYTES =
    ((FFN_GRID_RECORD_WORDS * 4 + FFN_GRID_SCB_LINE_STRIDE - 1) / FFN_GRID_SCB_LINE_STRIDE) *
    FFN_GRID_SCB_LINE_STRIDE; // 320
constexpr int FFN_GRID_SLOT_REGION_OFFSET =
    FFN_GRID_FLAGS_BYTES + FFN_GRID_CONTROL_BYTES + FFN_GRID_RECORD_BYTES; // 5248
constexpr int FFN_GRID_CHAN_COUNT = FFN_GRID_CHAN_MAX;                     // legacy demo pipe: the default full array
constexpr int FFN_GRID_RECEIVE_REGION_BYTES =
    FFN_GRID_SLOT_REGION_OFFSET + FFN_GRID_CHAN_COUNT * FFN_SLOT_COUNT * FFN_SLOT_BYTES;
constexpr int FFN_GRID_UNICAST_WINDOW_BYTES = FFN_GRID_RECEIVE_REGION_BYTES + FFN_SLOT_BYTES;
#ifdef CONFIG_FFN_GRID_ALLGATHER
// Largest group this grid forms (a row or a column) = max(rows, cols).  It sizes
// the group mailbox (one request + one response line per RANK-IN-GROUP) and the
// per-member tables; the collective's PAYLOAD lives in the reserved broadcast
// channel's ordinary ring, which FFN_SLOT_COUNT already pays for.
constexpr int FFN_GRID_GROUP_MAX = (FFN_GRID_ROWS > FFN_GRID_COLS) ? FFN_GRID_ROWS : FFN_GRID_COLS;
constexpr int FFN_GRID_GROUP_MAILBOX_BYTES = 2 * FFN_GRID_GROUP_MAX * FFN_GRID_SCB_LINE_STRIDE;
constexpr int FFN_GRID_WINDOW_BYTES =
    FFN_GRID_UNICAST_WINDOW_BYTES + FFN_GRID_GROUP_MAILBOX_BYTES; // rings + producer slot already counted
#else
...
//
// These are PURE-BROADCAST pipes: no unicast TPUSH/TPOP ever runs on them.  They
// still declare ChanCount = 1, because that one channel is the RESERVED BROADCAST
// CHANNEL and its ring is where the collective's payload lands -- the collective
// no longer owns a region of its own.  Ring depth = group width, so every member
// can publish before anybody drains (each publisher's basek = its rank in the
// single round, so the slots are distinct by construction); a shallower ring would
// be legal but would force the kernel to interleave its drains.
//
// Window = header(5248) + group mailbox(2 * GroupMax lines) + SlotCount*SlotStride
// + one isolated producer staging slot, matching the layout in
// include/pto/npu/a2a3/grid_pipe_runtime.hpp.  The collective's doorbells cost no
// window bytes at all: they are the channel scoreboards of the fixed header.
constexpr int FFN_NCUT_BCAST_CHAN_COUNT = 1; // == pto::kGridBcastChanCount (the broadcast channel's ring)
constexpr int FFN_NCUT_RELAY_CHAN_COUNT = 2; // a relay runs two flows through each cell
constexpr int FFN_NCUT_GRID_FLAGS_BYTES = FFN_GRID_FLAGS_BYTES;
constexpr int FFN_NCUT_GRID_SLOT_REGION_OFFSET = FFN_GRID_SLOT_REGION_OFFSET;
...
constexpr int FFN_NCUT_SLOT_COUNT_P1 = FFN_NCUT_GROUP_P1;
constexpr int FFN_NCUT_SLOT_COUNT_P2 = FFN_NCUT_GROUP_P2;
constexpr int FFN_NCUT_MAILBOX_P1 = 2 * FFN_NCUT_GROUP_P1 * FFN_GRID_SCB_LINE_STRIDE;
constexpr int FFN_NCUT_MAILBOX_P2 = 2 * FFN_NCUT_GROUP_P2 * FFN_GRID_SCB_LINE_STRIDE;
constexpr int FFN_NCUT_WIN_P1 = FFN_NCUT_GRID_SLOT_REGION_OFFSET + FFN_NCUT_MAILBOX_P1 +
                                FFN_NCUT_BCAST_CHAN_COUNT * FFN_NCUT_SLOT_COUNT_P1 * FFN_NCUT_SLOT_BYTES_P1 +
                                FFN_NCUT_SLOT_BYTES_P1;
constexpr int FFN_NCUT_WIN_P2 = FFN_NCUT_GRID_SLOT_REGION_OFFSET + FFN_NCUT_MAILBOX_P2 +
                                FFN_NCUT_BCAST_CHAN_COUNT * FFN_NCUT_SLOT_COUNT_P2 * FFN_NCUT_SLOT_BYTES_P2 +
                                FFN_NCUT_SLOT_BYTES_P2;
...
//   header(1216) + 2 channels * SlotCount * SlotStride + one producer staging slot
// (mirrors FFN_RS_REDUCE_WIN).  Two channels, not four: a relay runs exactly two
// flows through a cell -- the forward gather and the backward scatter -- so the
// GridPipe ChanCount names that pair and the other rings are never allocated.
// P1 relays the [8,768] row block (row gather),
...
constexpr int FFN_NCUT_TPUSH_WIN_P1 =
    FFN_NCUT_GRID_SLOT_REGION_OFFSET +
    FFN_NCUT_RELAY_CHAN_COUNT * FFN_NCUT_TPUSH_SLOT_COUNT * FFN_NCUT_TPUSH_SLOT_BYTES_P1 + FFN_NCUT_TPUSH_SLOT_BYTES_P1;
constexpr int FFN_NCUT_TPUSH_WIN_P2 =
    FFN_NCUT_GRID_SLOT_REGION_OFFSET +
    FFN_NCUT_RELAY_CHAN_COUNT * FFN_NCUT_TPUSH_SLOT_COUNT * FFN_NCUT_TPUSH_SLOT_BYTES_P2 + FFN_NCUT_TPUSH_SLOT_BYTES_P2;
...
// TPUSH RELAY variant (distributed_ffn_grid_tpush_reducesum): phase B reduces
// EAST and phase C SOUTH over ONE window and ONE physical channel, deliberately,
// to exercise the time-division MPSC relay-count protocol: phase B's final TPUSH
// publishes CLOSE; phase C sends its producer id, receives the surviving
// ready/cons baselines, and continues the absolute sequence on the same ring
// without clearing an SCB.
constexpr int FFN_RS_REDUCE_CHAN_COUNT = 1;
constexpr int FFN_RS_REDUCE_WIN = FFN_NCUT_GRID_SLOT_REGION_OFFSET +
                                  FFN_RS_REDUCE_CHAN_COUNT * FFN_RS_REDUCE_SLOT_COUNT * FFN_RS_REDUCE_TILE_BYTES +
                                  FFN_RS_REDUCE_TILE_BYTES;

// TREDUCE GROUP variant (distributed_ffn_grid_treduce_reducesum): an N->1 fan-in
// (mov_ubuf_group) whose contributions stay in the caller's own arena, so the pipe
// carries no unicast ring at all -- ChanCount = 0 and the window is header + one
// producer staging slot.  What it does carry is the collective's handshake:
// channels [0, GroupMax) of the fixed header, one per group member, whose
// ready/free/close scoreboards are the doorbells.  GroupMax must cover the widest
// group either phase forms (a row of 8).
constexpr int FFN_RS_GROUP_CHAN_COUNT = 0;
constexpr int FFN_RS_REDUCE_GROUP_MAX = (FFN_NCUT_ROWS > FFN_NCUT_COLS) ? FFN_NCUT_ROWS : FFN_NCUT_COLS; // 8
// GroupMax > 0 also buys the group mailbox (2 lines per rank-in-group).  A pull
// reduce does not use it -- it binds through the unicast mailbox, GROUP_PULL --
// but the region is part of the layout, so the host mirror has to count it.
constexpr int FFN_RS_GROUP_MAILBOX_BYTES = 2 * FFN_RS_REDUCE_GROUP_MAX * FFN_GRID_SCB_LINE_STRIDE;
constexpr int FFN_RS_GROUP_REDUCE_WIN = FFN_NCUT_GRID_SLOT_REGION_OFFSET + FFN_RS_GROUP_MAILBOX_BYTES +
                                        FFN_RS_GROUP_CHAN_COUNT * FFN_RS_REDUCE_SLOT_COUNT * FFN_RS_REDUCE_TILE_BYTES +
                                        FFN_RS_REDUCE_TILE_BYTES;
// ONE WINDOW PER PHASE, and the reason is the group channel model.  Phase B
// reduces along rows and phase C along columns, so the SAME channel index means a
// DIFFERENT core in the two phases (channel r is bound to rank r by construction,
// with no negotiation to renegotiate).  Sharing one window would hand phase C a
// channel whose ready_scb still carries phase B's count from another core, with
// no handshake able to relay the baseline -- the fault kFaultGroupChannelOwner
// exists to catch exactly that.  Two windows keep the two channel spaces apart;
// the per-cell stride below is also the HCCL context's winSize, so the peer
// resolver maps either phase's addresses correctly.
constexpr int FFN_RS_GROUP_PHASE_WINDOWS = 2;
constexpr int FFN_RS_GROUP_REDUCE_WIN_STRIDE = FFN_RS_GROUP_PHASE_WINDOWS * FFN_RS_GROUP_REDUCE_WIN;
