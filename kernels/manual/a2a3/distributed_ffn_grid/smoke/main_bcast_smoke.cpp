...
static bool Verify(Resources& r)
{
    ...
    // Golden: every non-source participant holds the SUM OF THE ROUNDS it drained
    // -- round r carried the source's stamp (source cell index + 1) plus r, so the
    // sum is rounds*stamp + rounds*(rounds-1)/2.  With BCAST_ROUNDS = 1 that is
    // just the stamp.  The source cell and any non-participant cell stay zero.
    //   * ROW/COL : non-source cells on the span hold their span source's total.
    //   * SUBRECT : in-rectangle non-source cells hold the single rect source's
    //               total; the source and every out-of-rect cell stay zero.
    const auto roundSum = [](size_t srcCell) {
        const double stamp = static_cast<double>(srcCell + 1);
        return static_cast<float>(BCAST_ROUNDS * stamp + 0.5 * BCAST_ROUNDS * (BCAST_ROUNDS - 1));
    };
    double maxDiff = 0.0;
    ...
    for (int row = 0; row < r.rows; ++row) {
        for (int col = 0; col < r.cols; ++col) {
            ...
            float expected = 0.0f;
            if constexpr (BCAST_ALL_SOURCES != 0 && BCAST_SUBRECT != 0) {
                // Rectangle AllGather: every cell INSIDE the rectangle broadcast
                // every round, so it holds the sum of every other in-rect member's
                // rounds.  Cells outside stay zero -- nobody writes their window and
                // they never call the collective.
                const bool inRect =
                    row >= BCAST_RECT_R0 && row < BCAST_RECT_R1 && col >= BCAST_RECT_C0 && col < BCAST_RECT_C1;
                double total = 0.0;
                if (inRect) {
                    for (int rr = BCAST_RECT_R0; rr < BCAST_RECT_R1; ++rr) {
                        for (int cc = BCAST_RECT_C0; cc < BCAST_RECT_C1; ++cc) {
                            const size_t memberCell = static_cast<size_t>(rr) * r.cols + cc;
                            if (memberCell != cell) {
                                total += roundSum(memberCell);
                            }
                        }
                    }
                }
                expected = static_cast<float>(total);
            } else if constexpr (BCAST_ALL_SOURCES != 0) {
                // Every member broadcast every round, so each cell holds the sum of
                // all OTHER members' rounds.
                const int members = (BCAST_SPAN_COL != 0) ? r.rows : r.cols;
                double total = 0.0;
                for (int m = 0; m < members; ++m) {
                    const size_t memberCell = (BCAST_SPAN_COL != 0) ? (static_cast<size_t>(m) * r.cols + col) :
                                                                      (static_cast<size_t>(row) * r.cols + m);
                    if (memberCell == cell) {
                        continue;
                    }
                    total += roundSum(memberCell);
                ...
                expected = static_cast<float>(total);
            } else if constexpr (BCAST_SUBRECT != 0) {
                const bool inRect =
                    row >= BCAST_RECT_R0 && row < BCAST_RECT_R1 && col >= BCAST_RECT_C0 && col < BCAST_RECT_C1;
                if (inRect && cell != subrectSrcCell) {
                    expected = roundSum(subrectSrcCell);
                }
            } else {
                int myIdx = (BCAST_SPAN_COL != 0) ? row : col;
                if (myIdx != BCAST_SRC) {
                    expected = roundSum(SpanSourceCell(row, col, r.cols));
                }
            }
    ...
    std::cout << " tile=" << BCAST_T << "x" << BCAST_W << " rounds=" << BCAST_ROUNDS
              << (BCAST_ALL_SOURCES != 0 ? " all-sources" : "") << " max diff=" << maxDiff << std::endl;
    if (maxDiff != 0.0) {
