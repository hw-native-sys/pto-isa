...
//   - the single source cell (rank-in-group BCAST_SRC along the active group
//     axis) issues ONE TBROADCAST<GridGroup> delivering its tile to every other
//     cell on its group (row for ROW, column for COL) over the 真·同时 MPSC
//     channel (design doc §4 方案②·前缀偏移): batched writes into each
//     receiver's shared ring, a single publish fence, then per-source ready
//     lanes.  This is NOT a per-hop TPUSH<Dir, k> loop.
                                        ...
                                        __gm__ uint8_t *outBuf, __gm__ uint8_t *hcclCtxRaw, int gridRows, int gridCols)
{
    ...
    if constexpr (DAV_VEC) {
        ...
        // This cell's rank along the active group axis; the source is BCAST_SRC.
        const int myIdx = (kGroup == GridGroup::COL) ? coord.row : coord.col;
        ...
        if (isSource) {
            ...
            TPOP<kGroup>(pipe, recvTile, BCAST_SRC);
#ifndef __PTO_AUTO__
            set_flag(PIPE_MTE2, PIPE_MTE3, EVENT_ID0);
            wait_flag(PIPE_MTE2, PIPE_MTE3, EVENT_ID0);
            pipe_barrier(PIPE_ALL);
