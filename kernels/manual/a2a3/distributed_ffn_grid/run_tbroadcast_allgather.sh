...
# Defaults = real DeepSeek-v4 Pro FFN on a 4x8 = 32-cell mesh (方案①).
...
echo "=== Pure 1D N-cut 32-cell WSE FFN emulation (方案①) ==="
...
# Regenerate flat full-tensor Batcher data + SwiGLU golden before running.  The
# host Batcher slices w_gate/w_up along I and w_down along H flat-32-way and
# broadcasts x; golden is the full [T,H] SwiGLU reference.
...
cmake -DRUN_MODE=${RUN_MODE} -DSOC_VERSION=${SOC_VERSION} \
      ...
      ..
make -j16 distributed_ffn_grid_tbroadcast_allgather
...
echo ""
echo "=== Running 32-cell N-cut FFN GridPipe AllGather (emulated on ${PHYS_CORES} phys AICores) ==="
export N_RANKS=${N_RANKS}
export FFN_GRID_DATA_DIR="${SCRIPT_DIR}/out"
./distributed_ffn_grid_tbroadcast_allgather --device-id "${DEVICE_ID}" --phys-cores "${PHYS_CORES}"
