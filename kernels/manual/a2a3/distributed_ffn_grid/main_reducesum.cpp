...
static bool AllocateResources(DeviceResources &r)
{
    ...
    if (!r.batcher.Allocate(r.rows, r.cols, /*splitMode=*/"reduce")) {
        return false;
    }
    ...
    std::cout << "[INFO] grid=" << r.rows << "x" << r.cols << " cells=" << r.cells << " dataDir=" << r.dataDir
              << " reducePipeBytes=" << r.reducePipeBytes << std::endl;
    return true;
}

// Drive the GM-simulated Batcher: load resident full weights into GM, split them
// into per-column shards (w_down row-shard [Fi,H] for this ReduceSum variant),
// and broadcast the full input.  Replaces the legacy per-cell LoadInputs/
// LoadWeights (the kernel now reads x per-row, weight shards per-col, and the
// EAST-reduced output lands in the Batcher y region).
