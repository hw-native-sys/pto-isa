...
# Single-device multi-block FFN GridPipe ReduceSum demo.  grid-rows x grid-cols is the
...
echo "=== Single-device Multi-block FFN GridPipe ReduceSum Demo ==="
...
cmake -DRUN_MODE=${RUN_MODE} -DSOC_VERSION=${SOC_VERSION} \
      ...
      ..
make -j16 distributed_ffn_grid_reducesum
...
echo ""
echo "=== Running Single-device Multi-block FFN GridPipe ReduceSum ==="
export N_RANKS=${N_RANKS}
export FFN_GRID_DATA_DIR="${SCRIPT_DIR}/out"
./distributed_ffn_grid_reducesum --device-id "${DEVICE_ID}"
