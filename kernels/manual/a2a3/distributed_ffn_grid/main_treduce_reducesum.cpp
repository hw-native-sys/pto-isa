...
{
    // One GridPipe window per cell, memset so the scoreboards start at 0.  The
    // group fan-in moves no payload through the pipe, so a window is just the
    // flag header that holds the group ready_scb / free_scb pair -- but phases B
    // and C are two separate collectives over the same cells, so each gets its
    // own sub-window (FFN_RS_GROUP_WIN_TOTAL = the pair) and neither inherits the
    // other's counts.
    r.reduceWindowsBytes = static_cast<size_t>(r.cells) * FFN_RS_GROUP_WIN_TOTAL;
    ...
    hostCtx.winSize = static_cast<uint64_t>(FFN_RS_GROUP_WIN_TOTAL);
    uint64_t base = reinterpret_cast<uint64_t>(r.reduce_windows_dev);
    for (int i = 0; i < r.cells && i < HCCL_MAX_RANK_NUM; ++i) {
        hostCtx.windowsIn[i] = base + static_cast<uint64_t>(i) * FFN_RS_GROUP_WIN_TOTAL;
...
static const char* GridPipeFaultName(uint32_t code)
{
    switch (code) {
        ...
        case 0x404:
            return "group publish out of turn";
        case 0x405:
            return "group peer block id not a member of the group";
        default:
            return "unknown";
    }
}
...
{
    // A window is exactly the two per-phase flag headers, so scanning it whole
    // covers both phases' fault sentinels.
    constexpr size_t kFlagWords = static_cast<size_t>(FFN_RS_GROUP_WIN_TOTAL) / sizeof(uint32_t);
    std::vector<uint32_t> flags(static_cast<size_t>(r.cells) * kFlagWords, 0);
    for (int cell = 0; cell < r.cells; ++cell) {
        auto* src =
            reinterpret_cast<uint8_t*>(r.reduce_windows_dev) + static_cast<size_t>(cell) * FFN_RS_GROUP_WIN_TOTAL;
