...
struct DeviceResources {
    ...
    void* row_partial_dev = nullptr;  // [cells, T, H] fp32 (Phase B -> C, written by the col-7 row sinks)
...
{
    // Two windows per cell -- one per reduce phase.  The row group (phase B) and
    // the column group (phase C) bind the same channel indices to different cores,
    // and a group channel is bound by rank rather than negotiated, so the two
    // phases must not share one window's channel space.  winSize below is the
    // per-cell STRIDE, which is what makes the device-side peer resolver map either
    // phase's addresses into the right window.
    r.reduceWindowsBytes = static_cast<size_t>(r.cells) * FFN_RS_GROUP_REDUCE_WIN_STRIDE;
    ...
    hostCtx.winSize = static_cast<uint64_t>(FFN_RS_GROUP_REDUCE_WIN_STRIDE);
    uint64_t base = reinterpret_cast<uint64_t>(r.reduce_windows_dev);
    for (int i = 0; i < r.cells && i < HCCL_MAX_RANK_NUM; ++i) {
        hostCtx.windowsIn[i] = base + static_cast<uint64_t>(i) * FFN_RS_GROUP_REDUCE_WIN_STRIDE;
...
static bool AllocateResources(DeviceResources& r, int physCores)
{
    ...
    // Indexed by BLOCK ID, not by row: the phase-C group fan-in addresses member b's
    // contribution as "this cell's slot + (b - self)*FFN_RS_ROW_PARTIAL_BYTES", so
    // the arena a row sink writes into has to be the mesh-wide one.  Only the four
    // col-7 slots are ever written; the rest is the price of a symmetric arena.
    r.row_partial_dev = MallocBufOrDie(static_cast<size_t>(r.cells) * FFN_RS_ROW_PARTIAL_BYTES, "row_partial");
...
static bool LoadBatcherData(DeviceResources& r) { return r.batcher.LoadAndDistributeNcutReduce(r.dataDir); }
...
static const char* GridPipeFaultName(uint32_t code)
{
    switch (code) {
        ...
        case 0x404:
            return "group geometry does not fit the pipe (size/rank/ring split)";
        case 0x501:
        ...
        case 0x50A:
            return "group channel changed owner before CLOSE + drain";
        default:
            return "unknown";
    }
}
...
static bool CheckGridPipeFaults(DeviceResources& r)
{
    ...
    // Both per-cell phase windows carry their own scoreboard header, so both are
    // scanned (phase 0 = the row-group window, phase 1 = the column-group one).
    const size_t windowCount = static_cast<size_t>(r.cells) * FFN_RS_GROUP_PHASE_WINDOWS;
    std::vector<uint32_t> flags(windowCount * kFlagWords, 0);
    for (size_t w = 0; w < windowCount; ++w) {
        auto* src = reinterpret_cast<uint8_t*>(r.reduce_windows_dev) + w * FFN_RS_GROUP_REDUCE_WIN;
        auto* dst = flags.data() + w * kFlagWords;
        if (aclrtMemcpy(
                dst, kFlagWords * sizeof(uint32_t), src, kFlagWords * sizeof(uint32_t), ACL_MEMCPY_DEVICE_TO_HOST) !=
            ACL_SUCCESS) {
            std::cerr << "[ERROR] GridPipe flag D2H failed (window " << w << ")" << std::endl;
            return false;
        }
    }
    bool ok = true;
    for (size_t w = 0; w < windowCount; ++w) {
        const int cell = static_cast<int>(w / FFN_RS_GROUP_PHASE_WINDOWS);
        const int phaseWin = static_cast<int>(w % FFN_RS_GROUP_PHASE_WINDOWS);
        int row = cell / r.cols;
        int col = cell - row * r.cols;
        const uint32_t* cellFlags = flags.data() + w * kFlagWords;
        for (size_t line = 0; line < kScbLines; ++line) {
            ...
            if (value >= 0x100U) {
                std::cerr << "[ERROR] GridPipe fault cell=" << cell << " row=" << row << " col=" << col
                          << " phaseWindow=" << phaseWin << " word=" << i << " code=0x" << std::hex << value << std::dec
...
static bool VerifyOutput(DeviceResources& r)
{
    ...
    if (!PtoTestCommon::ReadFile(goldenPath, fileSize, golden.data(), outputBytes) || fileSize != outputBytes) {
        std::cerr << "[ERROR] golden.bin mismatch: " << goldenPath << " (got " << fileSize << " expected "
...
static bool LaunchWave(
    DeviceResources& r, int phase, int rowStart, int colStart, int waveCols, int blockCount, const char* tag)
{
    ...
    std::cout << "[INFO] phase " << tag << "  blocks=" << blockCount << "  " << us
              << " us  (rc=" << static_cast<int>(rc) << " faults_ok=" << (faultsOk ? 1 : 0) << ")" << std::endl;
    return faultsOk;
}
...
static bool RunSingleDevice(int physCores)
{
    ...
    std::cout << "[INFO] emulated 32-cell N-cut TREDUCE ReduceSum on " << r.physCores
              << " phys AICores, 3 launches (B=" << bWaves << " EAST waves + 1 SOUTH wave), total " << us << " us"
              << std::endl;
...
int main(int argc, char** argv)
{
    ...
    std::cout << "  per-cell I_shard=" << FFN_NCUT_I_SHARD
              << "  reduce EAST(8-way)+SOUTH(4-way), H-chunk=" << FFN_RS_REDUCE_H_BASE << std::endl;
    std::cout << "================================================================" << std::endl;
