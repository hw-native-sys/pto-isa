...
template <
    ...
    bool CAUSAL_MASK, int SRC_VEC_TN_BUFFERS, typename TileMatQData, typename TileMatKData, typename TileQKData,
    typename TSyncUBBuf>
AICORE inline void compute_qk(
    QKPipe& qkPipe, int tile_id, int sub_tile_id, __gm__ half* q, __gm__ half* k, __gm__ float* qk_tile_fifo,
    TileMatQData& qMatTile, TileMatKData& kMatTile, TileQKData& qkAccTile, uint64_t qkMatTileEventId, int accTileEvtID,
    TSyncUBBuf& ubBufSync, int blk_idx)
{
    if constexpr (DAV_CUBE) {
        ...
        pto_macro_matmul<Cube_S0, Cube_HEAD, Cube_S1>(
            qMatTile, kMatTile, qkAccTile, UF_ENABLE ? AccMode::InitFinalSum : AccMode::Init);
...
template <
    ...
    bool INTERMEDIATE_CHECK, bool CAUSAL_MASK, int OUT_O_TILE_NBUFFERS, typename TileMatPData, typename TileMatVData,
    typename TilePVData, typename TileOutT>
AICORE inline void compute_pv(
    PPipe& pPipe, PVPipe& pvPipe, int tile_id, int sub_tile_id, __gm__ half* v, __gm__ float* pv_tile_fifo,
    TileMatPData& pMatTile, TileMatVData& vMatTile, TilePVData& pvAccTile, TileOutT& runningOTile,
...
template <
    ...
    bool INTERMEDIATE_CHECK, bool CAUSAL_MASK, typename TileDataF_T, typename TileDataH_T, typename TileDataH_NZ_T,
    typename ReduceTileF_T, typename TileMatPData, typename TSyncUBBuf>
AICORE inline void compute_p(
    QKPipe& qkPipe, PPipe& pPipe, int tile_id, int row_slice, __gm__ half* p_tile_fifo, __gm__ float* exp_max_ififo,
    TileDataF_T& qkVecTile, TileDataH_T& x_expT, TileDataF_T& input_reduce_tmp, ReduceTileF_T& m1_local_max,
    ReduceTileF_T& l1_local_sum, ReduceTileF_T& m2_global_max, ReduceTileF_T& l2_global_sum,
    ReduceTileF_T& l1_exp_max_ififo, TileMatPData& pMatTile, TileDataH_NZ_T& nzConvBuffer, uint64_t pTileEventId,
    TSyncUBBuf& ubBufSync, int blk_idx)
{
    constexpr uint32_t Cube_S0 = CUBE_S0;
    constexpr uint32_t Cube_S1 = CUBE_S1;
...
template <
    typename PVPipe, int S0, int HEAD_SIZE, int S1, int CUBE_S0, int TILE_S1, bool INTERMEDIATE_CHECK, bool CAUSAL_MASK,
    int SRC_VEC_TN_BUFFERS, int OUT_O_TILE_NBUFFERS, typename TileOutT, typename ReduceTileF_T>
AICORE inline void compute_gu(
    PVPipe& pvPipe, int tile_id, int num_tiles, __gm__ float* o_out, TileOutT& runningOTile, TileOutT& pvVecTile,
    ...
    __gm__ uint8_t* profile_buf)
{
    ...
    using TileDataH_NZ_T = Tile<
        TileType::Vec, half, NzBufRows, Cube_S1, BLayout::ColMajor, Vec_S0, Cube_S1, SLayout::RowMajor, 512,
    ...
    allocate_vec_tile_buffers<
        TileDataF_T, ReduceTileF_T, TileDataH_T, TileOutGuT, srcVecTNBuffers, xexpVecTNBuffers, outOTileNBuffers>(
    ...
    using QKPipe = TMPipe<
        BUF0_QK_READY, FIFOType::GM_FIFO, QKFiFoDepth, QKFiFoSyncT, TileQKData, TileDataF_T, UF_ENABLE ? true : false,
        0>;
    ...
    using PVPipe = TMPipe<
        UPDATE_READY, FIFOType::GM_FIFO, PVFiFoDepth, PVFiFoSyncT, TilePVData, TileOutGuT, UF_ENABLE ? true : false, 0>;
        ...
        if constexpr (DAV_CUBE) {
            for (int sub_tile = 0; sub_tile < static_cast<int>(kTileFactor); ++sub_tile) {
                ...
                compute_qk<
                    ...
                    qkAccTileEvtID, ubBufSync, block_idx);
                k_src_pingpong_id++;
            }
        }
        if constexpr (DAV_VEC) {
    ...
    for (int tile_id = 0; tile_id < num_tiles_s1; ++tile_id) {
        ...
        for (int sub_tile = 0; sub_tile < static_cast<int>(kTileFactor); ++sub_tile) {
...
#endif
                    compute_p<
                        QKPipe, PPipe, S0, HEAD_SIZE, S1, CUBE_S0, CUBE_S1, Tile_S1, INTERMEDIATE_CHECK, CAUSAL_MASK>(
            ...
            if constexpr (DAV_CUBE) {
                ...
                compute_pv<
                    ...
                    outOTileNBuffers>(
                    pPipe, pvPipe, tile_id, sub_tile, v, pv_tile_fifo_block,
                    pMatTile[pv_src_pingpong_id % pMatTNBuffers], vMatTile[pv_src_pingpong_id % vMatTNBuffers],
                    pvAccTile, runningOTile, pv_src_pingpong_id % vMatTNBuffers + PV_EVENT_ID0, pvAccTileEvtID,
                    block_idx);
                pv_src_pingpong_id++;
            }
        }
        ...
        if constexpr (DAV_VEC) {
            ...
            compute_gu<
                PVPipe, S0, HEAD_SIZE, S1, CUBE_S0, Tile_S1, INTERMEDIATE_CHECK, CAUSAL_MASK, srcVecTNBuffers,
                outOTileNBuffers>(
