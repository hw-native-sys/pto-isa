...
本示例在 Ascend950 A5 上使用 PTO 实现 MoE combine 阶段。它对应
...
| A5 HCCL window 头部 guard 处理 | 上层公共 ABI 适配层 |
...
`localWindowBase` 是 HCCL window 的原始起始地址。A5 在 window 头部保留 4096B head guard；传给 kernel 的
`peerWindow` 指向 live payload，即 `localWindowBase + 4096`。
...
| `aivBlocks` | 逻辑 AIV block 数；A5 默认 `40`，可传参覆盖 |
...
- **DCCI 批量 acquire**：每个 token 在消费返回的 `ptrD` 行前先刷新对应 GM range，然后对本轮 cached routes
  做一次 `dsb(DSB_DDR)`。
- **A5 restore event chain**：A5 restore loop 使用 PTO event 串联 `TAXPY -> next TLOAD`，减少 topK loop 内
  不必要的 `PIPE_ALL` barrier。
...
| `aivBlocks` | `40` | A5 resource model 默认值；可通过 `--aiv-blocks N` 覆盖 |
| 内部 Vector tile 列宽 | `1024` | 示例实现固定值 |
| 内部 return chunk | `8 rows` | 固定的 return 阶段行 chunk |
| 内部 metadata pad | `16` | expert metadata 对齐粒度 |

常用调试命令 `--aiv-blocks 24`，并使用 `PES=2, M=64, K=7168, topK=8, expertPerPe=2` 时，各布局大小为：
...
Host 会在每轮迭代前清零包含 head guard 和 live payload 的 HCCL window 区域，因此 kernel 固定等待每个 peer
的一次 notify。前置 `MPIBarrier` 只用于保证所有 rank 完成清理后再 launch kernel，不计入 `combine_e2e`。
...
A5 使用 `TAXPY` 到下一次 `TLOAD` 的 event chain，保证 topK loop 顺序，同时避免每个 route 后都插入完整 pipe
barrier。

## 实测性能

当前开发环境可编译 A5 工程；A5 runtime 数字需要在 A5 机器上采集。脚本会输出如下 profile：
...
| `--aiv-blocks` | `0 -> 40` | 逻辑 AIV block 数，用于匹配不同硬件资源规划 |
