...
AICORE inline void ComputeAndStoreTile(
    ...
    int linear_idx, uint32_t mi, uint32_t ni, uint32_t k_per_rank, int32_t& enqueue_slot)
{
...
AICORE inline void GemmComputeImpl(
    __gm__ half* gemm_output, __gm__ half* src0, __gm__ half* src1, __gm__ MultiBlockQueueSet* queue_set,
    int launch_block_count, uint32_t k_per_rank)
{
    ...
    for (int linear_idx = my_start_tile; linear_idx < my_end_tile && linear_idx < total_tiles; linear_idx++) {
        ...
        ComputeAndStoreTile(
            gemm_output, src0, src1, my_queue, aMatTile, bMatTile, aTile, bTile, cTile, linear_idx, mi, ni, k_per_rank,
            enqueue_slot);
    }
}
