...
**大 Tile 支持**：当单 rank 数据在行和/或列方向超过 UB Tile 容量时，指令会自动通过二维滑动进行分块传输。
...
// Ping-pong scatter (使用两个暂存 Tile 实现双缓冲)
template <CollEngine engine = CollEngine::AIV,
          typename ParallelGroupType, typename GlobalSrcData, typename TileData, typename... Args>
PTO_INST RecordEvent TSCATTER(ParallelGroupType &parallelGroup, GlobalSrcData &srcGlobalData,
...
> **CCU 路径**：与 AIV 路径（仅根节点调用 `TSCATTER`）不同，CCU 路径要求所有 rank 通过 Host 侧 `HcclCcuKernelRegister` / `HcclCcuKernelLaunch` 注册并启动 CCU kernel。完整示例参见 `tests/npu/a5/comm/st/testcase/tscatter_ccu/`。
