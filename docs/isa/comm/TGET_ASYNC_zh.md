...
- **SDMA**：`TGET_ASYNC` 仅提交数据传输 SQE 并敲 DoorBell，flag SQE 延迟到 `Wait` 时提交，`Wait` 通过轮询 flag 判断完成。
- **URMA (仅 Ascend950 / NPU_ARCH 3510)**：`TGET_ASYNC` 立即提交 RDMA READ WQE 并敲 DoorBell。`Wait` 通过轮询 Completion Queue (CQ) 等待所有预期的 CQE 被消费。
...
`Wait` 成功后，所有读入的 `dstGlobalData` 均已就绪。
