...
![TROWEXPANDDIV 模式 1 tile operation](../figures/isa/TROWEXPANDDIV.svg)

### 模式 2 — 每行 32 字节块（RowMajor src1）

![TROWEXPANDDIV 模式 2 tile operation](../figures/isa/TROWEXPANDDIV_mode2.svg)

## 简介

行广播除法：将 `src0` 的每一行除以扩展操作数的每行标量。
...
- `TileDataDst::DType`、`TileDataSrc0::DType`、`TileDataSrc1::DType` 必须是以下之一：`half`、`float`（适用于A2、A3和A5）、`int16`、`int32`、`uint16`、`uint32`（仅适用于A5）。
...
- **A2A3**：tmp Tile 作为广播缓冲区使用。ColMajor 扩展操作数的每行标量值通过 `vbrcb` 指令广播到 tmp 缓冲区，为每行创建一个 32 字节块，然后在二元运算中作为扩展操作数使用。`vbrcb` 指令的 repeat stride 为 8 个块（256字节），每个 repeat 处理 8 行。最小 tmp 大小计算：
    - **公共参数**：
