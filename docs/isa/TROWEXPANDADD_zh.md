...
行广播加法：将扩展操作数的每行标量加到全尺寸操作数（`src0` 或 `src1`）的每一行。
...
- `TileDataDst::DType`、`TileDataSrc0::DType`、`TileDataSrc1::DType` 必须是以下之一：`half`、`float`、`int16`、`int32`（适用于Atlas A2 训练系列产品/Atlas A2 推理系列产品、Atlas A3 训练系列产品/Atlas A3 推理系列产品和Ascend 950PR/Ascend 950DT），`uint16`、`uint32`、`bfloat16_t`、`int8`、`uint8`、`int64`、`uint64`（仅适用于Ascend 950PR/Ascend 950DT）。
