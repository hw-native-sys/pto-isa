...
当前向量微指令参考页使用两个 VPTO 来源交叉校验：

- `PTOAS/docs/vpto-spec.md`（最新抓取的 `feature_vpto_backend` 分支）负责对齐操作名、操作数合法性和执行面语义。
- `~/visa.txt` 负责对齐公开 Vector Thread ISA 文本，尤其是公开披露的时延或流级吞吐描述。
