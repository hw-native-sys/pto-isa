...
| 等待操作 | `wait_flag_dev` | `wait_intra_block` |
...
wait_intra_block(wait_pipe, semaphore_id);
```
...
- Vector→Cube：Cube 侧要分别对两个 ID 做两次 `wait_intra_block`
...
- **语法：** `pto.wait_intra_block %block_id, %event_id : i64, i64`
...
A5 的 wait_intra_block "PIPE_MTE2"：
    只阻塞 PIPE_MTE2
