...
`pto.tcolexpanddiv` lowers to a per-column `vdiv` sequence in which a single per-column scalar from `src1` is broadcast across the working tile and combined with each element of `src0` via division.

**Cycle model**:

```
total = startup + R × (per_column_vdiv + interval)
```

where `R = dst.GetValidRow()` (row-direction) or `R = dst.GetValidCol()` (column-direction), and `per_column_vdiv` scales with `C = dst.GetValidCol()` rounded up to the native vector width.

### Instruction Sequence by Shape (FP32)

| Valid Shape | Instruction Sequence | Estimated Cycles |
|-------------|----------------------|------------------|
| 16×16 | `vdiv`*16 → PIPE_V | ~O(64) |
| 32×32 | `vdiv`*32 → PIPE_V | ~O(128) |
| 64×64 | `vdiv`*64 → PIPE_V | ~O(256) |
| R×C   | `vdiv`*R → PIPE_V  | ~O(R × ⌈C / vlen⌉) |
...
|--------|----------|--------------|
| `RowMajor` | ≥ vlen (FP32) | Continuous fast path; one `vdiv` per column |
