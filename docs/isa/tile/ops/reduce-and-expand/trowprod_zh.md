...
- 当前实现路径中，没有额外要求 `tmp` 必须满足特定shape/layout约束。
...
`tmp` 被接口接受但Ascend 950PR/Ascend 950DT实现**不使用**。Ascend 950PR/Ascend 950DT后端使用基于向量寄存器的归约（`vmul` + `vintlv` 进行树形归约），不需要暂存Tile存储。`tmp` 仅为了与Atlas A2/A3 训练系列产品/Atlas A2/A3 推理系列产品的API兼容性而保留在C++内建接口签名中。
