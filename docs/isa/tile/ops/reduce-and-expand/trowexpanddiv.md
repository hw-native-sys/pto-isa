...
`pto.trowexpanddiv` lowers to a per-row `vdiv` sequence in which a single per-row scalar from `src1` is broadcast across the working tile and combined with each element of `src0` via division.

**Cycle model**:

```
total = startup + R × (per_row_vdiv + interval)
```

where `R = dst.GetValidRow()` (row-direction) or `R = dst.GetValidCol()` (row-direction), and `per_row_vdiv` scales with `C = dst.GetValidCol()` rounded up to the native vector width.
...
|--------|----------|--------------|
| `RowMajor` | ≥ vlen (FP32) | Continuous fast path; one `vdiv` per row |
