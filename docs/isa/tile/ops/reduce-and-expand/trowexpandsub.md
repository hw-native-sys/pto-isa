...
`pto.trowexpandsub` lowers to a per-row `vsub` sequence in which a single per-row scalar from `src1` is broadcast across the working tile and combined with each element of `src0` via subtraction.

**Cycle model**:

```
total = startup + R × (per_row_vsub + interval)
```

where `R = dst.GetValidRow()` (row-direction) or `R = dst.GetValidCol()` (row-direction), and `per_row_vsub` scales with `C = dst.GetValidCol()` rounded up to the native vector width.
...
|--------|----------|--------------|
| `RowMajor` | ≥ vlen (FP32) | Continuous fast path; one `vsub` per row |
