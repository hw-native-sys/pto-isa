...
`pto.tcolexpandmul` lowers to a per-column `vmul` sequence in which a single per-column scalar from `src1` is broadcast across the working tile and combined with each element of `src0` via multiplication.

**Cycle model**:

```
total = startup + R × (per_column_vmul + interval)
```

where `R = dst.GetValidRow()` (row-direction) or `R = dst.GetValidCol()` (column-direction), and `per_column_vmul` scales with `C = dst.GetValidCol()` rounded up to the native vector width.
...
|--------|----------|--------------|
| `RowMajor` | ≥ vlen (FP32) | Continuous fast path; one `vmul` per column |
