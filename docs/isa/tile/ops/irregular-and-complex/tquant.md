...
DN data FP8 mantissas share the same physical addresses as ND (the `(r,c)` element is identical), so the 2-arg `TMOV` ND→NZ is correct for DN data unchanged. Only the **exponent** path differs (DN→ZZ via `TMOV<0>`). See [`TQUANT_DN.md`](../../../TQUANT_DN.md) for the DN→ZZ transform.
...
See [`TQUANT_DN.md`](../../../TQUANT_DN.md) for the DN→ZZ transform details and `tests/npu/a5/src/st/testcase/tquant_dn/` for a complete ST example.
