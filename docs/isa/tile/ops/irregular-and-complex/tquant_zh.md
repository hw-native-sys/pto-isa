...
| **2. 指数 + 缩放** | $s_g = \mathrm{biasedExp}(m_g) - e_{\max}$；$\alpha_g = 2^{254 - s_g}$ | `exp`（E8M0，1 B/组）、`scaling`（scratch，FP） |
...
| $R \cdot C \le 59461$ | MX（UB 256 KB） | 复用后的缓冲预算 |
...
DN 数据的 FP8 mantissa 与 ND 共享相同的物理地址（`(r,c)` 元素完全相同），因此 2 参数 `TMOV` ND→NZ 对 DN 数据同样适用。仅**指数**路径不同（DN→ZZ 通过 `TMOV<0>`）。详见 [`TQUANT_DN.md`](../../../TQUANT_DN.md)。
...
详见 [`TQUANT_DN.md`](../../../TQUANT_DN.md)（DN→ZZ 转换）和 `tests/npu/a5/src/st/testcase/tquant_dn/`（完整 ST 示例）。
