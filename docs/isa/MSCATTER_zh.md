...
**形状约束：**
- 填充后的 `TileSrc::Cols * sizeof(T)` 必须 32 字节对齐。
...
**形状约束：**
- 填充后的 `TileSrc::Cols * sizeof(T)`（RowMajor）或 `TileSrc::Rows * sizeof(T)`（ColMajor）必须 32 字节对齐。
...
- **常量。** `kC0 = 32 / sizeof(T)`；`kFRow = 16`。每个分形块为 `16 × kC0` 元素（= 512 B）。
...
AIV 向量核心 192 KB UB。`MSCATTER` 不从内核内部分配任何 UB scratch — 仅消耗调用者分配的源 tile 和索引 tile。
...
默认安全工作集 ≤ 128 KB。超过时需通过 `kernel_name<<<numBlocks, dynUBufSize, stream>>>(args...)` 显式声明。
