...
- 原因：ccec/bisheng 路径下 `GetTaskRation()` 恒为 **2**，`dav-c220` 自动拆分物理固定 **1:2**，无法得到真 1:1。
...
- 启动：AIC 与 AIV 分别在两个 `aclrtStream` 上 chevron `<<<aic>>>`；`aicBlocks` / `totalParticipants` 由 Host 运行时传入。
...
- `dcci` / DMA 以 **32B cache line** 为粒度操作；若相邻核 slot 共享同一条 cache line，跨核刷新会互相覆盖 / 丢写。
