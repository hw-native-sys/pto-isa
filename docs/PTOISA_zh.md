...
| 逐元素（Tile-Tile） | [`TSUBRELU`](isa/TSUBRELU_zh.md) | src0和src1逐元素相减后ReLU。 |
| 逐元素（Tile-Tile） | [`TFUSEDMULADDRELU`](isa/TFUSEDMULADDRELU_zh.md) | 三元逐元素运算：`ReLU(src0 * dst + src1)`。 |
...
| 轴归约 / 扩展 | [`TROWARGMIN`](isa/TROWARGMIN_zh.md) | 获取每行最大值对应列索引。 |
...
| 手动 / 资源绑定 | [`TSUBVIEW`](isa/TSUBVIEW_zh.md) | 表达一个tile是另一个tile的subview。 |
...
| 复杂指令 | [`TSCATTER`](isa/TSCATTER_zh.md) | 使用逐元素扁平偏移将源 Tile 元素散播到目标 Tile 中。 |
| 复杂指令 | [`TQUANT`](isa/TQUANT_zh.md) | 量化 Tile（例如 FP32 到 FP8），生成指数/缩放/最大值输出。 |
| 复杂指令 | [`TADDReluConv`](isa/TADDReluConv_zh.md) | 融合逐元素加法、ReLU 限幅和类型转换：dst = convert(max(0, src0 + src1))。 |
