...
void runTloadDynamic(__gm__ T *out, __gm__ T *src)
{
    ...
    float costResult = convTile.GetCycle();
...
{
    runTloadDynamic<half, 1, 2, 4, 4, Layout::NC1HWC0, 32.0f, 0.0f>((__gm__ half *)o, (__gm__ half *)s);
}

extern "C" __global__ AICORE void launch_2(__gm__ uint8_t *o, __gm__ uint8_t *s)
{
    runTloadDynamic<float, 1, 4, 10, 10, Layout::NC1HWC0, 400.0f, 0.0f>((__gm__ float *)o, (__gm__ float *)s);
}

extern "C" __global__ AICORE void launch_3(__gm__ uint8_t *o, __gm__ uint8_t *s)
{
    runTloadDynamic<half, 16, 2, 1, 18, Layout::FRACTAL_Z, 576.0f, 0.0f>((__gm__ half *)o, (__gm__ half *)s);
}

extern "C" __global__ AICORE void launch_4(__gm__ uint8_t *o, __gm__ uint8_t *s)
{
    runTloadFractalZ5D<int8_t, 4, 2, 6, 16, 768.0f, 0.0f>((__gm__ int8_t *)o, (__gm__ int8_t *)s);
}
