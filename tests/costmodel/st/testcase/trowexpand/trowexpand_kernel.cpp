...
void LaunchTRowExpand(T *out, T *src, void *stream)
{
    if constexpr (std::is_same_v<T, aclFloat16>)
        runTRowExpand<half, kGRows_, kGCols_, kTRows_, kTCols_, profiling, accuracy>((half *)(out), (half *)(src));
    else
        runTRowExpand<T, kGRows_, kGCols_, kTRows_, kTCols_, profiling, accuracy>(out, src);
}

template void LaunchTRowExpand<float, 64, 64, 64, 64, 526.0f, 1.0f>(float *out, float *src, void *stream);
template void LaunchTRowExpand<aclFloat16, 64, 64, 64, 64, 526.0f, 1.0f>(aclFloat16 *out, aclFloat16 *src,
                                                                         void *stream);
template void LaunchTRowExpand<int16_t, 64, 64, 64, 64, 526.0f, 1.0f>(int16_t *out, int16_t *src, void *stream);
template void LaunchTRowExpand<aclFloat16, 16, 256, 16, 256, 142.0f, 1.0f>(aclFloat16 *out, aclFloat16 *src,
                                                                           void *stream);
