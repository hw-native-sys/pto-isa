...
template <typename outType, typename AType, typename BType, typename BiasType, int validM, int validK, int validN,
          bool isBias, float profiling, float accuracy>
__global__ AICORE void RunTMATMUL(__gm__ outType *out, __gm__ AType *src0, __gm__ BType *src1, __gm__ BiasType *src2)
{
    ...
    // accuracy compare
    float costResult = cTile.GetCycle();
    float precision = 1 - fabs(profiling - costResult) / profiling;
    bool ret = precision >= accuracy;
    EXPECT_TRUE(ret);
}

template <typename T, typename U, typename S, int M, int K, int N, float profiling, float accuracy>
...
void LaunchTMATMUL(uint8_t *out, uint8_t *src0, uint8_t *src1, void *stream)
{
    if constexpr (tilingKey == 1) {
        // half A[40x50] * half B[50x60]: baskK=16, repeats=3*4*4=48, cycles=14+48*1=62
        RunTMATMUL<float, half, half, float, 40, 50, 60, false, 62.0f, 1.0f>(
...
{
    if constexpr (tilingKey == 1) {
        // int8 A[8x7] * int8 B[7x6]: baskK=32, repeats=1*1*1=1, cycles=14+1*1=15
        RunTMATMUL<int32_t, int8_t, int8_t, int32_t, 8, 7, 6, true, 15.0f, 1.0f>(
            reinterpret_cast<int32_t *>(out), reinterpret_cast<int8_t *>(src0), reinterpret_cast<int8_t *>(src1),
            reinterpret_cast<int32_t *>(src2));
    } else if constexpr (tilingKey == 2) {
        // half A[16x15] * half B[15x16]: baskK=16, repeats=1*1*1=1, cycles=14+1*1=15
        RunTMATMUL<float, half, half, float, 16, 15, 16, true, 15.0f, 1.0f>(
            reinterpret_cast<float *>(out), reinterpret_cast<half *>(src0), reinterpret_cast<half *>(src1),
            reinterpret_cast<float *>(src2));
    } else if constexpr (tilingKey == 5) {
        // float A[127x128] * float B[128x63]: baskK=8, repeats=8*4*16=512, cycles=14+512*2=1038
        RunTMATMUL<float, float, float, float, 127, 128, 63, true, 1038.0f, 1.0f>(
