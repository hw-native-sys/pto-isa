...
{
    bool ret = TSUBSTestFramework<1, float, 32, 64, 32, 32, 64, 64>();
    EXPECT_TRUE(ret);
}

TEST_F(TSUBSTest, case2)
{
    bool ret = TSUBSTestFramework<2, aclFloat16, 63, 64, 63, 63, 64, 64>();
    EXPECT_TRUE(ret);
}

TEST_F(TSUBSTest, case3)
{
    bool ret = TSUBSTestFramework<3, int32_t, 31, 128, 31, 31, 128, 128>();
    EXPECT_TRUE(ret);
}
...
{
    bool ret = TSUBSTestFramework<5, float, 7, 448, 7, 7, 448, 448>();
    EXPECT_TRUE(ret);
}

TEST_F(TSUBSTest, case6)
{
    bool ret = TSUBSTestFramework<6, float, 256, 16, 256, 256, 16, 16>();
    EXPECT_TRUE(ret);
}

TEST_F(TSUBSTest, case7)
{
    bool ret = TSUBSTestFramework<7, float, 32, 128, 32, 32, 64, 64>();
    EXPECT_TRUE(ret);
}

TEST_F(TSUBSTest, case8)
{
    bool ret = TSUBSTestFramework<8, aclFloat16, 63, 128, 63, 63, 64, 64>();
    EXPECT_TRUE(ret);
}

TEST_F(TSUBSTest, case9)
{
    bool ret = TSUBSTestFramework<9, int32_t, 31, 256, 31, 31, 128, 128>();
    EXPECT_TRUE(ret);
}

TEST_F(TSUBSTest, case10)
{
    bool ret = TSUBSTestFramework<10, int16_t, 15, 192, 15, 15, 192, 192>();
    EXPECT_TRUE(ret);
}

TEST_F(TSUBSTest, case11)
{
    bool ret = TSUBSTestFramework<11, float, 7, 512, 7, 7, 448, 448>();
    EXPECT_TRUE(ret);
}

TEST_F(TSUBSTest, case12)
{
    bool ret = TSUBSTestFramework<12, float, 256, 32, 256, 256, 16, 16>();
    EXPECT_TRUE(ret);
}