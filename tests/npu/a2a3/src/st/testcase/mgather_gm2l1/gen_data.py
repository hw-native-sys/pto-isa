...
def nd_to_nz(arr_2d, c0):
    ...
    n_block_rows = r // 16
    n_block_cols = c // c0
    out = np.zeros((n_block_cols, n_block_rows, 16, c0), dtype=arr_2d.dtype)
    for bc in range(n_block_cols):
        for br in range(n_block_rows):
            out[bc, br] = arr_2d[br * 16 : (br + 1) * 16, bc * c0 : (bc + 1) * c0]
...
def case_row(name, dtype, r, c, table_rows, oob="undefined", idx_kind="random"):
    ...
    return table.reshape(-1), idx, nd_to_nz(golden_nd, c0_of(dtype))
...
def case_elem(name, dtype, r, c, table_size, oob="undefined", idx_kind="random"):
    ...
    return table, idx, nd_to_nz(golden_nd, c0_of(dtype))
...
if __name__ == "__main__":
    ...
    print("All MGATHER GM2L1 A2/A3 test data generated successfully")
