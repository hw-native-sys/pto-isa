...
__tf__ PTO_INTERNAL void tf_copy_cbuf_to_gm(
    __gm__ T __out__* dst, typename TileDataSrc::TileDType __in__ src, uint8_t n, uint16_t nBurst, uint16_t lenBurst,
    uint16_t l1Gap, uint16_t gmGap)
{
    copy_cbuf_to_gm(dst, __cce_get_tile_ptr(src), n, nBurst, lenBurst, l1Gap, gmGap);
}
...
template <
    typename T, int dstN, int dstD, int dstC1, int dstH, int dstW, int gWholeShape0, int gWholeShape1, int gWholeShape2,
    int gWholeShape3, int gWholeShape4>
AICORE inline void RunTLoad6HD(__gm__ T __out__* out, __gm__ T __in__* src)
{
    constexpr int dstC0 = 32 / sizeof(T);
    constexpr int gStride[5] = {
    ...
    // for auto mode, bufferSize is a misleading variable name in convTile, it shouldn't be number of bytes it should be
    // the number of elements
    constexpr int bufferSize = dstN * dstD * dstC1 * dstH * dstW * dstC0; // * sizeof(T);
    constexpr int validRow = dstN * dstD * dstC1 * dstH * dstW;
    constexpr int validCol = dstC0;
    constexpr int Rows = dstN * dstD * dstC1 * dstH * dstW;
    constexpr int Cols = (dstC0 + blockSize - 1) / blockSize * blockSize;

    using ShapeDim5 = pto::Shape<dstN, dstD, dstC1, dstH, dstW>;
    using StridDim5 = pto::Stride<gStride[0], gStride[1], gStride[2], gStride[3], gStride[4]>;
    using GlobalDataIn = GlobalTensor<T, ShapeDim5, StridDim5, Layout::NDC1HWC0>;
    using TileData = ConvTile<
        TileType::Mat, T, bufferSize, Layout::NDC1HWC0, pto::ConvTileShape<dstN, dstD, dstC1, dstH, dstW, dstC0>>;
...
// C1HWNC0
template <
    typename T, int dstC1, int dstH, int dstW, int dstN, int dstC0, int gWholeShape0, int gWholeShape1,
    int gWholeShape2, int gWholeShape3, int gWholeShape4>
...
void LaunchTLoad(T* out, T* src, void* stream)
{
    if constexpr (std::is_same_v<T, uint16_t>) {
        TLoadKernel<
            bfloat16_t, format, gShape0, gShape1, gShape2, gShape3, gShape4, gWholeShape0, gWholeShape1, gWholeShape2,
            gWholeShape3, gWholeShape4>
            <<<1, nullptr, stream>>>(reinterpret_cast<bfloat16_t*>(out), reinterpret_cast<bfloat16_t*>(src));
    } else {
        TLoadKernel<
            T, format, gShape0, gShape1, gShape2, gShape3, gShape4, gWholeShape0, gWholeShape1, gWholeShape2,
...
template void LaunchTLoad<5, uint16_t, 1, 16, 10, 100, 16, 1, 16, 100, 100, 16>(
    uint16_t* out, uint16_t* src, void* stream);
template void LaunchTLoad<5, uint16_t, 10, 16, 16, 2, 16, 256, 16, 100, 16, 16>(
    uint16_t* out, uint16_t* src, void* stream);
template void LaunchTLoad<5, uint16_t, 1, 1, 1, 8192, 16, 8, 16, 16, 8192, 16>(
    uint16_t* out, uint16_t* src, void* stream);
...
template void LaunchTLoad<7, uint16_t, 1, 49, 7, 16, 16, 1, 980, 32, 16, 16>(
    uint16_t* out, uint16_t* src, void* stream);
...
template void LaunchTLoad<8, int8_t, 1, 2, 3, 16, 128, 2, 3, 4, 1024, 1024>(int8_t* out, int8_t* src, void* stream);
template void LaunchTLoad<8, int8_t, 1, 3, 4, 128, 8, 2, 3, 4, 128, 128>(int8_t* out, int8_t* src, void* stream);
template void LaunchTLoad<8, int8_t, 1, 3, 4, 8, 128, 2, 3, 8, 8, 128>(int8_t* out, int8_t* src, void* stream);
template void LaunchTLoad<8, uint16_t, 1, 1, 16, 10, 100, 2, 1, 16, 100, 100>(
    uint16_t* out, uint16_t* src, void* stream);
template void LaunchTLoad<8, uint16_t, 1, 1, 10, 16, 2, 2, 2, 256, 16, 100>(uint16_t* out, uint16_t* src, void* stream);
