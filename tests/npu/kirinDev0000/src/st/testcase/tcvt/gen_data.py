...
class _Bfloat16Type:
    """Sentinel type for bfloat16 - data stored as BF16 bit patterns (uint16)"""

    pass


bfloat16 = _Bfloat16Type()


def _is_bf16(t):
    return isinstance(t, _Bfloat16Type)
...
def gen_golden(case_name, param):
    ...
    float_types = tuple(t for t in (np.float32, np.float16) if t is not None)
    int8_like_types = tuple(t for t in (np.int8, fp8_e5m2, fp8_e4m3, hifloat8) if t is not None)

    # Generate input data
    if _is_bf16(srctype):
        x1_f32 = (np.random.random([m, n]) * 200 - 100).astype(np.float32)
        x1_gm = _float32_to_bf16_bits(x1_f32)
    elif srctype in float_types:
    ...
    is_float_src = _is_bf16(srctype) or np.issubdtype(srctype, np.floating)
    is_int_dst = not _is_bf16(dsttype) and np.issubdtype(dsttype, np.integer)
    is_f32_to_f32 = srctype == np.float32 and dsttype == np.float32
    needs_rounding = is_float_src and (is_int_dst or is_f32_to_f32)

    # For BF16 source, decode bit patterns to float32 for golden computation
    if _is_bf16(srctype):
        x1_for_golden = _bf16_bits_to_float32(x1_gm)
    else:
        x1_for_golden = x1_gm
    ...
    if needs_rounding:
        if mode == "RoundMode::CAST_NONE":
            ...
            converted_golden = np.trunc(x1_for_golden) if is_int_dst else np.rint(x1_for_golden)
        else:
            converted_golden = rounding_funcs.get(mode, lambda x: x)(x1_for_golden)
    else:
        converted_golden = x1_for_golden

    # Apply saturation mode (default per conversion type)
    if _is_bf16(dsttype):
        golden = _float32_to_bf16_bits(converted_golden)
    elif np.issubdtype(dsttype, np.integer):
    ...
    if valid_m < m or valid_n < n:
        output_dtype = np.uint16 if _is_bf16(dsttype) else dsttype
        output = np.zeros([m, n], dtype=output_dtype)
...
if __name__ == "__main__":
    ...
    type_pairs = [
        ...
        # FP4 not supported on kirinDev0000
        # U8/I8 conversions
        ...
        # I64 conversions (not supported on kirinDev0000)
    ]
