...
{
    bool ret = TRowMaxTestFramework<5, float, 7, 7, 448, 448, 1>();
    EXPECT_TRUE(ret);
}
...
{
    bool ret = TRowMaxTestFramework<7, float, 64, 64, 128, 128, 1>();
    EXPECT_TRUE(ret);
}

TEST_F(TROWMAXTest, case8)
{
    bool ret = TRowMaxTestFramework<8, float, 32, 32, 256, 256, 1>();
    EXPECT_TRUE(ret);
}

TEST_F(TROWMAXTest, case9)
{
    bool ret = TRowMaxTestFramework<9, float, 16, 16, 512, 512, 1>();
    EXPECT_TRUE(ret);
}

TEST_F(TROWMAXTest, case10)
{
    bool ret = TRowMaxTestFramework<10, float, 8, 8, 1024, 1024, 1>();
    EXPECT_TRUE(ret);
}

TEST_F(TROWMAXTest, case11)
{
    bool ret = TRowMaxTestFramework<11, int32_t, 63, 63, 64, 64, 1>();
    EXPECT_TRUE(ret);
}

TEST_F(TROWMAXTest, case12)
{
    bool ret = TRowMaxTestFramework<12, int16_t, 15, 15, 192, 192, 1>();
    EXPECT_TRUE(ret);
}
TEST_F(TROWMAXTest, case13)
{
    bool ret = TRowMaxTestFramework<13, aclFloat16, 64, 64, 128, 128, 1>();
    EXPECT_TRUE(ret);
}
TEST_F(TROWMAXTest, case14)
{
    bool ret = TRowMaxTestFramework<14, int32_t, 32, 32, 256, 256, 1>();
    EXPECT_TRUE(ret);
}
TEST_F(TROWMAXTest, case15)
{
    bool ret = TRowMaxTestFramework<15, int16_t, 16, 16, 512, 512, 1>();
    EXPECT_TRUE(ret);
}
