...
__global__ AICORE void runTHistogram(__gm__ uint16_t* src, __gm__ uint32_t* dst, __gm__ uint8_t* idx)
{
    ...
    TASSIGN<0x0>(srcTile);
    TASSIGN<TileDataSrc::Numel * sizeof(uint16_t)>(dstTile);
    TASSIGN<TileDataSrc::Numel * sizeof(uint16_t) + TileDataDst::Numel * sizeof(uint32_t)>(idxTile);
...
// ---------------------------------------------------------------------------
template <int validRows, int validCols, HistByte byte>
__global__ AICORE void runTHistogramU32(__gm__ uint32_t* src, __gm__ uint32_t* dst, __gm__ uint8_t* idx)
{
    ...
    TASSIGN<0x0>(srcTile);
    TASSIGN<TileDataSrc::Numel * sizeof(uint32_t)>(dstTile);
    TASSIGN<TileDataSrc::Numel * sizeof(uint32_t) + TileDataDst::Numel * sizeof(uint32_t)>(idxTile);
