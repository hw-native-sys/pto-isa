...
__global__ AICORE void runTRowArgMin(__gm__ TDst* out, __gm__ TSrc* src)
{
    ...
    constexpr size_t dstSize = sizeof(TDst) * dstTileH * dstTileW;
    constexpr size_t srcSize = sizeof(TSrc) * srcTileH * srcTileW;
    constexpr size_t srcOffset = dstSize;
    constexpr size_t tmpOffset = srcOffset + srcSize;
    TASSIGN<0x0>(dstTile);
    TASSIGN<srcOffset>(srcTile);
...
__global__ AICORE void runTRowArgMin(__gm__ TVal* outVal, __gm__ TIdx* outIdx, __gm__ TVal* src)
{
    using DynShape = pto::Shape<1, 1, 1, -1, -1>;
    using DynStride = pto::Stride<-1, -1, -1, -1, -1>;
    ...
    constexpr size_t dstValSize = sizeof(TVal) * dstValTileH * dstValTileW;
    constexpr size_t dstIdxSize = sizeof(TIdx) * dstIdxTileH * dstIdxTileW;
    constexpr size_t srcSize = sizeof(TVal) * srcTileH * srcTileW;
    constexpr size_t dstValOffset = 0;
    constexpr size_t dstIdxOffset = dstValOffset + dstValSize;
    constexpr size_t srcOffset = dstIdxOffset + dstIdxSize;
    constexpr size_t tmpOffset = srcOffset + srcSize;
    TASSIGN<dstValOffset>(dstValTile);
    TASSIGN<dstIdxOffset>(dstIdxTile);
    TASSIGN<srcOffset>(srcTile);
    TASSIGN<tmpOffset>(tmpTile);
...
template void LaunchTRowArgMinHalf<uint32_t, 1023, 16, 1023, 32, 1023, 17>(
    uint32_t* out, aclFloat16* src, void* stream);
template void LaunchTRowArgMin<uint32_t, float, 8, 1, 8, 1, 8, 8, 8, 8>(
    float* outVal, uint32_t* outIdx, float* src, void* stream);
template void LaunchTRowArgMin<uint32_t, float, 8, 8, 8, 1, 8, 8, 8, 8>(
    float* outVal, uint32_t* outIdx, float* src, void* stream);
template void LaunchTRowArgMin<uint32_t, float, 8, 1, 8, 8, 8, 8, 8, 8>(
    float* outVal, uint32_t* outIdx, float* src, void* stream);
template void LaunchTRowArgMin<uint32_t, float, 8, 8, 8, 8, 8, 8, 8, 8>(
    float* outVal, uint32_t* outIdx, float* src, void* stream);
template void LaunchTRowArgMin<uint32_t, float, 1024, 1, 1024, 1, 1024, 8, 1024, 7>(
    float* outVal, uint32_t* outIdx, float* src, void* stream);
template void LaunchTRowArgMinHalf<uint16_t, 16, 1, 16, 1, 8, 16, 8, 16>(
    aclFloat16* outVal, uint16_t* outIdx, aclFloat16* src, void* stream);
template void LaunchTRowArgMinHalf<uint16_t, 8, 16, 16, 1, 8, 16, 8, 16>(
    aclFloat16* outVal, uint16_t* outIdx, aclFloat16* src, void* stream);
template void LaunchTRowArgMinHalf<uint16_t, 16, 1, 8, 16, 8, 16, 8, 16>(
    aclFloat16* outVal, uint16_t* outIdx, aclFloat16* src, void* stream);
template void LaunchTRowArgMinHalf<uint16_t, 8, 16, 8, 16, 8, 16, 8, 16>(
    aclFloat16* outVal, uint16_t* outIdx, aclFloat16* src, void* stream);
template void LaunchTRowArgMinHalf<uint16_t, 1024, 1, 1024, 1, 1024, 16, 1024, 13>(
    aclFloat16* outVal, uint16_t* outIdx, aclFloat16* src, void* stream);
