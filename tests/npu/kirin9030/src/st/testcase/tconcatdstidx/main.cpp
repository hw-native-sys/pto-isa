...
void test_tconcat()
{
    ...
    size_t fileSizeDstIdx = dstTileH * dstTileW * sizeof(idxType);
    ...
    idxType *src0IdxHost, *src1IdxHost, *dstIdxHost;
    ...
    aclrtMemset(dstIdxHost, fileSizeDstIdx, 0, fileSizeDstIdx);
    ...
    WriteFile(GetGoldenDir() + "/outputIdx.bin", dstIdxHost, fileSizeDstIdx);
    ...
    ReadFile(GetGoldenDir() + "/outputIdx.bin", fileSizeDstIdx, devFinalIdx.data(), fileSizeDstIdx);

    bool ret = ResultCmp<dataType>(golden, devFinal, 0.001f);
    ASSERT_TRUE(ret);
    ret = ResultCmp<idxType>(goldenIdx, devFinalIdx, 0.001f);
    ASSERT_TRUE(ret);
}
