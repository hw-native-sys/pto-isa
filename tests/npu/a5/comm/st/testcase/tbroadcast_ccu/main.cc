...
// CCU-based TBroadcast end-to-end ST (GTest).
//
...
//
// Run:  mpirun -n 2 ./tbroadcast_ccu_st
//       mpirun -n 4 ./tbroadcast_ccu_st   (enables 4-rank tests)
...
bool SetupChannelsForCcu(HcclComm comm, int rankId, int nRanks, std::vector<ChannelHandle>& channels)
{
    ...
    for (int peer = 0; peer < nRanks; ++peer) {
        ...
        if (!found) {
            std::fprintf(stderr, "[TBROADCAST_CCU] rank=%d no UBC_CTP link to peer=%d\n", rankId, peer);
            return false;
        }
    }
    ...
    std::fprintf(stderr, "[TBROADCAST_CCU] rank=%d acquired %zu CCU channel(s)\n", rankId, channels.size());
    return true;
}
...
static bool DiscoverGateDescriptor(pto::comm::ccu::CcuGateDescriptor& gateDesc)
{
    ...
    std::fprintf(stderr, "[TBROADCAST_CCU] rank=%d TryGet FAILED after retries\n", g_env.rankId);
    return false;
}
...
static bool ResolveCkeMmio(const pto::comm::ccu::CcuGateDescriptor& gateDesc)
{
    ...
    if (qrc != 0 || addr == 0) {
        std::fprintf(stderr, "[TBROADCAST_CCU] rank=%d rtGetDevResAddress FAIL rc=%d\n", g_env.rankId, qrc);
        return false;
    }
    g_env.mmioAddr = addr;
...
static bool PrepareBroadcastBuffers(int rankId, uint32_t rootId, size_t numElements, size_t payloadSize)
{
    std::vector<float> inputHost(numElements);
    if (static_cast<uint32_t>(rankId) == rootId) {
        for (size_t i = 0; i < numElements; ++i)
            inputHost[i] = static_cast<float>(i + 1);
    } else {
        std::fill(inputHost.begin(), inputHost.end(), 0.0f);
    }
    std::vector<float> outInit(numElements, -1.0f);
    ACL_OK(aclrtMemcpy(g_env.inputDev, kMaxPayload, inputHost.data(), payloadSize, ACL_MEMCPY_HOST_TO_DEVICE));
    ACL_OK(aclrtMemcpy(g_env.outputDev, kMaxPayload, outInit.data(), payloadSize, ACL_MEMCPY_HOST_TO_DEVICE));
    return true;
}
...
static bool TriggerAndSyncBroadcastCcu(int rankId)
{
    if (!ResolveGateOnce())
        return false;
    std::fprintf(
    ...
    std::fprintf(stderr, "[TBROADCAST_CCU] rank=%d triggering CKE via TBROADCAST<CCU>...\n", rankId);
    int rc = tbroadcast_ccu_trigger_launch(g_env.aivStream, g_env.mmioAddr, g_env.gateMask);
    if (rc != 0)
        return false;
    std::fprintf(stderr, "[TBROADCAST_CCU] rank=%d trigger returned, syncing aivStream...\n", rankId);

    ACL_OK(aclrtSynchronizeStream(g_env.aivStream));
    std::fprintf(stderr, "[TBROADCAST_CCU] rank=%d aivStream synced, syncing ccuStream...\n", rankId);
    ACL_OK(aclrtSynchronizeStream(g_env.stream));
    std::fprintf(stderr, "[TBROADCAST_CCU] rank=%d ccuStream synced\n", rankId);
    return true;
}

static bool VerifyBroadcastResult(int rankId, size_t numElements, size_t payloadSize)
{
    ...
    std::fprintf(stderr, "[TBROADCAST_CCU] rank=%d %s: %zu elements\n", rankId, pass ? "PASS" : "FAIL", numElements);
    return pass;
}

bool RunBroadcastCcu(size_t numElements, uint32_t rootId)
{
    CommMpiBarrier();
    if (!EnsureEnvReady())
        return false;
    ...
    if (!PrepareBroadcastBuffers(rankId, rootId, numElements, payloadSize))
        return false;

    uint64_t seq = g_seqNo++;
    if (!RegisterAndLaunchBroadcastCcu(rankId, nRanks, rootId, payloadSize, seq))
        return false;
    if (!TriggerAndSyncBroadcastCcu(rankId))
        return false;

    bool pass = VerifyBroadcastResult(rankId, numElements, payloadSize);
    CommMpiBarrier();
    return pass;
}
...
void ResetEnv()
{
    ...
    g_env = CcuEnv{};
    g_seqNo = 0;
    std::fprintf(stderr, "[TBROADCAST_CCU] === env reset for next test ===\n");
}
...
TEST_F(TBroadcastCcuTest, Float_1024_Root0_4Ranks)
{
    SKIP_IF_RANKS_LT(4);
    ASSERT_TRUE(RunBroadcastCcu(1024, 0));
}
TEST_F(TBroadcastCcuTest, Float_1024_Root1)
{
    SKIP_IF_RANKS_LT(2);
