...
// CCU-based TGather end-to-end ST (GTest).
//
...
//
// Run:  mpirun -n 2 ./tgather_ccu_st
//       mpirun -n 4 ./tgather_ccu_st   (enables 4-rank tests)
...
#include "pto/npu/comm/async/ccu/ccu_gather_kernel.hpp"
...
#define ACL_OK(expr)                                                                                                \
    do {                                                                                                            \
        aclError _r = (expr);                                                                                       \
        if (_r != ACL_SUCCESS) {                                                                                    \
            std::fprintf(stderr, "[TGATHER_CCU] ACL FAIL %s = %d (%s:%d)\n", #expr, static_cast<int>(_r), __FILE__, \
                         __LINE__);                                                                                 \
            return false;                                                                                           \
        }                                                                                                           \
    } while (0)
...
#define HCCL_OK(expr)                                                                                                \
    do {                                                                                                             \
        HcclResult _r = (expr);                                                                                      \
        if (_r != HCCL_SUCCESS) {                                                                                    \
            std::fprintf(stderr, "[TGATHER_CCU] HCCL FAIL %s = %d (%s:%d)\n", #expr, static_cast<int>(_r), __FILE__, \
                         __LINE__);                                                                                  \
            return false;                                                                                            \
        }                                                                                                            \
    } while (0)
...
bool SetupChannelsForCcu(HcclComm comm, int rankId, int nRanks, std::vector<ChannelHandle> &channels)
{
    ...
    for (int peer = 0; peer < nRanks; ++peer) {
        ...
        if (!found) {
            std::fprintf(stderr, "[TGATHER_CCU] rank=%d no UBC_CTP link to peer=%d\n", rankId, peer);
            return false;
        }
    }
    ...
    std::fprintf(stderr, "[TGATHER_CCU] rank=%d acquired %zu CCU channel(s)\n", rankId, channels.size());
    return true;
}
...
bool EnsureEnvReady()
{
    ...
    g_env.outputBufSize = static_cast<size_t>(g_env.nRanks) * kMaxPayload;
    ...
    const uint64_t spanEnd =
        (g_env.inputVa < g_env.outputVa) ? (g_env.outputVa + g_env.outputBufSize) : (g_env.inputVa + kMaxPayload);
...
static bool DiscoverGateDescriptor(pto::comm::ccu::CcuGateDescriptor &gateDesc)
{
    ...
    std::fprintf(stderr, "[TGATHER_CCU] rank=%d TryGet FAILED after retries\n", g_env.rankId);
    return false;
}
...
static bool ResolveCkeMmio(const pto::comm::ccu::CcuGateDescriptor &gateDesc)
{
    ...
    if (qrc != 0 || addr == 0) {
        std::fprintf(stderr, "[TGATHER_CCU] rank=%d rtGetDevResAddress FAIL rc=%d\n", g_env.rankId, qrc);
        return false;
    }
    g_env.mmioAddr = addr;
...
static bool PrepareGatherBuffers(int rankId, int nRanks, uint32_t rootId, size_t numElements, size_t payloadSize,
                                 uint64_t &outSliceVA)
{
    std::vector<float> inputHost(numElements);
    for (size_t i = 0; i < numElements; ++i)
        inputHost[i] = static_cast<float>(i + rankId * 10000);
    ACL_OK(aclrtMemcpy(g_env.inputDev, kMaxPayload, inputHost.data(), payloadSize, ACL_MEMCPY_HOST_TO_DEVICE));

    const size_t outFloats = static_cast<size_t>(nRanks) * numElements;
    std::vector<float> outInit(outFloats, -1.0f);
    ACL_OK(aclrtMemcpy(g_env.outputDev, g_env.outputBufSize, outInit.data(), outFloats * sizeof(float),
                       ACL_MEMCPY_HOST_TO_DEVICE));

    std::vector<uint64_t> outputSliceVAs(static_cast<size_t>(nRanks), 0ULL);
    if (static_cast<uint32_t>(rankId) == rootId) {
        for (int r = 0; r < nRanks; ++r)
            outputSliceVAs[r] = g_env.outputVa + static_cast<uint64_t>(r) * payloadSize;
    }
    CommMpiBcast(outputSliceVAs.data(), static_cast<int>(nRanks * sizeof(uint64_t)), COMM_MPI_CHAR,
                 static_cast<int>(rootId));
    outSliceVA = outputSliceVAs[rankId];
    return true;
}

static bool RegisterAndLaunchGatherCcu(int rankId, int nRanks, uint32_t rootId, size_t payloadSize, uint64_t seq,
                                       uint64_t myOutputSliceVA)
{
    pto::comm::ccu::CcuGatherKernelArg karg(static_cast<uint32_t>(rankId), static_cast<uint32_t>(nRanks), rootId,
                                            payloadSize + seq);
    karg.channels = g_env.channels;

    hcomm::KernelCreator creator = pto::comm::ccu::MakeCcuGatherCreator();
    ...
    pto::comm::ccu::CcuGatherTaskArg targ{g_env.inputVa, myOutputSliceVA, payloadSize, g_env.token};

    // Exchange (inputVa, outputSliceVa, token) across all ranks at launch time
...
static bool TriggerAndSyncGatherCcu(int rankId)
{
    if (!ResolveGateOnce())
        return false;
    std::fprintf(stderr, "[TGATHER_CCU] rank=%d gate resolved mmio=0x%llx mask=0x%x\n", rankId,
                 (unsigned long long)g_env.mmioAddr, g_env.gateMask);

    usleep(200000);

    std::fprintf(stderr, "[TGATHER_CCU] rank=%d triggering CKE via TGATHER<CCU>...\n", rankId);
    int rc = tgather_ccu_trigger_launch(g_env.aivStream, g_env.mmioAddr, g_env.gateMask);
    if (rc != 0)
        return false;
    std::fprintf(stderr, "[TGATHER_CCU] rank=%d trigger returned, syncing aivStream...\n", rankId);

    ACL_OK(aclrtSynchronizeStream(g_env.aivStream));
    std::fprintf(stderr, "[TGATHER_CCU] rank=%d aivStream synced, syncing ccuStream...\n", rankId);
    ACL_OK(aclrtSynchronizeStream(g_env.stream));
    std::fprintf(stderr, "[TGATHER_CCU] rank=%d ccuStream synced\n", rankId);
    return true;
}

static bool VerifyGatherResult(int rankId, int nRanks, uint32_t rootId, size_t numElements)
{
    if (static_cast<uint32_t>(rankId) != rootId)
        return true;
    const size_t outFloats = static_cast<size_t>(nRanks) * numElements;
    std::vector<float> outputHost(outFloats);
    ACL_OK(aclrtMemcpy(outputHost.data(), outFloats * sizeof(float), g_env.outputDev, outFloats * sizeof(float),
                       ACL_MEMCPY_DEVICE_TO_HOST));
    int mismatch = 0;
    for (int r = 0; r < nRanks; ++r) {
        for (size_t i = 0; i < numElements; ++i) {
            const float expected = static_cast<float>(i + r * 10000);
            const size_t idx = static_cast<size_t>(r) * numElements + i;
            if (std::fabs(outputHost[idx] - expected) <= 1e-3f)
                continue;
            if (mismatch < 8)
                std::fprintf(stderr, "[TGATHER_CCU] mismatch [r=%d i=%zu]: got=%f expected=%f\n", r, i, outputHost[idx],
                             expected);
            ++mismatch;
        }
    }
    bool pass = (mismatch == 0);
    std::fprintf(stderr, "[TGATHER_CCU] root %s: %zu elements x %d ranks\n", pass ? "PASS" : "FAIL", numElements,
                 nRanks);
    return pass;
}

bool RunGatherCcu(size_t numElements, uint32_t rootId)
{
    CommMpiBarrier();
    if (!EnsureEnvReady())
        return false;
    ...
    uint64_t myOutputSliceVA = 0;
    if (!PrepareGatherBuffers(rankId, nRanks, rootId, numElements, payloadSize, myOutputSliceVA))
        return false;

    uint64_t seq = g_seqNo++;
    if (!RegisterAndLaunchGatherCcu(rankId, nRanks, rootId, payloadSize, seq, myOutputSliceVA))
        return false;
    if (!TriggerAndSyncGatherCcu(rankId))
        return false;

    bool pass = VerifyGatherResult(rankId, nRanks, rootId, numElements);
    CommMpiBarrier();
    return pass;
}
...
void ResetEnv()
{
    ...
    g_env = CcuEnv{};
    g_seqNo = 0;
    std::fprintf(stderr, "[TGATHER_CCU] === env reset for next test ===\n");
}

class TGatherCcuTest : public ::testing::Test {
protected:
...
TEST_F(TGatherCcuTest, Float_1024_2Ranks)
{
    SKIP_IF_RANKS_LT(2);
    ASSERT_TRUE(RunGatherCcu(1024, 0));
}
TEST_F(TGatherCcuTest, Float_1024_4Ranks)
{
    SKIP_IF_RANKS_LT(4);
    ASSERT_TRUE(RunGatherCcu(1024, 0));
}
TEST_F(TGatherCcuTest, Root1_Float_1024)
{
    SKIP_IF_RANKS_LT(2);
