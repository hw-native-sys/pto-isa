...
template <
    typename T, uint32_t DstBurstNum, uint16_t DstBurstLen, uint16_t SrcBurstLen, typename MatTile, typename SrcVecTile,
    uint32_t IndexRow, uint32_t IndexCol>
AICORE void LoadAndInsertFp4(
    __cbuf__ T* matAddr, __ubuf__ T* dstUbAddr, __ubuf__ T* srcUbAddr, __gm__ T* src, __gm__ T* srcGmAddr,
    MatTile& matTile, SrcVecTile& srcTile, uint8_t& syncId, uint8_t eventIdNum)
{
#if defined(__DAV_VEC__)
...
#if defined(__DAV_CUBE__)
    ReadbackCbufToUbuf(
        (__ubuf__ void*)dstUbAddr, (__cbuf__ void*)matAddr, DstBurstNum, DstBurstLen, 0, syncId, eventIdNum);
#endif
}

template <
    typename T, uint32_t SrcRows, uint32_t SrcCols, uint32_t ValidRows, uint32_t IndexRow, uint32_t IndexCol,
...
void launchTInsertCompactNullTLoad(uint64_t* out, uint64_t* src, void* stream)
{
    if constexpr (testKey == 1) {
        ...
        launchTInsertCompactNullTLoadKernel<bfloat16_t, 128, 128, 80, 48, 128, 128, 0, 16>
            <<<1, nullptr, stream>>>(out, src);
    }
}
...
void launchTInsertCompactNormalTMov(uint64_t* out, uint64_t* src, void* stream)
{
    if constexpr (testKey == 1) {
        ...
        launchTInsertCompactTMovKernel<bfloat16_t, 128, 128, 64, 48, 128, 128, 0, 16, CompactMode::Normal>
            <<<1, nullptr, stream>>>(out, src);
    }
}
...
void launchTInsertCompactRowPlusOneTMov(uint64_t* out, uint64_t* src, void* stream)
{
    if constexpr (testKey == 1) {
        ...
        launchTInsertCompactTMovKernel<bfloat16_t, 65, 128, 64, 48, 128, 128, 0, 16, CompactMode::RowPlusOne>
            <<<1, nullptr, stream>>>(out, src);
    }
}
