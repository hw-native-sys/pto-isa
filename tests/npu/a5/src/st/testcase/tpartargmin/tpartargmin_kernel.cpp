...
__global__ AICORE void runTPartArgMin(__gm__ T __out__ *out, __gm__ T __in__ *src0, __gm__ T __in__ *src1,
                                      __gm__ U __out__ *outIdx, __gm__ U __in__ *src0Idx, __gm__ U __in__ *src1Idx)
{
    ...
    TPARTARGMIN<TileDataDst, TileDataSrc0, TileDataSrc1, TileDataDstIdx, TileDataSrc0Idx, TileDataSrc1Idx>(
...
void LaunchTPartArgMin(T *out, T *src0, T *src1, U *outIdx, U *src0Idx, U *src1Idx, void *stream)
{
    if constexpr (std::is_same_v<T, aclFloat16> && isHalf == true) {
        runTPartArgMin<half, U, dstVR, dstVC, src0VR, src0VC, src1VR, src1VC, dstTR, dstTC, src0TR, src0TC, src1TR,
                       src1TC>
            <<<1, nullptr, stream>>>((half *)out, (half *)src0, (half *)src1, outIdx, src0Idx, src1Idx);
    } else {
        runTPartArgMin<T, U, dstVR, dstVC, src0VR, src0VC, src1VR, src1VC, dstTR, dstTC, src0TR, src0TC, src1TR, src1TC>
            <<<1, nullptr, stream>>>(out, src0, src1, outIdx, src0Idx, src1Idx);
    }
}

template void LaunchTPartArgMin<float, uint32_t, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64>(
    float *out, float *src0, float *src1, uint32_t *outIdx, uint32_t *src0Idx, uint32_t *src1Idx, void *stream);
template void LaunchTPartArgMin<float, int32_t, 2, 24, 2, 24, 2, 8, 4, 32, 3, 24, 2, 16>(
    float *out, float *src0, float *src1, int32_t *outIdx, int32_t *src0Idx, int32_t *src1Idx, void *stream);
template void LaunchTPartArgMin<float, int32_t, 12, 63, 12, 63, 6, 60, 12, 64, 12, 64, 6, 64>(
    float *out, float *src0, float *src1, int32_t *outIdx, int32_t *src0Idx, int32_t *src1Idx, void *stream);
template void LaunchTPartArgMin<aclFloat16, int16_t, 10, 31, 8, 16, 10, 31, 10, 32, 8, 32, 12, 32, true>(
...
template void LaunchTPartArgMin<aclFloat16, uint16_t, 5, 33, 5, 33, 5, 30, 8, 48, 5, 48, 6, 48, true>(
    ...
    void *stream);
template void LaunchTPartArgMin<float, uint32_t, 8, 7, 8, 7, 8, 0, 8, 8, 8, 8, 1, 8>(float *out, float *src0,
                                                                                     float *src1, uint32_t *outIdx,
                                                                                     uint32_t *src0Idx,
                                                                                     uint32_t *src1Idx, void *stream);
template void LaunchTPartArgMin<float, uint32_t, 8, 7, 8, 7, 0, 7, 8, 8, 8, 8, 1, 8>(float *out, float *src0,
                                                                                     float *src1, uint32_t *outIdx,
                                                                                     uint32_t *src0Idx,
                                                                                     uint32_t *src1Idx, void *stream);
template void LaunchTPartArgMin<float, uint32_t, 8, 7, 8, 0, 8, 7, 8, 8, 1, 8, 8, 8>(float *out, float *src0,
                                                                                     float *src1, uint32_t *outIdx,
                                                                                     uint32_t *src0Idx,
                                                                                     uint32_t *src1Idx, void *stream);
template void LaunchTPartArgMin<float, uint32_t, 8, 7, 0, 7, 8, 7, 8, 8, 1, 8, 8, 8>(float *out, float *src0,
                                                                                     float *src1, uint32_t *outIdx,
                                                                                     uint32_t *src0Idx,
                                                                                     uint32_t *src1Idx, void *stream);
