...
AICORE inline void MovL1ToUbuf(DstTileData& dstTile, SrcTileData& srcTile)
{
    ...
    tf_copy_cbuf_to_ubuf<DstTileData, SrcTileData>(
        dstTile.data(), srcTile.data(), 1, blockCount, blockLen, 0,
...
AICORE inline void runTLOAD_MIX_DN2NZ(__gm__ T* out, __gm__ T* src0, __gm__ T* src1)
{
    ...
    using GlobalDataSrc0 = GlobalTensor<
        T, pto::Shape<N1, N2, N3, M, K>, pto::Stride<WN2 * WN3 * WN4 * WN5, WN3 * WN4 * WN5, WN4 * WN5, 1, WN4>,
        Layout::DN>;
    using GlobalDataOut = GlobalTensor<
        T, pto::Shape<1, 1, 1, baseM, baseK>,
        pto::Stride<1 * baseM * baseK, 1 * baseM * baseK, baseM * baseK, baseK, 1>, Layout::ND>;
...
template <
    typename T, int N1, int N2, int N3, int M, int K, int WN1, int WN2, int WN3, int WN4, int WN5, int baseM, int baseK,
    int ValidCols = N3 * K, bool DynamicShape = false>
AICORE inline void runTLOAD_MIX_DN2ZN(__gm__ T* out, __gm__ T* src0, __gm__ T* src1)
{
    using SrcShape = pto::Shape<N1, N2, DynamicShape ? -1 : N3, M, DynamicShape ? -1 : K>;
    using GlobalDataSrc0 =
        GlobalTensor<T, SrcShape, pto::Stride<WN2 * WN3 * WN4 * WN5, WN3 * WN4 * WN5, WN4 * WN5, 1, WN4>, Layout::DN>;
    using GlobalDataOut = GlobalTensor<
        T, pto::Shape<1, 1, 1, baseM, baseK>,
        pto::Stride<1 * baseM * baseK, 1 * baseM * baseK, baseM * baseK, baseK, 1>, Layout::ND>;
    ...
    using TileMatAData = Tile<
        TileType::Mat, T, baseM, baseK, BLayout::RowMajor, M, DynamicShape ? -1 : ValidCols, SLayout::ColMajor,
        512>; // 大Z小N
    using TileUBData = Tile<TileType::Vec, T, baseM, baseK, BLayout::RowMajor, -1, -1>;
    TileUBData srcTile(baseM, baseK);
    TASSIGN(srcTile, 0x0);
    ...
    TileMatAData aMatTile;
    if constexpr (DynamicShape) {
        aMatTile.SetValidCol(ValidCols);
    }
    TASSIGN(aMatTile, 0x0);

    // A nonzero sentinel exposes writes outside the active merged prefix and its final C0 padding.
#if defined(__DAV_CUBE__)
    ...
    constexpr int fillValue = (N3 != 1 || DynamicShape) ? 0x33333333 : 0;
    tf_create_cbuf_matrix<TileMatAData>(aMatTile.data(), repeatBit, fillValue);
#endif
    /*************************************TLOAD****************************************/
    TLOAD<TileMatAData, GlobalDataSrc0>(aMatTile, src0Global);
...
// [N,H,W,C]->[N,C1,H,W,C0]
template <
    typename T, int dstShape0, int dstC1, int dstShape2, int dstShape3, int dstC0, int gWholeShape0, int gWholeShape1,
    int gWholeShape2, int gWholeShape3, int gWholeShape4>
AICORE inline void runTLOAD_MIX_NHWC(__gm__ T __out__* out, __gm__ T __in__* src)
{
    ...
    using TileData = ConvTile<
        TileType::Mat, T, bufferSize, Layout::NC1HWC0,
...
template <
    typename T, int dstC1HW, int dstN16, int dstShape2, int dstShape3, int srcN, int srcC, int srcH, int srcW, int N,
    int C, int H, int W>
AICORE inline void runTLOAD_MIX_NCHW2FZ4D(__gm__ T __out__* out, __gm__ T __in__* src)
{
    ...
    using TileData = ConvTile<
        TileType::Mat, T, bufferSize, Layout::FRACTAL_Z, pto::ConvTileShape<dstC1HW, dstN16, dstShape2, dstShape3>>;
    TileData srcTile;
...
template <
    typename T, int dstShape0, int dstShape1, int dstC1, int dstShape3, int dstShape4, int gWholeShape0,
    int gWholeShape1, int gWholeShape2, int gWholeShape3, int gWholeShape4>
AICORE inline void runTLOAD_MIX_NCDHW(__gm__ T __out__* out, __gm__ T __in__* src)
{
    ...
    using TileData = ConvTile<
        TileType::Mat, T, bufferSize, Layout::NDC1HWC0,
...
template <
    typename T, int srcN, int srcC, int srcD, int srcH, int srcW, int N, int C, int D, int H, int W, int dstC1DHW,
    int dstN16>
...
__global__ AICORE void TLOAD_MIX_KERNEL(__gm__ uint8_t* out, __gm__ uint8_t* src0, __gm__ uint8_t* src1)
{
    if constexpr (format == 0) { // ND2NZ
        runTLOAD_MIX_ND2NZ<T, N1, N2, N3, N4, N5, WN1, WN2, WN3, WN4, WN5, BASEM, BASEK, ValidExtent, DynamicShape>(
        ...
        runTLOAD_MIX_DN2ZN<T, N1, N2, N3, N4, N5, WN1, WN2, WN3, WN4, WN5, BASEM, BASEK, ValidExtent, DynamicShape>(
            reinterpret_cast<__gm__ T*>(out), reinterpret_cast<__gm__ T*>(src0), reinterpret_cast<__gm__ T*>(src1));
        ...
        runTLOAD_MIX_NHWC<T, N1, N2, N3, N4, N5, WN1, WN2, WN3, WN4, WN5>(
            reinterpret_cast<__gm__ T*>(out), reinterpret_cast<__gm__ T*>(src0));
    } else if constexpr (format == 10) {
        runTLOAD_MIX_NCHW<T, N1, N2, N3, N4, N5, WN1, WN2, WN3, WN4, WN5>(
            reinterpret_cast<__gm__ T*>(out), reinterpret_cast<__gm__ T*>(src0));
        ...
        runTLOAD_MIX_NCDHW<T, N1, N2, N3, N4, N5, WN1, WN2, WN3, WN4, WN5>(
            reinterpret_cast<__gm__ T*>(out), reinterpret_cast<__gm__ T*>(src0));
...
{
    TLOAD_MIX_KERNEL<T, format, N1, N2, N3, N4, N5, WN1, WN2, WN3, WN4, WN5, BASEM, BASEK, ValidExtent, DynamicShape>
...
// 9 : NHWC2NC1HWC0
template void launchTLOADMIX<int8_t, 9, 1, 3, 11, 109, 32, 1, 3, 1023, 1000, 111, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<int8_t, 9, 3, 2, 121, 9, 32, 1, 3, 128, 127, 65, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<uint16_t, 9, 1, 6, 10, 100, 16, 1, 1, 100, 100, 96, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<uint16_t, 9, 10, 16, 16, 2, 16, 1, 256, 100, 16, 255, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<float, 9, 1, 1, 56, 112, 8, 1, 2, 224, 224, 25, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<float, 9, 2, 1, 56, 43, 8, 1, 3, 333, 188, 19, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

// 10 : NCHW2NC1HWC0
template void launchTLOADMIX<int8_t, 10, 1, 3, 11, 109, 32, 1, 3, 111, 1023, 109, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<int8_t, 10, 3, 2, 121, 9, 32, 1, 3, 65, 128, 127, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<uint16_t, 10, 1, 6, 10, 100, 16, 1, 1, 96, 100, 100, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<uint16_t, 10, 10, 16, 16, 2, 16, 1, 256, 255, 100, 16, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<float, 10, 1, 1, 56, 112, 8, 1, 2, 25, 224, 112, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<float, 10, 2, 1, 56, 43, 8, 1, 3, 19, 333, 188, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

// 11 : NCHW2FZ4D
template void launchTLOADMIX<int8_t, 11, 75, 3, 16, 32, 48, 95, 5, 5, 50, 111, 5, 5>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<int8_t, 11, 98, 4, 16, 32, 64, 58, 7, 7, 121, 127, 7, 7>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<uint16_t, 11, 63, 6, 16, 16, 96, 111, 3, 3, 220, 112, 3, 3>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<uint16_t, 11, 75, 4, 16, 16, 64, 48, 5, 5, 100, 50, 5, 5>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<float, 11, 50, 3, 16, 8, 48, 14, 5, 5, 224, 224, 5, 5>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<float, 11, 27, 2, 16, 8, 32, 24, 3, 3, 333, 188, 3, 3>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
// 12 : NCDHW2NDC1HWC0
template void launchTLOADMIX<int8_t, 12, 1, 2, 3, 11, 109, 3, 111, 2, 1023, 109, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<int8_t, 12, 3, 3, 2, 15, 9, 3, 65, 4, 30, 50, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<uint16_t, 12, 1, 4, 6, 10, 10, 1, 96, 6, 100, 10, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<uint16_t, 12, 10, 2, 8, 16, 2, 256, 128, 2, 100, 7, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<float, 12, 1, 5, 1, 25, 31, 2, 25, 7, 112, 31, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<float, 12, 2, 2, 1, 43, 43, 3, 19, 2, 155, 180, 1, 1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

// 13 : NCDHW2FZ3D
template void launchTLOADMIX<int8_t, 13, 48, 95, 2, 5, 5, 50, 111, 4, 5, 5, 150, 3>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<int8_t, 13, 32, 58, 2, 7, 7, 63, 127, 2, 7, 7, 196, 2>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<uint16_t, 13, 48, 111, 2, 3, 3, 110, 112, 2, 3, 3, 126, 3>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<uint16_t, 13, 32, 48, 3, 3, 3, 70, 50, 4, 3, 3, 81, 2>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<float, 13, 48, 14, 5, 2, 2, 224, 224, 7, 2, 2, 40, 3>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
template void launchTLOADMIX<float, 13, 32, 24, 2, 3, 3, 333, 188, 2, 3, 3, 54, 2>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

template <
    typename T, int format, int dtype, int N1, int N2, int N3, int N4, int N5, int WN1, int WN2, int WN3, int WN4,
...
template void launchTLOADMIX<uint16_t, 5, 1, 1, 16, 35, 3, 1, 1, 16, 64, 9, 64, 64, 48, false>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

template void launchTLOADMIX<uint16_t, 5, 1, 1, 8, 35, 3, 1, 1, 8, 64, 9, 64, 32, 16, true>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

template void launchTLOADMIX<int8_t, 5, 1, 1, 8, 35, 3, 1, 1, 8, 64, 9, 96, 32, 17, false>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

template void launchTLOADMIX<float, 5, 1, 1, 8, 35, 3, 1, 1, 8, 64, 9, 64, 32, 17, false>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

template void launchTLOADMIX<uint16_t, 5, 1, 1, 8, 35, 3, 1, 1, 8, 64, 9, 64, 32, 2, true>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

template void launchTLOADMIX<uint16_t, 5, 1, 1, 8, 35, 3, 1, 1, 8, 64, 9, 64, 32, 6, false>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

template void launchTLOADMIX<uint16_t, 5, 1, 1, 1, 35, 3, 1, 1, 1, 64, 9, 64, 32, 2, true>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
