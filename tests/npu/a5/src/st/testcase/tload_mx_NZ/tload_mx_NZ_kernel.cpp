...
__tf__ PTO_INTERNAL void tf_copy_cbuf_to_ubuf(
    ...
    int block_count, int block_len, int src_stride, int dst_stride, int i, uint32_t tileMatStride,
    uint32_t tileUbStride)
{
    copy_cbuf_to_ubuf(
        (__ubuf__ void*)(__cce_get_tile_ptr(dst) + i * tileUbStride),
        (__cbuf__ void*)(__cce_get_tile_ptr(src) + i * tileMatStride), vec_core, block_count, block_len, src_stride,
        dst_stride);
}

template <
    typename T, int N0, int N1, int N2, int N3, int N4, int WN0, int WN1, int WN2, int WN3, int WN4, int baseRow,
...
AICORE inline void runTLOAD_SCALE(__gm__ T* out, __gm__ T* src0, __gm__ T* src1)
{
    ...
    using GlobalDataSrc0 = std::conditional_t<
        isScaleA,
        GlobalTensor<
            T, pto::Shape<N0, N1, N2, N3, N4>, pto::Stride<WN1 * WN2 * WN3 * WN4, WN2 * WN3 * WN4, WN3 * WN4, WN4, 1>,
            Layout::MX_A_ZZ>,
        GlobalTensor<
            T, pto::Shape<N0, N1, N2, N3, N4>, pto::Stride<WN1 * WN2 * WN3 * WN4, WN2 * WN3 * WN4, WN3 * WN4, WN4, 1>,
            Layout::MX_B_NN>>;
    using GlobalDataOut = GlobalTensor<
        T, pto::Shape<1, 1, 1, 1, validSize>, pto::Stride<1 * validSize, 1 * validSize, validSize, validSize, 1>,
        Layout::ND>;
    ...
    for (int i = 0; i < N0; i++) {
        tf_copy_cbuf_to_ubuf<TileUBData, TileMatAData>(
            srcTile.data(), aMatTile.data(), 0, blockCount, l12ubBlockLen, srcStride, 0, i, tileMatStride,
            tileUbStride); // move to vector0
        tf_copy_cbuf_to_ubuf<TileUBData, TileMatAData>(
            srcTile.data(), aMatTile.data(), 1, blockCount, l12ubBlockLen, srcStride, 0, i, tileMatStride,
            tileUbStride); // move to vector1
    }
...
template <
    typename T, int format, int N0, int N1, int N2, int N3, int N4, int WN0, int WN1, int WN2, int WN3, int WN4,
...
__global__ AICORE void TLOAD_SCALE_KERNEL(__gm__ uint8_t* out, __gm__ uint8_t* src0, __gm__ uint8_t* src1)
{
    if constexpr (format == 11) { // ZZ2ZZ
        runTLOAD_SCALE<
            float8_e8m0_t, N0, N1, N2, N3, N4, WN0, WN1, WN2, WN3, WN4, BASEROW, BASECOL, N0 * N1 * N3, N2 * N4, true>(
            reinterpret_cast<__gm__ float8_e8m0_t*>(out), reinterpret_cast<__gm__ float8_e8m0_t*>(src0),
            reinterpret_cast<__gm__ float8_e8m0_t*>(src1));
    } else if constexpr (format == 12) { // NN2NN
        runTLOAD_SCALE<
            float8_e8m0_t, N0, N1, N2, N3, N4, WN0, WN1, WN2, WN3, WN4, BASEROW, BASECOL, N2 * N4, N0 * N1 * N3, false>(
            reinterpret_cast<__gm__ float8_e8m0_t*>(out), reinterpret_cast<__gm__ float8_e8m0_t*>(src0),
            reinterpret_cast<__gm__ float8_e8m0_t*>(src1));
    }
}
...
template void launchTLOADSCALE<uint8_t, 11, 1, 1, 2, 16, 2, 1, 2, 3, 16, 2, 16, 4>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

template void launchTLOADSCALE<uint8_t, 12, 1, 2, 1, 16, 2, 1, 3, 2, 16, 2, 2, 32>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

// shape[0] == 1, L1Size > [validRow, validCol]
template void launchTLOADSCALE<uint8_t, 11, 1, 2, 2, 16, 2, 1, 2, 3, 16, 2, 48, 10>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

template void launchTLOADSCALE<uint8_t, 12, 1, 2, 2, 16, 2, 1, 3, 2, 16, 2, 8, 64>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

template void launchTLOADSCALE<uint8_t, 11, 1, 5, 33, 16, 2, 1, 11, 40, 16, 2, 128, 96>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

template void launchTLOADSCALE<uint8_t, 12, 1, 64, 29, 16, 2, 1, 65, 59, 16, 2, 58, 1088>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

// shape[0] > 1, L1Size = [validRow, validCol]
template void launchTLOADSCALE<uint8_t, 11, 3, 1, 2, 16, 2, 3, 2, 3, 16, 2, 48, 4>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

template void launchTLOADSCALE<uint8_t, 12, 4, 2, 1, 16, 2, 4, 3, 2, 16, 2, 2, 128>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);
// shape[0] > 1, L1Size > [validRow, validCol]
template void launchTLOADSCALE<uint8_t, 11, 4, 3, 3, 16, 2, 4, 10, 5, 16, 2, 192, 10>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);

template void launchTLOADSCALE<uint8_t, 12, 7, 5, 3, 16, 2, 7, 7, 11, 16, 2, 12, 560>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, void* stream);