...
__tf__ PTO_INTERNAL void tf_copy_cbuf_to_ubuf(
    typename DstTileData::TileDType __out__ dst, typename SrcTileData::TileDType __in__ src, uint16_t nBurst,
    uint16_t lenBurst)
{
...
__tf__ PTO_INTERNAL void tf_copy_ubuf_to_gm(
    typename GlobalData::DType __out__* dst, typename TileData::TileDType __in__ src, int startDstAddr, int gShape0,
    int gStride0, uint16_t nBurst, uint32_t lenBurst, uint64_t burstDstStride, uint32_t burstSrcStride,
    int64_t tileStride)
{
...
__global__ AICORE void RunTMOV(__gm__ OutType* out, __gm__ AType* src0, __gm__ BType* src1, __gm__ OutType* src2)
{
    ...
    using SrcTileData = std::conditional_t<
        isNZUnalign,
        Tile<
            TileType::Mat, OutType, row, col, GetTileBLayout<layoutType>(), row, col, GetTileSLayout<layoutType>(),
            sfractalSize>,
        Tile<
            TileType::Mat, OutType, staticRow, staticCol, GetTileBLayout<layoutType>(), copyOutM, copyOutN,
    ...
    using TileMatAData = std::conditional_t<
        isNZUnalign, Tile<TileType::Mat, AType, M, K, BLayout::ColMajor, M, K, SLayout::RowMajor, 512>,
        Tile<TileType::Mat, AType, M, K, BLayout::ColMajor, validM, validK, SLayout::RowMajor, 512>>;

    using TileMatBData = std::conditional_t<
        isNZUnalign, Tile<TileType::Mat, BType, K, N, BLayout::ColMajor, K, N, SLayout::RowMajor, 512>,
...
template <
    ...
    int col, bool isNZUnalign = false, bool isRelu = false, Layout layoutType = Layout::ND, int sfractalSize = 512,
    int indexRow = 0, int indexCol = 0, bool isInsert = false, int dstRow = 0, int dstCol = 0>
    ...
    __gm__ OutType* out, __gm__ AType* src0, __gm__ BType* src1, __gm__ FbType* src2, __gm__ OutType* src3)
{
    ...
    using GlobalDataSrc2 = std::conditional_t<
        isNZUnalign, GlobalTensor<FbType, pto::Shape<1, 1, 1, 1, N>, pto::Stride<1 * N, 1 * N, 1 * N, N, 1>>,
        GlobalTensor<
            FbType, pto::Shape<1, 1, 1, 1, validN>, pto::Stride<1 * validN, 1 * validN, 1 * validN, validN, 1>>>;

    using TileMatFbData = std::conditional_t<
        isNZUnalign, Tile<TileType::Mat, FbType, 1, N, BLayout::RowMajor, 1, N, SLayout::NoneBox>,
    ...
    __gm__ OutType* out, __gm__ AType* src0, __gm__ BType* src1, __gm__ OutType* src2, float scalar)
{
    ...
    using GlobalDataSrc2 = std::conditional_t<
        isNZUnalign, GlobalTensor<OutType, pto::Shape<1, 1, 1, 1, N>, pto::Stride<1 * N, 1 * N, 1 * N, N, 1>>,
        GlobalTensor<
            OutType, pto::Shape<1, 1, 1, 1, validN>, pto::Stride<1 * validN, 1 * validN, 1 * validN, validN, 1>>>;
    ...
    uint8_t* out, uint8_t* src0, uint8_t* src1, uint8_t* src2, uint8_t* src3, void* stream)
{
    if constexpr (tilingKey == 1) {
        ...
        RunTMOVFBQuant<
            int8_t, half, half, uint64_t, 128, 64, 128, 128, 128, false, false, Layout::NZ, 512, 32, 32, true, 256, 256>
            <<<1, nullptr, stream>>>(
                reinterpret_cast<int8_t*>(out), reinterpret_cast<half*>(src0), reinterpret_cast<half*>(src1),
                reinterpret_cast<uint64_t*>(src2), reinterpret_cast<int8_t*>(src3));
    }
...
template void LaunchTMOVAcc2MatFBQuantNZ2NZ<1>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, uint8_t* src2, uint8_t* src3, void* stream);
template void LaunchTMOVAcc2MatFBQuantNZ2NZ<2>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, uint8_t* src2, uint8_t* src3, void* stream);
template void LaunchTMOVAcc2MatFBQuantNZ2NZ<3>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, uint8_t* src2, uint8_t* src3, void* stream);
template void LaunchTMOVAcc2MatFBQuantNZ2NZ<4>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, uint8_t* src2, uint8_t* src3, void* stream);
template void LaunchTMOVAcc2MatFBQuantNZ2NZ<5>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, uint8_t* src2, uint8_t* src3, void* stream);
template void LaunchTMOVAcc2MatFBQuantNZ2NZ<6>(
    uint8_t* out, uint8_t* src0, uint8_t* src1, uint8_t* src2, uint8_t* src3, void* stream);
