    ...
    uint8_t* out, uint8_t* src0, uint8_t* src1, uint8_t* src2, uint8_t* src3, void* stream)
{
    ...
    if constexpr (tilingKey == 1) {
        ...
        RunTMOVFBQuant<
            bfloat16_t, int8_t, int8_t, uint64_t, 128, 64, 96, 128, 96, 0, false, false, Layout::ND, 512, 0, 0, false,
        ...
        RunTMOVFBQuant<
            int8_t, float, float, uint64_t, 60, 128, 64, 64, 64, 0, false, true, Layout::ND, 512, 7, 32, false, 80, 256>
            <<<1, nullptr, stream>>>(
                reinterpret_cast<int8_t*>(out), reinterpret_cast<float*>(src0), reinterpret_cast<float*>(src1),
                reinterpret_cast<uint64_t*>(src2), reinterpret_cast<int8_t*>(src3));
    } else if constexpr (tilingKey == 5) {
        RunTMOVFBQuant<
            half, float, float, uint64_t, 31, 128, 128, 31, 128, 0, false, true, Layout::ND, 512, 0, 64, false, 40, 256>
            <<<1, nullptr, stream>>>(
...
void LaunchTMOVAcc2VecSCQuantNZ2ND(uint8_t* out, uint8_t* src0, uint8_t* src1, uint8_t* src2, void* stream)
{
    ...
    if constexpr (tilingKey == 1) {
        ...
        RunTMOVSCQuant<
            int8_t, float, float, 60, 128, 64, 64, 64, 0, false, true, Layout::ND, 512, 0, 32, false, 128, 128>
            <<<1, nullptr, stream>>>(
                reinterpret_cast<int8_t*>(out), reinterpret_cast<float*>(src0), reinterpret_cast<float*>(src1),
