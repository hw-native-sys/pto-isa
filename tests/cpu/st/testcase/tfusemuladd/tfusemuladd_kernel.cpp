...
void LaunchTFUSEMULADD(T* out, T* src0, T* src1, void* stream)
{
    if constexpr (std::is_same_v<T, aclFloat16>) {
        ...
        runTFUSEMULADD<T, dstTileH, dstTileW, src0TileH, src0TileW, src1TileH, src1TileW, vRows, vCols>(
            out, src0, src1);
    }
}

template void LaunchTFUSEMULADD<float, 64, 64, 64, 64, 64, 64, 64, 64>(
    float* out, float* src0, float* src1, void* stream);
template void LaunchTFUSEMULADD<float, 32, 128, 32, 192, 32, 256, 32, 127>(
    float* out, float* src0, float* src1, void* stream);
template void LaunchTFUSEMULADD<aclFloat16, 64, 64, 64, 64, 64, 64, 64, 64>(
    aclFloat16* out, aclFloat16* src0, aclFloat16* src1, void* stream);
template void LaunchTFUSEMULADD<aclFloat16, 32, 128, 32, 192, 32, 256, 32, 127>(
    aclFloat16* out, aclFloat16* src0, aclFloat16* src1, void* stream);
