...
template <
    ...
    bool apply_relu>
AICORE inline void RunTStoreRowMajorQuant(
    __gm__ Dst __out__* out, __gm__ SrcT __in__* src, __gm__ uint64_t __in__* fbQuant)
{
    ...
    if constexpr (is_v_quant) {
        using GlobalDataFp = GlobalTensor<
            uint64_t, pto::Shape<1, 1, 1, 1, validCol>, pto::Stride<1 * validCol, validCol, validCol, validCol, 1>>;
    ...
    __gm__ Dst __out__* out, __gm__ SrcT __in__* src, __gm__ uint64_t __in__* fbQuant)
{
    ...
    if constexpr (is_v_quant) {
        using GlobalDataFp = GlobalTensor<
            uint64_t, pto::Shape<1, 1, 1, 1, validRow>, pto::Stride<1 * validRow, validRow, validRow, validRow, 1>>;
        GlobalDataFp fpGlobal(fbQuant);
...
template <
    typename DstT, typename SrcT, pto::Layout format, int gShape0, int gShape1, int gShape2, int gShape3, int gShape4,
...
__global__ AICORE void TStoreKernelQuant(__gm__ DstT* out, __gm__ SrcT* src, __gm__ uint64_t* fbQuant)
{
    if constexpr (format == pto::Layout::ND) {
        RunTStoreRowMajorQuant<
            DstT, SrcT, gShape0, gShape1, gShape2, gShape3, gShape4, gWholeShape0, gWholeShape1, gWholeShape2,
            gWholeShape3, gWholeShape4, is_v_quant, saturate_inf, apply_relu>(out, src, fbQuant);
    } else if constexpr (format == pto::Layout::DN) {
        RunTStoreColMajorQuant<
            DstT, SrcT, gShape0, gShape1, gShape2, gShape3, gShape4, gWholeShape0, gWholeShape1, gWholeShape2,
            gWholeShape3, gWholeShape4, is_v_quant, saturate_inf, apply_relu>(out, src, fbQuant);
...
void LaunchTStoreQuant(DstT* out, SrcT* src, uint64_t* fbQuant, void* stream)
{
    ...
    if constexpr (format == 0) {
        TStoreKernelQuant<
            DstT, SrcT, pto::Layout::ND, gShape0, gShape1, gShape2, gShape3, gShape4, gWholeShape0, gWholeShape1,
        ...
        TStoreKernelQuant<
            DstT, SrcT, pto::Layout::DN, gShape0, gShape1, gShape2, gShape3, gShape4, gWholeShape0, gWholeShape1,
            gWholeShape2, gWholeShape3, gWholeShape4, is_v_quant, saturate_inf, apply_relu>(
            static_cast<DT*>(out), static_cast<ST*>(src), fbQuant);
    } else if constexpr (format == 2) {
        TStoreKernelQuant<
            DstT, SrcT, pto::Layout::NZ, gShape0, gShape1, gShape2, gShape3, gShape4, gWholeShape0, gWholeShape1,
            gWholeShape2, gWholeShape3, gWholeShape4, is_v_quant, saturate_inf, apply_relu>(
            static_cast<DT*>(out), static_cast<ST*>(src), fbQuant);
    }
}

template void LaunchTStoreQuant<0, int8_t, float, 1, 1, 1, 2, 128, 1, 1, 1, 2, 128, true, true, true>(
    int8_t* out, float* src, uint64_t* fbQuant, void* stream);
