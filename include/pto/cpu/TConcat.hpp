...
PTO_INTERNAL void TConcatTileCheck(TileDst& dst, TileSrc0& src0, TileSrc1& src1)
{
    ...
    assert(
        dst.GetValidRow() == src0.GetValidRow() && dst.GetValidRow() == src1.GetValidRow() &&
...
PTO_INTERNAL void TConcatIdxTileCheck(
    TileDst& dst, TileSrc0& src0, TileSrc1& src1, TileSrc0Idx& src0Idx, TileSrc1Idx& src1Idx)
{
    TConcatTileCheck<TileDst, TileSrc0, TileSrc1>(dst, src0, src1);
    using TIdx = typename TileSrc0Idx::DType;

    static_assert(
        std::is_same_v<TIdx, typename TileSrc1Idx::DType>,
        "TCONCAT: Data type of src0Idx and src1Idx must be the same.");
    static_assert(
        std::is_same_v<TIdx, int32_t> || std::is_same_v<TIdx, int16_t> || std::is_same_v<TIdx, int8_t> ||
            std::is_same_v<TIdx, uint32_t> || std::is_same_v<TIdx, uint16_t> || std::is_same_v<TIdx, uint8_t>,
        "TCONCAT: Invalid data type of src0Idx.");
    static_assert(
        TileSrc0Idx::Loc == TileType::Vec && TileSrc1Idx::Loc == TileType::Vec,
    ...
    TileDst& dst, TileSrc0& src0, TileSrc1& src1, TileDstIdx& dstIdx, TileSrc0Idx& src0Idx, TileSrc1Idx& src1Idx)
{
    ...
    static_assert(
        std::is_same_v<typename TileDstIdx::DType, typename TileSrc0Idx::DType>,
...
struct TConCatCols {
    ...
    PTO_INTERNAL std::pair<unsigned, unsigned> GetColsNumber(unsigned r) { return {cols0, cols1}; }
};
