    ...
    std::array<uint32_t, kCounterWords> counter, std::array<uint32_t, kKeyCount> key)
{
    for (uint16_t i = 0; i < Rounds; ++i) {
        ...
        const uint32_t hi0 = static_cast<uint32_t>(mul0 >> BLOCK_SIZE);
        const uint32_t lo0 = static_cast<uint32_t>(mul0);
        const uint32_t hi1 = static_cast<uint32_t>(mul1 >> BLOCK_SIZE);
...
PTO_INTERNAL void TRANDOM_IMPL(DstTile& dst, TRandomKey& key, TRandomCounter& counter)
{
    ...
    std::array<uint32_t, cpu_random::kCounterWords> rowCounter = {counter[0], counter[1], counter[2], counter[3]};
    ...
    for (uint32_t row = 0; row < validRows; ++row) {
        for (uint32_t loop = 0; loop < loopCount; ++loop) {
            for (uint32_t lane = 0; lane < elementsPerRepeat; ++lane) {
                std::array<uint32_t, cpu_random::kCounterWords> laneCounter = rowCounter;
