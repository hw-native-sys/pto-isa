...
struct RowExpandAddOp {
    ...
    {
        Int64BinaryCalcRegs<Int64Op::Add, T>(dstLow, dstHigh, src0Low, src0High, src1Low, src1High, preg);
    }
#endif
};
...
__tf__ PTO_INTERNAL OP_NAME(TROWEXPANDADD) OP_TYPE(broadcast) void TRowExpandAdd(
    ...
    unsigned version = VFImplKind::VFIMPL_DEFAULT)
{
    ...
    if constexpr (std::is_same_v<T, int64_t> || std::is_same_v<T, uint64_t>) {
        Int64RowExpandBinary<RowExpandAddOp<T>, TileDataDst, TileDataSrc0, TileDataSrc1>(
            dstPtr, src0Ptr, src1Ptr, validRow, validCol);
    } else {
        RowExpandBinaryInstr<
            RowExpandAddOp<T>, TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem>(
            dstPtr, src0Ptr, src1Ptr, validRow, validCol);
    }
}
...
PTO_INTERNAL void TROWEXPANDADD_IMPL(
