...
__simt_callee__ AICORE PTO_INLINE bool last_owner_find_elem(
...
template <
    typename T, typename TIdx, typename TileSrc, ScatterAtomicOp Atomic, ScatterOOB Oob, uint32_t ValidRowsT,
    uint32_t ValidColsT, uint32_t TableRowsT>
AICORE __simt_vf__ LAUNCH_BOUND(1024) PTO_INLINE void simt_mscatter_row_kernel(
    __gm__ T* __restrict__ table, __ubuf__ const T* __restrict__ src, __ubuf__ const TIdx* __restrict__ indices,
...
template <
    typename T, typename TIdx, typename TileSrc, ScatterOOB Oob, uint32_t ValidRowsT, uint32_t ValidColsT,
    uint32_t TableRowsT>
AICORE __simt_vf__ LAUNCH_BOUND(1024) PTO_INLINE void simt_mscatter_row_last_kernel(
    __gm__ T* __restrict__ table, __ubuf__ const T* __restrict__ src, __ubuf__ const TIdx* __restrict__ indices,
    uint32_t validRowsRT, uint32_t validColsRT, uint32_t tableRowsRT)
{
    constexpr bool kStaticR = (ValidRowsT > 0u);
...
template <
    typename T, typename TIdx, typename TileSrc, typename TileIdx, ScatterAtomicOp Atomic, ScatterOOB Oob,
    uint32_t ValidRowsT, uint32_t ValidColsT, uint32_t TableSizeT>
AICORE __simt_vf__ LAUNCH_BOUND(1024) PTO_INLINE void simt_mscatter_elem_kernel(
    __gm__ T* __restrict__ table, __ubuf__ const T* __restrict__ src, __ubuf__ const TIdx* __restrict__ indices,
...
template <
    typename T, typename TIdx, typename TileSrc, typename TileIdx, ScatterOOB Oob, uint32_t ValidRowsT,
    uint32_t ValidColsT, uint32_t TableSizeT>
AICORE __simt_vf__ LAUNCH_BOUND(1024) PTO_INLINE void simt_mscatter_elem_last_kernel(
    __gm__ T* __restrict__ table, __ubuf__ const T* __restrict__ src, __ubuf__ const TIdx* __restrict__ indices,
    uint32_t validRowsRT, uint32_t validColsRT, uint32_t tableSizeRT)
{
    constexpr bool kStaticR = (ValidRowsT > 0u);
...
template <
    typename T, typename TIdx, ScatterAtomicOp Atomic, ScatterOOB Oob, ScatterConflict Conflict, uint32_t ValidRowsT,
    uint32_t ValidColsT, uint32_t TableRowsT, typename SrcTileData, typename IdxTileData>
__tf__ AICORE void MScatterRowImpl(
    __gm__ T* __restrict__ tablePtr, typename SrcTileData::TileDType __in__ src,
...
template <
    typename T, typename TIdx, ScatterAtomicOp Atomic, ScatterOOB Oob, ScatterConflict Conflict, uint32_t ValidRowsT,
    uint32_t ValidColsT, uint32_t TableSizeT, typename SrcTileData, typename IdxTileData>
    ...
    typename IdxTileData::TileDType __in__ indices, uint32_t validRows, uint32_t validCols, uint32_t tableSize)
{
    ...
    if constexpr (kIsLast) {
        ...
        cce::async_invoke<simt_mscatter_elem_kernel<
            T, TIdx, SrcTileData, IdxTileData, Atomic, Oob, ValidRowsT, ValidColsT, TableSizeT>>(
...
__tf__ AICORE void MScatterScalarImpl(
    __gm__ T* __restrict__ tablePtr, typename SrcTileData::TileDType __in__ src,
...
PTO_INTERNAL void MScatterCheck(const GlobalTable& table, const TileSrc& src, const TileIdx& indices)
{
    ...
    static_assert(
        IsValidScatterAtomic<T, Atomic>::value,
    ...
    if constexpr (Mode == Coalesce::Row) {
        ...
        if constexpr (kSrcValidR > 0 && kSrcValidC > 0) {
            static_assert(
                kSrcValidR >= 1 && kSrcValidC >= 1, "MSCATTER Coalesce::Elem requires non-empty valid source shape.");
        }
    }
}
...
template <
    ...
    ScatterOOB Oob = ScatterOOB::Undefined, ScatterConflict Conflict = ScatterConflict::Last, typename GlobalTable,
