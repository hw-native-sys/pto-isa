...
__tf__ PTO_INTERNAL OP_NAME(TORS)
    OP_TYPE(element_wise) void TOrS(typename TileDataDst::TileDType __out__ dst,
...
PTO_INTERNAL void TORS_IMPL(TileDataDst &dst, TileDataSrc &src0, typename TileDataSrc::DType src1)
{
    ...
    static_assert(std::is_same_v<T, typename TileDataSrc::DType>,
                  "Fix: TORS input tile src0, src1 and dst tile data type mismatch.");

    unsigned validRow = dst.GetValidRow();
    unsigned validCol = dst.GetValidCol();
    PTO_ASSERT(src0.GetValidRow() == validRow && src0.GetValidCol() == validCol,
               "Fix: TORS input tile src0 valid shape mismatch with output tile dst shape.");

    TOrS<T, TileDataDst, TileDataSrc>(dst.data(), src0.data(), src1, validRow, validCol);
}
} // namespace pto
#endif
