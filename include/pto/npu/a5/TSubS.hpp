    ...
    VFImplKind version = VFImplKind::VFIMPL_DEFAULT)
{
    ...
    if constexpr (std::is_same_v<T, int64_t> || std::is_same_v<T, uint64_t>) {
        Int64Scalar<Int64Op::Sub, T, TileDataDst::Cols, TileDataSrc::Cols>(
            dstPtr, src0Ptr, src1, kValidRows, kValidCols);
    } else {
        BinaryInstr<
            SubSOp<T>, TileDataDst, TileDataSrc, T, elementsPerRepeat, blockSizeElem, dstRowStride, src0RowStride>(
