...
template <typename T>
PTO_INTERNAL void TRandomInitKeyCnt(
    T& key0, T& key1, T& ctr0, T& ctr1, T& ctr2, T& ctr3, TRandomKey key, TRandomCounter counter)
{
    vbr(key0, key[0]);
    vbr(key1, key[1]);
    vbr(ctr0, counter[0]);
...
PTO_INTERNAL void TRandomStore(
    __ubuf__ T* dst, U& ctr0, U& ctr1, U& ctr2, U& ctr3, uint16_t i, uint16_t j, unsigned& sReg)
{
...
__tf__ PTO_INTERNAL void TRandom(
    typename DstTile::TileDType __out__ dstData, TRandomKey key, TRandomCounter counter, unsigned validRow,
    unsigned validCol)
{
