...
struct ROWIDXMAX {
    ...
    static PTO_INTERNAL void Reduce(RegType& dst, RegType& src, MaskReg& preg) { vcmax(dst, src, preg, MODE_ZEROING); }
...
struct ROWIDXMIN {
    ...
    using PaddingType = typename Padding<T>::Type;
    using RegType = typename TypeGet<T>::T;
    static PTO_INTERNAL void Reduce(RegType& dst, RegType& src, MaskReg& preg) { vcmin(dst, src, preg, MODE_ZEROING); }
    static PTO_INTERNAL void Compare(MaskReg& dst, RegType& src0, RegType& src1, MaskReg& preg)
    {
...
PTO_INTERNAL void UpdateIdxValue(
...
template <
    typename ReduceIdxOp, typename TileDataOutVal, typename TileDataOutIdx, typename TileDataIn, bool outputVal,
    bool postUpdate>
PTO_INTERNAL void TRowReduceIdxProc(
    __ubuf__ typename TileDataOutVal::DType* dstValPtr, __ubuf__ typename TileDataOutIdx::DType* dstIdxPtr,
    __ubuf__ typename TileDataIn::DType* srcPtr, uint32_t rows, uint32_t cols, int32_t repeatTimes, int32_t srcAdjust)
{
    ...
    vbr(vregZero, 0);
        ...
        for (uint16_t i = 0; i < (uint16_t)rows; ++i) {
            vdup(
                (RegTensor<typename ReduceIdxOp::PaddingType>&)vregValOrig, ReduceIdxOp::InitVal, pRegOneElem,
                MODE_ZEROING);
            vbr(vregIdxOrig, 0);
            vbr(vregIdxOffset, 0);
            ...
            for (uint16_t j = 0; j < (uint16_t)repeatTimes; j++) {
                ...
                UpdateIdxValue<ReduceIdxOp, TDstIdx, TSrc>(
...
PTO_INTERNAL void TRowReduceIdxImpl(
    __ubuf__ typename TileDataOutVal::DType* dstValPtr, __ubuf__ typename TileDataOutIdx::DType* dstIdxPtr,
...
__tf__ PTO_INTERNAL OP_NAME(TROWARGMAX) OP_TYPE(reduce) void TRowArgMax(
    typename TVal::TileDType __out__ dstVal, typename TIdx::TileDType __out__ dstIdx,
    typename TIn::TileDType __in__ src, uint32_t srcValidRows, uint32_t srcValidCols, uint32_t dstValValidRow,
    uint32_t dstIdxValidRow, unsigned version = VFImplKind::VFIMPL_DEFAULT)
{
    ...
    TRowReduceIdxImpl<ROWIDXMAX<typename TIn::DType>, TVal, TIdx, TIn, outputVal>(
...
{
    TRowArgMax<TileDataOut, TileDataOut, TileDataIn, false>(
        dst.data(), dst.data(), src.data(), src.GetValidRow(), src.GetValidCol(), dst.GetValidRow(), dst.GetValidRow());
}
...
PTO_INTERNAL void TROWARGMAX_IMPL(TileDataOutVal& dstVal, TileDataOutIdx& dstIdx, TileDataIn& src, TileDataTmp& tmp)
{
    TRowArgMax<TileDataOutVal, TileDataOutIdx, TileDataIn, true>(
        dstVal.data(), dstIdx.data(), src.data(), src.GetValidRow(), src.GetValidCol(), dstVal.GetValidRow(),
        dstIdx.GetValidRow());
}

template <typename TileDataOutVal, typename TileDataOutIdx, typename TileDataIn, bool outputVal>
__tf__ PTO_INTERNAL OP_NAME(TROWARGMIN) OP_TYPE(reduce) void TRowArgMin(
    typename TileDataOutVal::TileDType __out__ dstVal, typename TileDataOutIdx::TileDType __out__ dstIdx,
    typename TileDataIn::TileDType __in__ src, uint32_t srcValidRows, uint32_t srcValidCols, uint32_t dstValValidRow,
    uint32_t dstIdxValidRow, unsigned version = VFImplKind::VFIMPL_DEFAULT)
{
    using TDstVal = typename TileDataOutVal::DType;
    using TDstIdx = typename TileDataOutIdx::DType;
    using TSrc = typename TileDataIn::DType;
    TRowReduceIdxCheck<TileDataOutVal, TileDataOutIdx, TileDataIn, outputVal>(
        srcValidRows, srcValidCols, dstValValidRow, dstIdxValidRow);
...
{
    TRowArgMin<TileDataOut, TileDataOut, TileDataIn, false>(
        dst.data(), dst.data(), src.data(), src.GetValidRow(), src.GetValidCol(), dst.GetValidRow(), dst.GetValidRow());
}
