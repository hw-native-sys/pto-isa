...
__tf__ PTO_INTERNAL OP_NAME(TSUB) OP_TYPE(element_wise) void TSub(
...
PTO_INTERNAL void TSubCheck(const TileDataDst& dst, const TileDataSrc0& src0, const TileDataSrc1& src1)
{
    ...
    static_assert(
        ...
        "Fix: TSUB input tile src0, src1 and dst tile data type mismatch.");
...
{
    using T = typename TileDataDst::DType;
    TSubCheck<TileDataDst, TileDataSrc0, TileDataSrc1>(dst, src0, src1);
    constexpr unsigned blockSizeElem = BLOCK_BYTE_SIZE / sizeof(T);
    constexpr unsigned elementsPerRepeat = CCE_VL / sizeof(T);

    TSub<TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem>(
