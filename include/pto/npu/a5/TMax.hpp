...
__tf__ PTO_INTERNAL OP_NAME(TMAX) OP_TYPE(element_wise) void TMax(
    ...
    VFImplKind version = VFImplKind::VFIMPL_DEFAULT)
{
    ...
    if constexpr (std::is_same_v<T, int64_t> || std::is_same_v<T, uint64_t>) {
        Int64Binary<Int64Op::Max, T, TileDataDst::Cols, TileDataSrc0::Cols, TileDataSrc1::Cols>(
            dstPtr, src0Ptr, src1Ptr, validRows, validCols);
    } else {
        BinaryInstr<MaxOp<T>, TileDataDst, TileDataSrc0, TileDataSrc1, ElementsPerRepeat, BlockSizeElem>(
...
PTO_INTERNAL void TMaxCheck(const TileDataDst& dst, const TileDataSrc0& src0, const TileDataSrc1& src1)
{
    ...
    static_assert(
        ...
        "Fix: TMAX input tile src0, src1 and dst tile data type mismatch.");
...
{
    using T = typename TileDataDst::DType;
    TMaxCheck<TileDataDst, TileDataSrc0, TileDataSrc1>(dst, src0, src1);
    constexpr unsigned blockSizeElem = BLOCK_BYTE_SIZE / sizeof(T);
    constexpr unsigned elementsPerRepeat = CCE_VL / sizeof(T);

    TMax<TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem>(
