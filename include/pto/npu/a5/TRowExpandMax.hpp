    ...
    unsigned version = VFImplKind::VFIMPL_DEFAULT)
{
    ...
    if constexpr (std::is_same_v<T, int64_t> || std::is_same_v<T, uint64_t>) {
        Int64RowExpandBinary<RowExpandMaxOp<T>, TileDataDst, TileDataSrc0, TileDataSrc1>(
            dstPtr, src0Ptr, src1Ptr, validRow, validCol);
    } else {
        RowExpandBinaryInstr<
            RowExpandMaxOp<T>, TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem>(
            dstPtr, src0Ptr, src1Ptr, validRow, validCol);
    }
}
