    ...
    VFImplKind version = VFImplKind::VFIMPL_DEFAULT)
{
    ...
    if constexpr (std::is_same_v<T, int64_t> || std::is_same_v<T, uint64_t>) {
        Int64Scalar<Int64Op::Shr, T, TileDataDst::Cols, TileDataSrc::Cols>(
            dstPtr, src0Ptr, src1, kValidRows, kValidCols);
    } else {
        BinaryInstr<
            ShrSOp<T>, TileDataDst, TileDataSrc, T, elementsPerRepeat, blockSizeElem, dstRowStride, src0RowStride>(
