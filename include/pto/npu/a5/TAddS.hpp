...
__tf__ PTO_INTERNAL OP_NAME(TADDS) OP_TYPE(element_wise) void TAddS(
    ...
    VFImplKind version = VFImplKind::VFIMPL_DEFAULT)
{
    ...
    if constexpr (std::is_same_v<T, int64_t> || std::is_same_v<T, uint64_t>) {
        Int64Scalar<Int64Op::Add, T, TileDataDst::Cols, TileDataSrc::Cols>(
            dstPtr, src0Ptr, src1, kValidRows, kValidCols);
    } else {
        BinaryInstr<
            AddSOp<T>, TileDataDst, TileDataSrc, T, elementsPerRepeat, blockSizeElem, dstRowStride, src0RowStride>(
...
PTO_INTERNAL void TADDS_IMPL(TileDataDst& dst, TileDataSrc& src0, typename TileDataSrc::DType src1)
{
    ...
    static_assert(
        std::is_same_v<T, int64_t> || std::is_same_v<T, uint64_t> || std::is_same_v<T, int32_t> ||
            std::is_same_v<T, int16_t> || std::is_same_v<T, int8_t> || std::is_same_v<T, uint32_t> ||
            std::is_same_v<T, uint16_t> || std::is_same_v<T, uint8_t> || std::is_same_v<T, half> ||
            std::is_same_v<T, float32_t> || std::is_same_v<T, bfloat16_t>,
