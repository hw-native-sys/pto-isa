...
__tf__ PTO_INTERNAL void TLRelu(
...
PTO_INTERNAL void TLRELU_IMPL(TileDataDst& dst, TileDataSrc& src, typename TileDataSrc::DType scalar)
{
    ...
    static_assert(
        std::is_same_v<T, typename TileDataDst::DType>, "TLRELU: The data type of dst must be consistent with src.");
    static_assert(
    ...
    PTO_ASSERT(
        ...
        "TLRELU: Number of valid rows and valid columns of dst tile must be greater than 0.");
