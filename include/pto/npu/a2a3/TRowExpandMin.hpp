...
struct RowExpandMinOp {
    ...
    PTO_INTERNAL static void RowExpandBinInstr(
...
PTO_INTERNAL void TROWEXPANDMIN_IMPL(TileDataDst& dst, TileDataSrc0& src0, TileDataSrc1& src1)
{
    ...
    static_assert(
        std::is_same_v<T, int32_t> || std::is_same_v<T, int> || std::is_same_v<T, int16_t> || std::is_same_v<T, half> ||
            std::is_same_v<T, float16_t> || std::is_same_v<T, float> || std::is_same_v<T, float32_t>,
    ...
    if (src0eqdst) {
        ...
        TRowExpandBin<RowExpandMinOp<T>, TileDataDst, TileDataSrc0, TileDataSrc1>(
        ...
        TRowExpandBin<RowExpandMinOp<T>, TileDataDst, TileDataSrc1, TileDataSrc0>(
...
PTO_INTERNAL void TROWEXPANDMIN_IMPL(TileDataDst& dst, TileDataSrc0& src0, TileDataSrc1& src1, TileDataTmp& tmp)
{
    ...
    if (src0eqdst) {
        PTO_ASSERT(
            (!TileDataSrc1::isRowMajor && src1ValidCol == 1) && src1ValidRow == validRow,
        ...
        PTO_ASSERT(
            (!TileDataSrc0::isRowMajor && src0ValidCol == 1) && src0ValidRow == validRow,
