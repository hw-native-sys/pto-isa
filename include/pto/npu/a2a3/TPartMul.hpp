...
#ifndef TPARTIALMUL_HPP
#define TPARTIALMUL_HPP
...
__tf__ PTO_INTERNAL void TPartMul(
    ...
    unsigned src1ValidCol, unsigned dstValidRow, unsigned dstValidCol)
{
    ...
    PTO_ASSERT(
        ...
        "Fix: TPARTMUL At most one entry in the valid-rows and valid-cols of src0 and src1 is smaller than dst.");
    ...
    if (condSrc0EqDst) {
        TPartInstr<
            PartMulOp<T>, T, dstCol, src0Col, dstRow, elementsPerRepeat, blockSizeElem, dstRowStride, src0RowStride,
            src1RowStride>(
            dstPtr, src0Ptr, src1Ptr, src0ValidRow, src0ValidCol, src1ValidRow, src1ValidCol, dstValidRow, dstValidCol);
    } else if (condSrc1EqDst) {
        TPartInstr<
            PartMulOp<T>, T, dstCol, src1Col, dstRow, elementsPerRepeat, blockSizeElem, dstRowStride, src1RowStride,
            src0RowStride>(
...
PTO_INTERNAL void TPARTMUL_IMPL(TileDataDst& dst, TileDataSrc0& src0, TileDataSrc1& src1)
{
    ...
    static_assert(
        ...
        "Fix: TPARTMUL not supported BLayout type.");
