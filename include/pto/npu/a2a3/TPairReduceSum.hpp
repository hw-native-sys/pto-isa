    ...
    __ubuf__ T* dstPtr, __ubuf__ T* src0Ptr, unsigned validRows, unsigned validCols)
{
    ...
    unsigned remainElem = validCols % ElementsPerRepeat;
    constexpr unsigned dstRptStride = DstRowStride / BlockSizeElem;
    constexpr unsigned src0RptStride = Src0RowStride / BlockSizeElem;
    ...
    unsigned validCols)
{
    ...
    if constexpr (isContiguous) {
        TPairReduceSum_1D_NoPostUpdate<T, ElementsPerRepeat, BlockSizeElem, dstRowStride, src0RowStride>(
