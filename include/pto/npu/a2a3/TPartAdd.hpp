...
__tf__ PTO_INTERNAL void TPartAdd(
    ...
    unsigned src1ValidCol, unsigned dstValidRow, unsigned dstValidCol)
{
    ...
    if (condSrc0EqDst) {
        TPartInstr<
            PartAddOp<T>, T, dstCol, src0Col, dstRow, elementsPerRepeat, blockSizeElem, dstRowStride, src0RowStride,
            src1RowStride>(
            dstPtr, src0Ptr, src1Ptr, src0ValidRow, src0ValidCol, src1ValidRow, src1ValidCol, dstValidRow, dstValidCol);
    } else if (condSrc1EqDst) {
        TPartInstr<
            PartAddOp<T>, T, dstCol, src1Col, dstRow, elementsPerRepeat, blockSizeElem, dstRowStride, src1RowStride,
            src0RowStride>(
...
PTO_INTERNAL void TPARTADD_IMPL(TileDataDst& dst, TileDataSrc0& src0, TileDataSrc1& src1)
{
    ...
    static_assert(
        ...
        "Fix: TPARTADD not supported BLayout type.");
