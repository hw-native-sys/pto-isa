...
AICORE void vcmp_dispatch(
    __ubuf__ uint8_t* dst, __ubuf__ T_src* src0, T_scalar scalar, CmpMode mode, uint8_t rep, uint16_t b_dst,
...
AICORE void GenCmpCall(
    __ubuf__ TOUT* dst, __ubuf__ TIN* src0, TIN src1, CmpMode cmpMode, uint8_t repeat, uint16_t dstblockstride,
    uint16_t srcblockstride, uint16_t dstrepeatstride, uint16_t srcrepeatstride)
{
    if constexpr (std::is_same<TIN, int32_t>::value) {
        ...
        if (sizeof(TIN) == 4) {
            vcmp_dispatch(
                dst, src0, src1, cmpMode, repeat, dstblockstride, srcblockstride, dstrepeatstride, srcrepeatstride);
        } else {
            half scalar;
            ...
            vcmp_dispatch(
                dst, src_ptr, scalar, cmpMode, repeat, dstblockstride, srcblockstride, dstrepeatstride,
                srcrepeatstride);
        }
    }
}
...
template <typename TIN, typename TOUT>
AICORE void TCmps(
    __ubuf__ TOUT* dst, __ubuf__ TIN* src0, TIN src1Value, CmpMode mode, unsigned validRow, unsigned repeatPerLine,
    int srcAlignCols, int dstAlignCols)
{
    ...
    for (size_t i = 0; i < validRow; i++) {
        for (size_t j = 0; j < nLoop; j++) {
            GenCmpCall<TIN, TOUT>(
                dst + i * dstAlignCols + j * dstOffset, src0 + i * srcAlignCols + j * srcOffset, src1Value, mode,
...
__tf__ AICORE void TCmps_Scalar(
...
{
    static_assert(
        std::is_same<typename TileDataSrc::DType, int32_t>::value ||
