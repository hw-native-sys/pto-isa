...
struct SubSOp {
    ...
    {
        vadds(dst, src0, (T)(-(float)src1), repeats, 1, 1, 8, 8);
    }
    ...
    {
        vadds(dst, src0, (T)(-(float)src1), repeats, 1, 1, dstRepeatStride, srcRepeatStride);
    }
};
...
PTO_INTERNAL void TSUBS_IMPL(TileDataDst& dst, TileDataSrc& src, typename TileDataSrc::DType scalar)
{
    using T = typename TileDataSrc::DType;
    static_assert(
        std::is_same_v<T, typename TileDataDst::DType>, "TSUBS: The data type of dst must be consistent with src.");
    static_assert(
    ...
    PTO_ASSERT(src.GetValidCol() == dst.GetValidCol(), "Number of valid columns of src and dst must be the same.");
    PTO_ASSERT(src.GetValidRow() == dst.GetValidRow(), "Number of valid rows of src and dst must be the same.");

    unsigned dstValidRow = dst.GetValidRow();
    unsigned dstValidCol = dst.GetValidCol();
    if ((dstValidRow != 0 && dstValidCol != 0) &&
        (dstValidRow == src.GetValidRow() && dstValidCol == src.GetValidCol())) {
        TSubS<T, TileDataDst, TileDataSrc>(dst.data(), src.data(), scalar, dstValidRow, dstValidCol);
    } else {
        PTO_ASSERT(false, "TSUBS: dstTile validRow/validCol must be consistent with of src.");
    }
}
} // namespace pto
