...
PTO_INTERNAL void TRowReduceIdxCheck(
    uint32_t srcValidRows, uint32_t srcValidCols, uint32_t dstValValidRow, uint32_t dstIdxValidRow)
{
    ...
    if constexpr (outputVal) {
        ...
        static_assert(
            std::is_same_v<uint32_t, TIdx> || std::is_same_v<int32_t, TIdx>,
            "Input and output tile data types must match. "
            "Fix: Ensure TileDataOutIdx uses the same DType as TileDataIn.");
    }
...
PTO_INTERNAL void ReduceThenGroupValIdx(
    __ubuf__ TVal* dstVal, __ubuf__ TIdx* dstIdx, __ubuf__ TVal* src, uint32_t count)
{
...
template <
    bool outputVal, typename InstrOp, typename TileDataOutVal, typename TileDataOut, typename TileDataIn,
    typename TileDataTmp>
PTO_INTERNAL void ProcReduceIdxStage2(
    __ubuf__ typename TileDataOutVal::DType* dstVal, __ubuf__ typename TileDataOut::DType* dst,
    __ubuf__ typename TileDataIn::DType* src, __ubuf__ typename TileDataTmp::DType* tmp, int validRow, int validCol)
{
...
template <
    bool outputVal, typename InstrOp, typename TileDataOutVal, typename TileDataOut, typename TileDataIn,
    typename TileDataTmp>
PTO_INTERNAL void ProcReduceIdxStage1(
    __ubuf__ typename TileDataOutVal::DType* dstVal, __ubuf__ typename TileDataOut::DType* dst,
    __ubuf__ typename TileDataIn::DType* src, __ubuf__ typename TileDataTmp::DType* tmp, int validRow, int validCol)
{
    using T = typename TileDataIn::DType;
    ...
    for (int i = 0; i < validRow; i++) {
        ReduceThenGroupValIdx<InstrOp, T, U>(
            ...
            reinterpret_cast<__ubuf__ U*>(tmp + i * TileDataTmp::Cols), src + i * TileDataIn::Cols, (uint32_t)validCol);
...
PTO_INTERNAL void OneRepeatProcIdx(
    __ubuf__ typename TileDataOut::DType* dst, __ubuf__ typename TileDataIn::DType* src, int validRow, int validCol)
{
    using T = typename TileDataIn::DType;
    ...
    if constexpr (TileDataOut::Cols > B16_REPEAT_MAX) {
        ...
        for (int i = 0; i < validRow; i++) {
            InstrOp::ReduceIdxInstrImpl(
                reinterpret_cast<__ubuf__ T*>(dst) + i * TileDataOut::Cols, src + i * TileDataIn::Cols, 1, 0, 1, 0);
        }
        pipe_barrier(PIPE_V);
    } else {
...
PTO_INTERNAL void ExtractValIdxFromTmpColMajor(
    ...
    __ubuf__ typename TileDataTmp::DType* tmp, int validRow)
{
    ...
    if constexpr (TileDataOutVal::Cols == 1) {
        vreducev2(
            reinterpret_cast<__ubuf__ U*>(dstVal), reinterpret_cast<__ubuf__ U*>(tmp),
    ...
    if constexpr (TileDataOutIdx::Cols == 1) {
        if constexpr (sizeof(TIdx) != sizeof(T)) {
            ...
            pipe_barrier(PIPE_V);
            vconv_f322s32z(
                reinterpret_cast<__ubuf__ int32_t*>(dstIdx), reinterpret_cast<__ubuf__ float*>(dstIdx), 0, 1, 1,
                BLOCK_MAX_PER_REPEAT, BLOCK_MAX_PER_REPEAT);
            pipe_barrier(PIPE_V);
        } else {
            vreducev2(
                reinterpret_cast<__ubuf__ U*>(dstIdx), reinterpret_cast<__ubuf__ U*>(tmp),
...
template <typename TileDataOutVal, typename TileDataOutIdx, typename TileDataTmp>
PTO_INTERNAL void ExtractValIdxFromTmp(
    __ubuf__ typename TileDataOutVal::DType* dstVal, __ubuf__ typename TileDataOutIdx::DType* dstIdx,
    __ubuf__ typename TileDataTmp::DType* tmp, int validRow)
{
    using T = typename TileDataOutVal::DType;
...
PTO_INTERNAL void OneRepeatProcValIdx(
    __ubuf__ typename TileDataOutVal::DType* dstVal, __ubuf__ typename TileDataOutIdx::DType* dstIdx,
    __ubuf__ typename TileDataIn::DType* src, __ubuf__ typename TileDataTmp::DType* tmp, int validRow, int validCol)
{
    using T = typename TileDataIn::DType;
...
template <
    bool outputVal, typename InstrOp, typename TileDataOutVal, typename TileDataOut, typename TileDataIn,
    typename TileDataTmp>
PTO_INTERNAL void TRowReduceIdxInstr(
    __ubuf__ typename TileDataOutVal::DType* dstVal, __ubuf__ typename TileDataOut::DType* dst,
    __ubuf__ typename TileDataIn::DType* src, __ubuf__ typename TileDataTmp::DType* tmp, int validRow, int validCol,
    int dstValValidRow, int dstIdxValidRow)
{
    TRowReduceIdxCheck<TileDataOutVal, TileDataOut, TileDataIn, outputVal>(
        validRow, validCol, dstValValidRow, dstIdxValidRow);
    constexpr uint8_t elemPerRpt = REPEAT_BYTE / sizeof(typename TileDataIn::DType);
    if (validCol <= elemPerRpt) {
        if constexpr (outputVal) {
            OneRepeatProcValIdx<InstrOp, TileDataOutVal, TileDataOut, TileDataIn, TileDataTmp>(
...
struct TRowArgMaxOp {
    PTO_INTERNAL static void ReduceIdxInstrImpl(
        __ubuf__ TVal* dst, __ubuf__ TVal* src, uint8_t rptTimes, uint16_t dstRptStride, uint16_t srcBlkStride,
        uint16_t srcRptStride)
    {
        vcmax(dst, src, rptTimes, dstRptStride, srcBlkStride, srcRptStride, ONLY_INDEX);
    }

    PTO_INTERNAL static void ReduceValIdxInstrImpl(
        __ubuf__ TVal* dst, __ubuf__ TVal* src, uint8_t rptTimes, uint16_t dstRptStride, uint16_t srcBlkStride,
        uint16_t srcRptStride)
    {
...
__tf__ PTO_INTERNAL void TRowIdxMax(
    typename TVal::TileDType __out__ dstValData, typename TIdx::TileDType __out__ dstData,
    typename TIn::TileDType __in__ srcData, typename TTmp::TileDType __in__ tmpData, int validRow, int validCol,
...
__tf__ PTO_INTERNAL void TRowIdxMin(
    typename TileDataOutVal::TileDType __out__ dstValData, typename TileDataOut::TileDType __out__ dstData,
    typename TileDataIn::TileDType __in__ srcData, typename TileDataTmp::TileDType __in__ tmpData, int validRow,
    int validCol, int dstValValidRow, int dstIdxValidRow, unsigned version)
{
