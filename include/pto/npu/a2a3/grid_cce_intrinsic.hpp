...
//
// clang-format off
//   V8 machine instr  | CCE facade (this file) | CCE builtin (native)
//   ------------------+------------------------+--------------------------------------
//   COPY_L1_TO_PEER   | copy_l1_to_peer_l1     | __builtin_cce_copy_ubuf_to_peer_ubuf
//   SYNC_HSCB/ST_HSCB | sync_hscb              | __builtin_cce___sync_hscb
//   WAIT_SPR          | wait_ipc_scb           | __builtin_cce___wait_ipc_scb
//   MOV_SPR2X         | mov_ipc_scb_to_l1      | __builtin_cce___mov_ipc_scb_to_l1
//   MOVX2SPR          | mov_x_to_ipc_scb       | __builtin_cce___mov_x_to_ipc_scb
//   MOVX2GPR          | mov_x_to_gpr           | __builtin_cce___mov_x_to_gpr
//   ATOM_ADD_HSCB     | atom_add_hscb          | __builtin_cce___atom_add_hscb
// clang-format on
//
...
// exposes neither __builtin_cce_copy_ubuf_to_peer_ubuf nor a blocking
// __builtin_cce___wait_ipc_scb on IPC_SCB (__sync_hscb exists, but its machine op
// cannot address a resolved peer's IPC_SCB / L1).  So the DEFAULT build models
// each channel's IPC_SCB slot as a volatile GM word and the peer L1 as a GM
// window: SYNC_HSCB -> cross-core GM store + cache maintenance; WAIT_SPR -> GM
// spin-poll (read+block); COPY_L1_TO_PEER -> local producer-window read followed
...
// Inverse of GridBlockRectMember: the rank-in-group of `blockId`, or -1 when it is
// not a member.  A group collective indexes its PER-MEMBER bookkeeping by this rank
// (which member delivered a tile, where to push its credit) while the fabric names
// the same core by block id, so the two directions have to agree exactly -- hence
// one shared row-major definition rather than a second one open-coded at the call
// site.
AICORE constexpr int GridBlockRectRankOf(const GridBlockRect& g, uint32_t blockId)
{
    if (!GridBlockRectContains(g, blockId)) {
        return -1;
    }
    const uint32_t cols = GridBlockRectColSpan(g);
    const uint32_t rowStep = blockId / g.meshCols - g.topLeft / g.meshCols;
    const uint32_t colStep = blockId % g.meshCols - g.topLeft % g.meshCols;
    return static_cast<int>(rowStep * cols + colStep);
}
...
// ---------------------------------------------------------------------------
// (1) COPY_L1_TO_PEER  ->  copy_l1_to_peer_l1
//                      ->  __builtin_cce_copy_ubuf_to_peer_ubuf
//
...
//
// PEER, NOT NEIGHBOUR -- and the rename is a semantic correction, not cosmetics.
// The instruction was originally spelled COPY_UBUF_TO_NBR / COPY_L1_TO_NBR because
// its operands were (dir, dist): a COMPASS POINT plus a hop count, i.e. an address
// derived from mesh geometry.  Nothing in this family derives an address that way
// any more.  Every op names its counterpart by LOGICAL BLOCK ID and the resolver
// maps a local address to the same byte offset in that core's window, so the
// target is whichever core the schedule says -- adjacent or not.  "Neighbour"
// would keep suggesting a constraint (one hop, a fixed direction) that neither the
// instruction nor its callers have; "peer" says exactly what the operand is.  The
// mesh-topology helpers in grid_intrinsic.hpp keep their neighbour spelling on
// purpose: THEY are about geometry, and they are how a CALL SITE turns "my
// downstream on this axis" into the peer id it then hands to TPUSH.
//
// §3.3 G1 operands map to this facade as: `dstPeerSlot` = the resolved peer L1
// slot (native: the peer L1 address the fabric resolves from the peer id; mock:
// the GM window standing in for it); `srcProducerSlot` = the isolated local L1
// producer slot (mock: a disjoint range in this core's GM window);
// `transferScratch` is only the A3 mock's UB DMA pump and is not an architectural
// source address; `bytes` = payload size.
// ---------------------------------------------------------------------------
AICORE inline void copy_l1_to_peer_l1(
    __gm__ void* dstPeerSlot, __gm__ const void* srcProducerSlot, __ubuf__ void* transferScratch, uint32_t bytes)
{
#if defined(PTO_GRID_CCE_NATIVE)
    (void)transferScratch;
    // The builtin keeps the "ubuf" spelling of its OPERAND CLASS: on WSE that
    // qualifier names the same physical unified L1 SRAM, and the pointer value is
    // the producer staging address, never the caller's tile address.  Its TARGET
    // is spelled peer, like the facade and the machine instruction -- the whole
    // family addresses a resolved peer, never a geometric neighbour, and the name
    // is part of the ISA extension this file requests (HW-DEP-0).
    auto* srcUnifiedL1 = reinterpret_cast<__ubuf__ void*>(reinterpret_cast<uint64_t>(srcProducerSlot));
    __builtin_cce_copy_ubuf_to_peer_ubuf(dstPeerSlot, srcUnifiedL1, bytes, /*config=*/0);
    ...
    (void)transferScratch;
    auto* dstBytes = reinterpret_cast<uint8_t*>(dstPeerSlot);
    ...
    auto* dstBytes = reinterpret_cast<__gm__ uint8_t*>(dstPeerSlot);
...
// ===========================================================================
// (4b) ATOM_ADD_HSCB  ->  __atom_add_hscb  ->  __builtin_cce___atom_add_hscb
//
// ATOMICALLY ADD into a resolved peer IPC_SCB (HW-DEP-A, 选型文档 §2.3 -- the ISA
// does not endorse it yet, so it is a declared dependency, exactly like the
// neighbour addressing of SYNC_HSCB).  It is the ONE primitive that lets several
// cores raise the SAME scoreboard without losing updates: add is commutative and
// associative, so K concurrent contributions aggregate at a single point instead
// of clobbering each other (M1).  Overwrite stores stay the rule for every
// single-writer edge; this is only for the credit counters a group collective
// fans in.
//
// Why the counters it serves stay EXACT: every grantor contributes at most the
// number the waiter's threshold divides by, so `sum >= grantors * n` can only hold
// when EVERY grantor has reached n.  A sum whose per-grantor cap exceeds n would
// let a fast grantor mask a slow one -- that is the 伪解法 §9 warns about, and it
// is why the thresholds in GridTBroadcast.hpp / GridTReduce.hpp are derived from
// the grant COUNT rather than from an arbitrary absolute index.
//
// A2/A3 mock: there is no scalar cross-core atomic, so the accumulate rides the
// MTE3 atomic-add mode (`set_atomic_s32` + `set_atomic_add`, the same path
// MScatter uses for its GM accumulate) with a caller-provided single-word UB
// source.  `creditScratch` is that word; native ignores it because the machine
// instruction carries the delta as an operand.
// ===========================================================================
AICORE inline void atom_add_hscb(__gm__ uint32_t* peerScb, uint32_t delta, __ubuf__ uint32_t* creditScratch)
{
#if defined(PTO_GRID_CCE_NATIVE)
    (void)creditScratch;
    __atom_add_hscb(peerScb, delta); // -> __builtin_cce___atom_add_hscb; encoding per ISA manual (HW-DEP-A)
#else
    if (peerScb == nullptr || creditScratch == nullptr) {
        return;
    }
    *reinterpret_cast<volatile __ubuf__ uint32_t*>(creditScratch) = delta;
    __asm__ __volatile__("" ::: "memory");
#ifndef __PTO_AUTO__
    pipe_barrier(PIPE_ALL); // the scalar store above must land before MTE3 reads it
#endif
    set_atomic_s32();
    set_atomic_add();
    copy_ubuf_to_gm_align_b32(
        reinterpret_cast<__gm__ int32_t*>(peerScb), reinterpret_cast<__ubuf__ int32_t*>(creditScratch), 0, 1,
        static_cast<uint32_t>(sizeof(uint32_t)), 0, 0, 0, 0);
    set_atomic_none();
#ifndef __PTO_AUTO__
    pipe_barrier(PIPE_ALL);
#endif
...
// ===========================================================================
// (6b) [HW-DEP-B] PEER WORD PULL -- the sink half of the scheme-C group reduce.
//
// A pull collective (mov_ubuf_group with op = SUM/MAX/MIN) already fetches every
// member's CONTRIBUTION itself.  Scheme C (2026-08-12 组归约门铃归属方案分析 §4)
// extends that to the READY FLAG: a member publishes "my round-r contribution is
// in place" with an ordinary LOCAL store, and the sink pulls those words too, so
// the member side costs ZERO cross-core actions.
//
// The price is stated in that analysis and is not an implementation detail:
// WAIT_SPR can only block on THIS core's IPC_SCB, and this core may not write its
// own IPC_SCB (选型 §1.1 约束①), so a pulled flag cannot be suspended on.  The
// sink SPINS.
//
// HW-DEP-B, the proposal: give the group instruction a scalar reduce mode --
// `op = ALL_GE(threshold)` or `op = MIN` -- whose result may land in a
// WAIT_SPR-able IPC_SCB, i.e. let the group instruction perform the push the
// members no longer perform.  Until that exists, native and mock lower the same
// way, and this facade marks every place that would collapse into it.
//
// This is deliberately the ONLY scalar read of a peer's window in GridPipe.  It
// is legal here because it travels in the same direction the reduce's payload
// already does; a push collective (TBROADCAST) has no such licence, which is why
// scheme C applies to the reduce and not to the broadcast.
// ===========================================================================
AICORE inline uint32_t mov_peer_word_to_gpr(__gm__ uint32_t* peerWord)
{
    // Same load as mov_x_to_gpr, and deliberately a different NAME: the invalidate
    // in front of it is what makes another core's store visible, and calling it
    // "local" at this call site would hide the one place the rule is bent.
    return grid_cce_detail::read_local_word(peerWord);
}
    ...
    uint32_t blockStride, const pto::GridBlockRect& group, uint32_t selfBlockId, uint64_t groupDesc = 0)
{
    ...
    for (uint32_t k = 0; k < memberCount; ++k) {
        ...
        copy_l1_to_peer_l1(dst, srcProducerSlot, transferScratch, bytes);
    }
#endif
}
