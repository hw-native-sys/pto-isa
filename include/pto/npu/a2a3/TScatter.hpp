...
__tf__ PTO_INTERNAL void TScatterImpl(
    typename DstTile::TileDType __out__ dst, typename SrcTile::TileDType __in__ src,
...
PTO_INTERNAL void TSCATTER_IMPL(DstTile& dst, SrcTile& src, IdxTile& idx)
{
    ...
    static_assert(
        std::is_same<TI, uint16_t>::value || std::is_same<TI, uint32_t>::value || std::is_same<TI, int16_t>::value ||
...
__tf__ PTO_INTERNAL void TScatterMaskImpl(
    typename DstTile::TileDType __out__ dst, typename SrcTile::TileDType __in__ src, unsigned validRow,
    unsigned validCol)
{
    using T = typename DstTile::DType;
    __ubuf__ T* dstPtr = (__ubuf__ T*)__cce_get_tile_ptr(dst);
...
PTO_INTERNAL void TSCATTER_IMPL(DstTile& dst, SrcTile& src)
{
    ...
    if constexpr (mask == MaskPattern::P1111) {
        ...
        static_assert(
            ...
            "TSCATTER: Invalid data type.");
        static_assert(
            std::is_same_v<T, typename SrcTile::DType>, "TSCATTER: Data type of dst and src must be the same.");
        ...
        static_assert(
            ...
            "TSCATTER: Number of valid columns must not be greater than number of tile columns.");
        static_assert(
            DstTile::ValidRow <= DstTile::Rows && SrcTile::ValidRow <= SrcTile::Rows,
            "TSCATTER: Number of valid rows must not be greater than number of tile rows.");
        static_assert(
