...
__tf__ AICORE void TMrgsort(
    typename DstTileData::TileDType __out__ dst, typename TmpTileData::TileDType __out__ tmp,
    typename Src0TileData::TileDType __in__ src0, typename Src1TileData::TileDType __in__ src1,
    typename Src2TileData::TileDType __in__ src2, typename Src3TileData::TileDType __in__ src3, unsigned dstCol,
    uint16_t& mrgSortList0, uint16_t& mrgSortList1, uint16_t& mrgSortList2, uint16_t& mrgSortList3, unsigned src0Col,
    unsigned src1Col, unsigned src2Col, unsigned src3Col)
{
...
template <
    typename DstTileData, typename TmpTileData, typename Src0TileData, typename Src1TileData, typename Src2TileData,
    typename Src3TileData, unsigned listNum>
...
template <
    typename DstTileData, typename TmpTileData, typename Src0TileData, typename Src1TileData, typename Src2TileData,
    typename Src3TileData>
PTO_INTERNAL void CheckStatic()
{
    using DstType = typename DstTileData::DType;
    static_assert(
        (std::is_same<DstType, half>::value) || (std::is_same<DstType, float>::value),
        "TMrgsort: Unsupported data type! Supported types is half/float");
    static_assert(
        (std::is_same<DstType, typename TmpTileData::DType>::value) &&
    ...
    static_assert(
        (DstTileData::Loc == TileType::Vec) && (TmpTileData::Loc == TileType::Vec) &&
    ...
    static_assert(
        (DstTileData::Rows == ONE_ROW) && (TmpTileData::Rows == ONE_ROW) && (Src0TileData::Rows == ONE_ROW) &&
            (Src1TileData::Rows == ONE_ROW) && (Src2TileData::Rows == ONE_ROW),
        "TMrgsort: the row of Destination and Source tile must be 1.");
    static_assert(
        (DstTileData::isRowMajor && TmpTileData::isRowMajor && Src0TileData::isRowMajor && Src1TileData::isRowMajor &&
         Src2TileData::isRowMajor && Src3TileData::isRowMajor),
    ...
    Src2TileData& src2, Src3TileData& src3)
{
    ...
    PTO_ASSERT(
        (src0.GetValidCol() + src1.GetValidCol() + src2.GetValidCol() + src3.GetValidCol() + tmp.GetValidCol()) *
    ...
    Src2TileData& src2)
{
    ...
    unsigned dstCol = dst.GetValidCol();
    PTO_ASSERT(
        (src0.GetValidCol() + src1.GetValidCol() + src2.GetValidCol() + tmp.GetValidCol()) *
                sizeof(typename DstTileData::DType) <
            UB_SIZE,
        "ERROR: Total memory usage exceeds UB limit!");
    constexpr unsigned ELE_NUM_SHIFT = (std::is_same<typename DstTileData::DType, float>::value) ? 1 : 2;
