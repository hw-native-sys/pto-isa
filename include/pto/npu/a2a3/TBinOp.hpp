...
template <
    typename Op, typename T, unsigned elementsPerRepeat, unsigned blockSizeElem, unsigned rowStride, unsigned tileCols>
    ...
    __ubuf__ T* dstPtr, __ubuf__ T* src0Ptr, __ubuf__ T* src1Ptr, unsigned validRow, unsigned numRemainPerLine)
{
    ...
    if constexpr (Rows > pto::REPEAT_MAX) {
        ...
        for (int i = 0; i < numLoop; i++) {
            if constexpr (strideOverFlag) {
                ...
                Op::BinInstr(
                    dstPtr + offset, src0Ptr + offset, src1Ptr + offset, REPEAT_MAX, repeatStride, repeatStride,
                    repeatStride);
            }
        }
    ...
    if (remainAfterLoop) {
        if constexpr (strideOverFlag) {
            ...
            uint8_t repeatStride = stride / blockSizeElem;
            Op::BinInstr(
                dstPtr + offset, src0Ptr + offset, src1Ptr + offset, remainAfterLoop, repeatStride, repeatStride,
                repeatStride);
        }
    }
    SetFullVecMaskByDType<T>();
...
template <
    typename Op, typename T, unsigned Rows, unsigned elementsPerRepeat, unsigned blockSizeElem, unsigned rowStride>
    ...
    __ubuf__ T* dstPtr, __ubuf__ T* src0Ptr, __ubuf__ T* src1Ptr, unsigned validRow, unsigned validCol)
{
    ...
    if constexpr (condRowRpt) {
        ...
        for (unsigned i = 0; i < numLoop; i++) {
            ...
            Op::BinInstr(
                dstPtr + offset, src0Ptr + offset, src1Ptr + offset, validRow, repeatStride, repeatStride,
                repeatStride);
        }
...
template <
    typename Op, typename T, typename TileData, unsigned elementsPerRepeat, unsigned blockSizeElem, unsigned rowStride>
PTO_INTERNAL void BinaryInstrFastPath(
    __ubuf__ T* dstPtr, __ubuf__ T* src0Ptr, __ubuf__ T* src1Ptr, unsigned validRow, unsigned validCol)
{
...
template <
    typename Op, typename T, typename TileData, unsigned elementsPerRepeat, unsigned blockSizeElem, unsigned rowStride>
PTO_INTERNAL void BinaryInstrGeneralPath(
    __ubuf__ T* dstPtr, __ubuf__ T* src0Ptr, __ubuf__ T* src1Ptr, unsigned validRow, unsigned validCol)
{
    ...
    __ubuf__ T* dstPtr, __ubuf__ T* src0Ptr, __ubuf__ T* src1Ptr, unsigned validRow, unsigned validCol)
{
    ...
    if constexpr ((TileData::Cols == TileData::ValidCol) || (TileData::Rows == 1)) {
        BinaryInstrFastPath<Op, T, TileData, elementsPerRepeat, blockSizeElem, rowStride>(
            dstPtr, src0Ptr, src1Ptr, validRow, validCol);
    } else {
        BinaryInstrGeneralPath<Op, T, TileData, elementsPerRepeat, blockSizeElem, rowStride>(
            dstPtr, src0Ptr, src1Ptr, validRow, validCol);
    }
}
...
template <
    typename Op, typename T, unsigned elemPerRpt, unsigned elemPerBlk, unsigned dstStride, unsigned src0Stride,
    unsigned src1Stride>
PTO_INTERNAL void Bin2LNormModeTail(
    __ubuf__ T* dst, __ubuf__ T* src0, __ubuf__ T* src1, unsigned validRow, unsigned remain)
{
    ...
    if (remainAfterLoop) {
        if constexpr (src0StrideOverFlag || src1StrideOverFlag || dstStrideOverFlag) {
            ...
            uint8_t src1BlkPerLine = src1Stride / elemPerBlk;
            Op::BinInstr(
                dst + dstOffset, src0 + src0Offset, src1 + src1Offset, remainAfterLoop, dstBlkPerLine, src0BlkPerLine,
                src1BlkPerLine);
        }
    }
    SetFullVecMaskByDType<T>();
