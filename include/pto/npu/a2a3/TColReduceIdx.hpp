...
template <typename TileDataOutVal, typename TileDataOutIdx, typename TileDataIn, typename TileDataTmp, bool IsArgMax,
          bool IsRemainingBlock, bool WithVal>
PTO_INTERNAL void ProcessSingleBlock(__ubuf__ typename TileDataOutVal::DType *dstVal,
                                     __ubuf__ typename TileDataOutIdx::DType *dstIdx,
                                     __ubuf__ typename TileDataIn::DType *src,
                                     __ubuf__ typename TileDataTmp::DType *tmp, unsigned srcValidRow,
                                     uint16_t blockIndex, uint32_t tmpGapElems, uint32_t elemPerRpt,
                                     uint16_t remainingElements = 0)
{
    ...
    if constexpr (IsRemainingBlock) {
        vcopy(reinterpret_cast<__ubuf__ TIdx *>(elemTmpPtr),
    ...
    for (uint16_t rowIdx = 1; rowIdx < srcValidRow; ++rowIdx) {
        ...
        TColIdxCompareOp<TCmp, IsArgMax>::VCmp(reinterpret_cast<__ubuf__ TCmp *>(elemTmpPtr),
                                               reinterpret_cast<__ubuf__ TCmp *>(currentSrc));
        pipe_barrier(PIPE_V);

        vsel(reinterpret_cast<__ubuf__ TCmp *>(selectedCmpVals), reinterpret_cast<__ubuf__ TCmp *>(selectedCmpVals),
             reinterpret_cast<__ubuf__ TCmp *>(tmp), 1, 1, 1, 1, 0, 0, 0, 0);

        TColIdxCompareOp<TCmp, IsArgMax>::UpdateValue(reinterpret_cast<__ubuf__ TCmp *>(elemTmpPtr),
                                                      reinterpret_cast<__ubuf__ TCmp *>(elemTmpPtr),
                                                      reinterpret_cast<__ubuf__ TCmp *>(currentSrc));
        pipe_barrier(PIPE_V);
    }

    if constexpr (isHalfType && !WithVal) {
        vconv_s162f16a(reinterpret_cast<__ubuf__ half *>(tmp) + 2 * tmpGapElems,
                       reinterpret_cast<__ubuf__ int16_t *>(tmp) + 2 * tmpGapElems, 1, 1, 1, 0, 0);
        pipe_barrier(PIPE_V);

        vconv_f162s32a(reinterpret_cast<__ubuf__ int32_t *>(dstIdx) + blockIndex * elemPerRpt,
    ...
    if constexpr (WithVal && !IsRemainingBlock) {
        ...
        vcopy(reinterpret_cast<__ubuf__ TIdx *>(dstVal) + blockIndex * elemPerRpt,
...
template <typename TileDataOutVal, typename TileDataOutIdx, typename TileDataIn, typename TileDataTmp, bool IsArgMax,
          bool WithVal>
PTO_INTERNAL void ProcessFullBlocks(__ubuf__ typename TileDataOutVal::DType *dstVal,
                                    __ubuf__ typename TileDataOutIdx::DType *dstIdx,
                                    __ubuf__ typename TileDataIn::DType *src, __ubuf__ typename TileDataTmp::DType *tmp,
                                    unsigned srcValidRow, unsigned srcValidCol, uint16_t numFullBlocks,
                                    uint32_t tmpGapElems, uint32_t elemPerRpt)
{
    if (numFullBlocks == 0)
        return;
...
template <typename TileDataOutVal, typename TileDataOutIdx, typename TileDataIn, typename TileDataTmp, bool IsArgMax,
          bool WithVal>
PTO_INTERNAL void ProcessRemainingElements(__ubuf__ typename TileDataOutVal::DType *dstVal,
                                           __ubuf__ typename TileDataOutIdx::DType *dstIdx,
                                           __ubuf__ typename TileDataIn::DType *src,
                                           __ubuf__ typename TileDataTmp::DType *tmp, unsigned srcValidRow,
                                           unsigned srcValidCol, uint16_t numFullBlocks, uint32_t tmpGapElems,
                                           uint32_t elemPerRpt)
{
...
template <typename TileDataOutVal, typename TileDataOutIdx, typename TileDataIn, typename TileDataTmp, bool IsArgMax,
          bool WithVal>
PTO_INTERNAL void TColReduceIdxImpl(__ubuf__ typename TileDataOutVal::DType *dstVal,
                                    __ubuf__ typename TileDataOutIdx::DType *dstIdx,
                                    __ubuf__ typename TileDataIn::DType *src, __ubuf__ typename TileDataTmp::DType *tmp,
...
template <bool IsArgMax, typename TileDataOutVal, typename TileDataOutIdx, typename TileDataIn, typename TileDataTmp,
          bool WithVal>
__tf__ PTO_INTERNAL void TColReduceIdxInstr(typename TileDataOutVal::TileDType __out__ dstValData,
                                            typename TileDataOutIdx::TileDType __out__ dstIdxData,
                                            typename TileDataIn::TileDType __in__ srcData,
                                            typename TileDataTmp::TileDType __in__ tmpData, unsigned srcValidRow,
...
template <bool IsArgMax, typename TileDataOutVal, typename TileDataOutIdx, typename TileDataIn, typename TileDataTmp,
          bool WithVal = false>
PTO_INTERNAL void TColReduceIdxDispatch(TileDataOutVal &dstVal, TileDataOutIdx &dstIdx, TileDataIn &src,
                                        TileDataTmp &tmp)
{
...
{
    TColReduceIdxDispatch<true, TileDataOutVal, TileDataOutIdx, TileDataIn, TileDataTmp, true>(dstVal, dstIdx, src,
                                                                                               tmp);
}
