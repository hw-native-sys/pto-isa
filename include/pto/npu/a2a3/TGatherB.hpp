    ...
    for (int i = 0; i < validRow; i++) {
        if (numLoop) {
            for (int j = 0; j < numLoop; j++) {
                if constexpr (sizeof(T) == 1) {
                    ...
                    Op::GatherBInstr(
                        (__ubuf__ T*)(dstPtr + i * dstRowStride + j * elementsPerRepeat * REPEAT_MAX),
                        offsetPtr + i * offsetRowStride + j * 8 * REPEAT_MAX, srcAddr, 8, REPEAT_MAX);
                }
            }
        }
        if (remainAfterLoop) {
            if constexpr (sizeof(T) == 1) {
                ...
                Op::GatherBInstr(
                    (__ubuf__ T*)(dstPtr + i * dstRowStride + numLoop * elementsPerRepeat * REPEAT_MAX),
                    offsetPtr + i * offsetRowStride + numLoop * 8 * REPEAT_MAX, srcAddr, 8, remainAfterLoop);
            }
        }
    }
    ...
    __ubuf__ T* dstPtr, __ubuf__ uint32_t* offsetPtr, uint32_t& srcAddr, unsigned validRow, unsigned validCol)
{
    ...
    if (numLoop) {
        for (int i = 0; i < numLoop; i++) {
            if constexpr (sizeof(T) == 1) {
                Op::GatherBInstrB8(
                    (__ubuf__ uint16_t*)(dstPtr + i * REPEAT_MAX * dstRowStride),
                    offsetPtr + i * REPEAT_MAX * offsetRowStride, srcAddr, dstRowStride / blockSizeElem, REPEAT_MAX);
            } else {
                Op::GatherBInstr(
                    (__ubuf__ T*)(dstPtr + i * REPEAT_MAX * dstRowStride), offsetPtr + i * REPEAT_MAX * offsetRowStride,
                    srcAddr, dstRowStride / blockSizeElem, REPEAT_MAX);
            }
        }
    }
    if (remainAfterLoop) {
        if constexpr (sizeof(T) == 1) {
            Op::GatherBInstrB8(
                (__ubuf__ uint16_t*)(dstPtr + numLoop * REPEAT_MAX * dstRowStride),
                offsetPtr + numLoop * REPEAT_MAX * offsetRowStride, srcAddr, dstRowStride / blockSizeElem,
                remainAfterLoop);
        } else {
            Op::GatherBInstr(
                (__ubuf__ T*)(dstPtr + numLoop * REPEAT_MAX * dstRowStride),
                offsetPtr + numLoop * REPEAT_MAX * offsetRowStride, srcAddr, dstRowStride / blockSizeElem,
                remainAfterLoop);
        }
    }
...
__tf__ PTO_INTERNAL void TGatherB(
    typename TileDataDst::TileDType __out__ dst, typename TileDataSrc::TileDType __in__ src,
    typename TileDataOffset::TileDType __in__ offset, unsigned validRow, unsigned validCol)
{
