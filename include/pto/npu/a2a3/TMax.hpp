    ...
    typename TileData::TileDType __in__ src1, unsigned validRows, unsigned validCols)
{
    ...
    if constexpr (dstRowStride == src0RowStride && dstRowStride == src1RowStride) {
        BinaryInstr<MaxOp<T>, T, TileData, elementsPerRepeat, blockSizeElem, dstRowStride>(
            dstPtr, src0Ptr, src1Ptr, validRows, validCols);
    } else {
        BinaryInstr<MaxOp<T>, T, elementsPerRepeat, blockSizeElem, dstRowStride, src0RowStride, src1RowStride>(
            dstPtr, src0Ptr, src1Ptr, validRows, validCols);
    }
    return;
}
...
PTO_INTERNAL void TMaxCheck(const TileDataDst& dst, const TileDataSrc0& src0, const TileDataSrc1& src1)
{
    static_assert(
        std::is_same<T, typename TileDataSrc0::DType>::value && std::is_same<T, typename TileDataSrc1::DType>::value,
        "Fix: TMAX the data type of dst must be consistent with of src0 and src1.");
    static_assert(
...
PTO_INTERNAL void TMAX_IMPL(TileDataDst& dst, TileDataSrc0& src0, TileDataSrc1& src1)
{
    ...
    TMaxCheck<T, TileDataDst, TileDataSrc0, TileDataSrc1>(dst, src0, src1);
    ...
    if constexpr (std::is_same_v<TileDataDst, TileDataSrc0> && std::is_same_v<TileDataDst, TileDataSrc1>) {
        ...
        TMax<TileDataDst, elementsPerRepeat, blockSizeElem, dstRowStride>(
        ...
        TMax<TileDataDst, elementsPerRepeat, blockSizeElem, dstRowStride, src0RowStride, src1RowStride>(
            dst.data(), src0.data(), src1.data(), dst.GetValidRow(), dst.GetValidCol());
    }
}
} // namespace pto
