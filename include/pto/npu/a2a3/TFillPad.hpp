...
PTO_INTERNAL void Handle32BAlignedPad_Byte(
    __ubuf__ uint8_t* dstPtr, uint64_t srcValidRow, uint64_t srcValidCol, uint64_t /* srcValidCol32B */,
    uint8_t padValue)
{
...
template <typename TileDataDst, typename TileDataSrc>
PTO_INTERNAL void Handle32BAlignedPad_Other(
    decltype(getCopyNullPtr<TileDataDst>()) dstPtr, uint64_t srcValidRow, uint64_t srcValidCol, uint64_t srcValidCol32B,
...
template <typename TileDataDst, typename TileDataSrc>
PTO_INTERNAL void PadBottomRows(
    decltype(getCopyNullPtr<TileDataDst>()) dstPtr, uint64_t srcValidRow, uint64_t dstValidRow, uint64_t copyDstCols,
    uint64_t dupPadValue)
{
...
template <typename TileDataDst, typename TileDataSrc>
__tf__ PTO_INTERNAL void TFillPad(
    ...
    if constexpr (TileDataDst::PadVal != TileDataSrc::PadVal) {
        if constexpr (sizeof(T) == 1) {
            Handle32BAlignedPad_Byte<TileDataDst, TileDataSrc>(
                (__ubuf__ uint8_t*)dstPtr, srcValidRow, srcValidCol, srcValidCol32B, (uint8_t)(padValue & 0xff));
        } else {
...
template <typename TileDataDst, typename TileDataSrc>
__tf__ PTO_INTERNAL void TFillPad_Inplace(
    typename TileDataDst::TileDType __out__ dst, typename TileDataSrc::TileDType __in__ src, uint64_t dstValidRow,
    uint64_t dstValidCol, uint64_t srcValidRow, uint64_t srcValidCol)
{
    using T = typename TileDataSrc::DType;
...
PTO_INTERNAL void TFILLPAD_INPLACE_IMPL(TileDataDst& dst, TileDataSrc& src)
{
    ...
    TFillPad_Inplace<TileDataDst, TileDataSrc>(
...
{
    static_assert(
        TileData::PadVal == PadValue::Zero || TileData::PadVal == PadValue::Null,
