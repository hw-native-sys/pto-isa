...
__tf__ PTO_INTERNAL void TConcatImpl(
    typename TileDataD::TileDType __out__ dst, typename TileDataS0::TileDType __in__ src0,
    typename TileDataS1::TileDType __in__ src1, unsigned validRow, unsigned validCol0, unsigned validCol1)
{
...
PTO_INTERNAL void TCONCAT_IMPL(TileDataD& dst, TileDataS0& src0, TileDataS1& src1)
{
    ...
    static_assert(
        std::is_same<TD, TS0>::value && std::is_same<TD, TS1>::value,
        "TCONCAT: Data type of dst, src0 and src1 must be the same.");
    static_assert(
        std::is_same<TD, int32_t>::value || std::is_same<TD, int16_t>::value || std::is_same<TD, int8_t>::value ||
            std::is_same<TD, uint32_t>::value || std::is_same<TD, uint16_t>::value ||
            std::is_same<TD, uint8_t>::value || std::is_same<TD, half>::value || std::is_same<TD, float16_t>::value ||
    ...
    static_assert(
        TileDataD::ValidRow <= TileDataD::Rows && TileDataS0::ValidRow <= TileDataS0::Rows &&
    ...
    TConcatImpl<TileDataD, TileDataS0, TileDataS1>(
        dst.data(), src0.data(), src1.data(), validRow, validCol0, validCol1);
}
...
__tf__ PTO_INTERNAL void TConcatIdx(
    ...
    typename Src0IdxTile::TileDType __in__ idx0, typename Src1IdxTile::TileDType __in__ idx1, unsigned validRow,
...
PTO_INTERNAL void TConcatIdxCheck()
{
    ...
    static_assert(
        std::is_same<dataType, typename Src0Tile::DType>::value &&
            std::is_same<dataType, typename Src1Tile::DType>::value,
        "TCONCAT: Data type of dst, src0 and src1 must be the same.");
    static_assert(
        std::is_same<idxType, typename Src1IdxTile::DType>::value,
    ...
    static_assert(
        DstTile::Loc == TileType::Vec && Src0Tile::Loc == TileType::Vec && Src1Tile::Loc == TileType::Vec,
        "TCONCAT: TileType of src and dst tiles must be TileType::Vec.");
    static_assert(
        DstTile::ValidRow <= DstTile::Rows && Src0Tile::ValidRow <= Src0Tile::Rows &&
...
PTO_INTERNAL void TCONCAT_IMPL(DstTile& dst, Src0Tile& src0, Src1Tile& src1, Src0IdxTile& src0Idx, Src1IdxTile& src1Idx)
{
    ...
    TConcatIdx<
        DstTile, Src0Tile, Src1Tile, Src0IdxTile, Src0IdxTile, Src1IdxTile, elementsPerBlock, elementsPerRepeat,
    ...
    DstTile& dst, Src0Tile& src0, Src1Tile& src1, DstIdxTile& dstIdx, Src0IdxTile& src0Idx, Src1IdxTile& src1Idx)
{
    TConcatIdxCheck<DstTile, Src0Tile, Src1Tile, Src0IdxTile, Src1IdxTile>();
    static_assert(
        std::is_same<typename DstIdxTile::DType, typename Src0IdxTile::DType>::value,
    ...
    TConcatIdx<
        DstTile, Src0Tile, Src1Tile, DstIdxTile, Src0IdxTile, Src1IdxTile, elementsPerBlock, elementsPerRepeat,
