    ...
    uint32_t gmGap, uint32_t ubGap, uint32_t ubPad)
{
    using T = std::conditional_t<
        sizeof(typename TileDataDst::DType) == 1, int8_t,
    ...
    copy_gm_to_ubuf_align(
        reinterpret_cast<__ubuf__ T*>(dst), reinterpret_cast<__gm__ T*>(src), 0, nBurst, lenBurst, 0, ubPad, gmGap,
        ubGap);
}
    ...
    int gStride4, int dstShape0, int dstShape1, int dstShape2, int dstShape3)
{
    ...
    if constexpr (TileDataDst::totalDimCount == 4) {
        static_assert(
            TileDataDst::staticShape[2] == FRACTAL_NZ_ROW && TileDataDst::staticShape[3] == c0ElemCount,
            "Fix: The TileDataDst last 2 dim must be static and satisfy [16, 32 / sizeof(DataType)]");
        static_assert(
            GlobalDataSrc::staticShape[3] == FRACTAL_NZ_ROW && GlobalDataSrc::staticShape[4] == c0ElemCount,
...
PTO_INTERNAL void CheckConvTileData(TileDataDst& dst, GlobalDataSrc& src)
{
    ...
    static_assert(
        sizeof(typename TileDataDst::DType) == sizeof(typename GlobalDataSrc::DType),
