    ...
    uint32_t gmGap, uint32_t ubGap)
{
    using T = std::conditional_t<
        ...
        std::conditional_t<
            sizeof(typename TileData::DType) == 2, int16_t,
    ...
    for (uint16_t i = 0; i < chunks; i++) {
        ...
        copy_ubuf_to_gm_align(
            reinterpret_cast<__gm__ T*>(dst), reinterpret_cast<__ubuf__ T*>(src), 0, burstNums, lenBurst, ubGap, gmGap);
...
PTO_INTERNAL void TStoreAccNz2nz(
...
template <
    typename GlobalDataTile, typename TileData, QuantMode_t quantizationMode = QuantMode_t::NoQuant,
    ReluPreMode reluPreMode = ReluPreMode::NoRelu, STPhase Phase = STPhase::Unspecified>
__tf__ AICORE void TStoreAcc(
    typename GlobalDataTile::DType __out__* dst, typename TileData::TileDType __in__ src, int gShape0, int gShape1,
    int gShape2, int gShape3, int gShape4, int gStride0, int gStride1, int gStride2, int gStride3, int gStride4,
    int validRow, int validCol)
{
...
{
    static_assert(
        (GlobalData::layout == Layout::ND || GlobalData::layout == Layout::NZ ||
    ...
    static_assert(
        std::is_same_v<typename TileData::DType, int32_t> || std::is_same_v<typename TileData::DType, half>,
    ...
    static_assert(
        (GlobalData::layout == Layout::ND && TileData::Rows >= 1 && TileData::Rows <= 8192) ||
