...
namespace pto {
PTO_INTERNAL int CEIL(int a, int b) { return (a + (b - 1)) / (b); }
...
PTO_INTERNAL void CheckValid()
{
    ...
    static_assert(
        std::is_same_v<V, int16_t> || std::is_same_v<V, uint16_t> || std::is_same_v<V, int32_t> ||
...
PTO_INTERNAL void TGather(
    __ubuf__ typename TileDataD::DType* dst, __ubuf__ typename TileDataS0::DType* src0,
