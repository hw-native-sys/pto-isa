...
__tf__ AICORE void TMovToBt(typename DstTileData::TileDType __out__ dst, typename SrcTileData::TileDType __in__ src)
{
    ...
    static_assert(SrcTileData::Rows == 1, "Fix: TMov: When TileType is Bias, row must be 1.");
    static_assert(DstTileData::Cols % BIAS_TABLE_UNIT_ELEM == 0,
                  "Fix: TMov: When TileType is Bias, col must be aligned to 16.");
    static_assert(DstTileData::Cols * sizeof(DstType) <= BIAS_TABLE_SIZE,
    ...
    constexpr uint16_t burstLen = SrcTileData::Numel * sizeof(SrcType) / BLOCK_BYTE_SIZE;
...
__tf__ AICORE void TMovToFb(typename DstTileData::TileDType __out__ dst, typename SrcTileData::TileDType __in__ src)
{
    ...
    static_assert(DstTileData::Cols * sizeof(DstType) <= FIXPIPE_BUFFER_SIZE,
                  "TMov: The memory occupation of FbTile exceeds 7.0KB fixpipe buffer size.");

    __cbuf__ SrcType *srcP = (__cbuf__ SrcType *)(src);
    __fbuf__ DstType *dstP = (__fbuf__ DstType *)(dst);

    constexpr uint16_t burstLen = SrcTileData::Numel * sizeof(SrcType) / FIXP_BURST_UNIT_LEN;
                              ...
                              uint16_t validRow, uint16_t validCol)
{
    ...
    copy_matrix_cc_to_cbuf(dstAddr, srcData, 0, validCol, validRow, dstStride, SrcTileData::Rows, 0, 0, QuantPre,
                              ...
                              uint16_t validRow, uint16_t validCol)
{
    ...
    copy_matrix_cc_to_ub(dstAddr, srcData, 0, validCol, validRow, dstStride, SrcTileData::Rows, 0, 0, quantPre,
                         reluMode, channelSplitEnable, enableNz2Nd, 0, 0, false, false, 0, false, false, false, false,
                         false, enableNz2Dn);
}
...
PTO_INTERNAL constexpr void CommonCheck()
{
    ...
    using U = typename SrcTileData::DType;
    ...
    if constexpr (DstTileData::Loc == TileType::Right) {
        static_assert(std::is_same_v<T, half> || std::is_same_v<T, int8_t>,
                      "Fix: TMov: Unsupported data type! Supported types: int8_t, half");
...
AICORE void TMovToLeft(DstTileData &dst, SrcTileData &src)
{
    ...
    if constexpr (SrcTileData::Rows == 1 && SrcTileData::isRowMajor) {
        ...
        if constexpr (DstTileData::Compact == CompactMode::Normal) {
            TExtractToACompact<DstTileData, SrcTileData>(dst.data(), src.data(), 0, 0, dst.GetValidRow(),
                                                         dst.GetValidCol());
        } else {
        ...
        if constexpr (DstTileData::Compact == CompactMode::Normal || sizeof(typename SrcTileData::DType) == 1) {
            TExtractToATransCompact<DstTileData, SrcTileData>(dst.data(), src.data(), 0, 0, dst.GetValidRow(),
                                                              dst.GetValidCol());
        } else {
...
AICORE void TMovToRight(DstTileData &dst, SrcTileData &src)
{
    ...
    if constexpr (DstTileData::SFractal == SrcTileData::SFractal) {
        if constexpr (DstTileData::Compact == CompactMode::Normal) {
            TExtractToBCompact<DstTileData, SrcTileData>(dst.data(), src.data(), 0, 0, dst.GetValidRow(),
                                                         dst.GetValidCol());
        } else {
        ...
        if constexpr (DstTileData::Compact == CompactMode::Normal || sizeof(typename SrcTileData::DType) == 1) {
            TExtractToBTransCompact<DstTileData, SrcTileData>(dst.data(), src.data(), 0, 0, dst.GetValidRow(),
                                                              dst.GetValidCol());
        } else {
...
PTO_INTERNAL void TMOV_IMPL(DstTileData &dst, SrcTileData &src)
{
    if constexpr (SrcTileData::Loc == TileType::Mat) {
        ...
        if constexpr (DstTileData::Loc == TileType::Bias) {
            ...
            static_assert(sizeof(DstTileData::DType) == 0, "TMov: ScaleLeft tile type is not supported.");
        } else if constexpr (DstTileData::Loc == TileType::ScaleRight) {
            static_assert(sizeof(DstTileData::DType) == 0, "TMov: ScaleRight tile type is not supported.");
        }
