...
__tf__ PTO_INTERNAL void TMovToFb(typename DstTile::TileDType __out__ dst, typename SrcTile::TileDType __in__ src)
{
    ...
    static_assert(
        DstTile::Cols * sizeof(DstType) % FIXPIPE_BUFFER_UNIT == 0,
...
template <
    typename DstTile, typename SrcTile, AccToVecMode mode, QuantMode_t quantPre, ReluPreMode reluMode,
...
PTO_INTERNAL void TMOV_IMPL(DstTile& dst, SrcTile& src)
{
    if constexpr (SrcTile::Loc == TileType::Mat) {
        ...
        if constexpr (DstTile::Loc == TileType::Vec) {
            TMovCcToUb<DstTile, SrcTile, AccToVecMode::SingleModeVec0, quantPre, ReluPreMode::NoRelu>(
...
PTO_INTERNAL void TMOV_IMPL(DstTile& dst, SrcTile& src)
{
    ...
    constexpr QuantMode_t quantPre = GetCastPreQuantMode<typename SrcTile::DType, typename DstTile::DType>();
    if constexpr (DstTile::Loc == TileType::Vec) {
        TMovCcToUb<DstTile, SrcTile, AccToVecMode::SingleModeVec0, quantPre, reluMode, Phase>(
            dst.data(), src.data(), m, n);
    } else if constexpr (DstTile::Loc == TileType::Mat) {
...
PTO_INTERNAL void TMOV_IMPL(DstTile& dst, SrcTile& src)
{
    ...
    uint16_t n = src.GetValidCol();
    constexpr QuantMode_t quantPre = GetCastPreQuantMode<typename SrcTile::DType, typename DstTile::DType>();
    TMovCcToUb<DstTile, SrcTile, mode, quantPre, reluMode, Phase>(dst.data(), src.data(), m, n);
}
