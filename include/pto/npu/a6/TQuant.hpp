...
#ifndef TQUANT_A6_HPP
#define TQUANT_A6_HPP
...
#include <pto/npu/common/TQuantShared.hpp>
#include <type_traits>
...
// Single validation point for HiF4 type constraints. HiF4 is BF16 -> float4_e1m2x2
// (two 4-bit e1m2 codes packed per 8-bit storage element). Called from the
// TQUANT_IMPL HiF4 branch; no other HiF4 function repeats these static_asserts.
template <typename T, typename OutT>
PTO_INTERNAL constexpr void CheckTQuantHif4Types()
{
    static_assert(std::is_same<T, bfloat16_t>::value, "Fix: HiF4 input has to be bfloat16");
    static_assert(std::is_same<OutT, float4_e1m2x2_t>::value, "Fix: HiF4 output has to be float4_e1m2x2_t");
}
...
// ============================================================================
// HiF4 (HiFloat4) BF16 -> HiF4 algorithm primitives.
//
// Three hierarchical per-group abs-max levels are reduced from the BF16 source
// (per-4, per-8, per-64), then turned into three per-level exponents Ea/Eb/Ec
// and a per-4 scaling factor, and finally scaled + cast to 4-bit HiF4 codes.
//
// Source layout has two cases, routed in TQuant_Hif4_Impl:
//   * CONTINUOUS  — validCols == srcCols; the source tile is a flat 1D buffer.
//   * STRIDED     — validCols != srcCols; rows are padded to srcCols in UB and
//                   the reducer honors the row stride.
// Each reduce stage therefore has a _Cont and a _Strided variant.
//
// Output exponent buffers Ea/Eb/Ec live INSIDE the single `exp` tile, carved
// out by TQuant_Hif4_Impl as three 2D [validRows, rowStride] sub-regions:
//   Ea (per-64):  cols = DivRoundup(validCols, 64), rowStride padded to 16 B
//   Eb (per-8):   cols = DivRoundup(validCols, 8),  rowStride padded to 16 B
//   Ec (per-4):   cols = DivRoundup(validCols, 4),  rowStride padded to 32 B
// Each sub-region base is 32-B aligned (for normal VLD loads) and the whole
// `exp` region is zero-filled up front so padding bytes are zero.
//
// `max` is intermediate scratch, also carved into per-4/per-8/per-64 2D
// sub-regions (32-B aligned each) by TQuant_Hif4_Impl.
// ============================================================================

// Zero-fill an arbitrary contiguous UB element range with zeros. Caller must
// be inside a __VEC_SCOPE__. Models the vdup+vsts pattern from
// ZeroPadColumns_VLAligned (no general A6 zero-fill helper exists). T must be a
// 16/32-bit type supported by vdup + NORM_B16 (bfloat16_t/half). Used to
// pre-zero the `exp` workspace before carving Ea/Eb/Ec into it — the caller
// casts the byte pointer to bfloat16_t* (the region is always 16-B-aligned).
template <typename T>
PTO_INTERNAL void ZeroFillUB(__ubuf__ T* ptr, uint32_t totalElems)
{
    constexpr uint32_t elemPerVL = REPEAT_BYTE / sizeof(T);
    uint16_t vlCount = (uint16_t)CeilDivision((int32_t)totalElems, (int32_t)elemPerVL);
    uint32_t rem = totalElems;
    __VEC_SCOPE__
    {
        MaskReg pgAll = TQuantPSetTyped<T>(PAT_ALL);
        RegTensor<T> vregZero;
        vbr(vregZero, (T)0);
        for (uint16_t vi = 0; vi < vlCount; ++vi) {
            MaskReg preg = CreatePredicate<T>(rem);
            vsts(vregZero, ptr, (uint32_t)vi * elemPerVL, NORM_B16, preg);
        }
    }
}

// Stage 1a (continuous): per-4 abs-max from a flat contiguous BF16 source.
//   Writes one abs-max per 4-element group into maxGp4Ptr, laid out 2D as
//   [validRows, DivRoundup(validCols, 4)].
PTO_INTERNAL void AbsReduceMax_gp4_Cont(
    __ubuf__ bfloat16_t* srcPtr, __ubuf__ bfloat16_t* maxGp4Ptr, uint32_t validRows, uint32_t validCols)
{
    uint32_t total_elem_count = validRows * validCols;
    constexpr uint32_t num_elem_per_loop = 512; // each loop 512 elm -> 128 max gp4
    uint32_t full_loops_num = total_elem_count / num_elem_per_loop;
    uint32_t num_elem_tail = total_elem_count % num_elem_per_loop;
    uint32_t num_loops_tail = CeilDivision(num_elem_tail, num_elem_per_loop); // 0 or 1: tail < 512
    vector_bf16 vb16_src_vl0, vb16_src_vl1, vb16_src_vl2, vb16_src_vl3;
    vector_bf16 vb16_max, vb16_max_vl0, vb16_max_vl2;
    MaskReg pgAll = TQuantPSetTyped<bfloat16_t>(PAT_ALL);
    for (uint32_t loop_idx = 0; loop_idx < full_loops_num; ++loop_idx) {
        vlds(vb16_src_vl0, vb16_src_vl1, srcPtr, loop_idx * num_elem_per_loop, DINTLV_B32);
        vabs((vector_f16&)vb16_src_vl0, (vector_f16&)vb16_src_vl0, pgAll, MODE_ZEROING);
        vabs((vector_f16&)vb16_src_vl1, (vector_f16&)vb16_src_vl1, pgAll, MODE_ZEROING);
        vlds(vb16_src_vl2, vb16_src_vl3, srcPtr, loop_idx * num_elem_per_loop + 256, DINTLV_B32);
        vabs((vector_f16&)vb16_src_vl2, (vector_f16&)vb16_src_vl2, pgAll, MODE_ZEROING);
        vabs((vector_f16&)vb16_src_vl3, (vector_f16&)vb16_src_vl3, pgAll, MODE_ZEROING);
        vmax(vb16_max_vl0, vb16_src_vl0, vb16_src_vl1, pgAll, MODE_ZEROING);
        vmax(vb16_max_vl2, vb16_src_vl2, vb16_src_vl3, pgAll, MODE_ZEROING);
        vdintlv(vb16_max_vl0, vb16_max_vl2, vb16_max_vl0, vb16_max_vl2); // handled as 2*vdintlvv2 in A6
        vmax(vb16_max, vb16_max_vl0, vb16_max_vl2, pgAll, MODE_ZEROING);
        vsts(vb16_max, maxGp4Ptr, loop_idx * 128, NORM_B16, pgAll);
    }
    // This loop shall only run once, or none at all
    for (uint32_t tail_loop_idx = 0; tail_loop_idx < num_loops_tail; ++tail_loop_idx) {
        // save all the scalar calculations inside loop to prevent running them if not needed
        uint32_t rem = num_elem_tail;
        uint32_t rem_st = CeilDivision(rem, 4);
        uint32_t rem_max_0 = CeilDivision(rem, 2);
        uint32_t rem_max_1 = (rem > 256) ? CeilDivision(rem - 256, 2) : 0;
        vector_bool preg_vl_0 = CreatePredicate<bfloat16_t>(rem_max_0);
        vector_bool preg_vl_1 = CreatePredicate<bfloat16_t>(rem_max_1);
        vector_bool preg_st = CreatePredicate<bfloat16_t>(rem_st);
        vlds(vb16_src_vl0, vb16_src_vl1, srcPtr, (full_loops_num + tail_loop_idx) * num_elem_per_loop, DINTLV_B32);
        vabs((vector_f16&)vb16_src_vl0, (vector_f16&)vb16_src_vl0, pgAll, MODE_ZEROING);
        vabs((vector_f16&)vb16_src_vl1, (vector_f16&)vb16_src_vl1, pgAll, MODE_ZEROING);
        vlds(
            vb16_src_vl2, vb16_src_vl3, srcPtr, (full_loops_num + tail_loop_idx) * num_elem_per_loop + 256, DINTLV_B32);
        vabs((vector_f16&)vb16_src_vl2, (vector_f16&)vb16_src_vl2, pgAll, MODE_ZEROING);
        vabs((vector_f16&)vb16_src_vl3, (vector_f16&)vb16_src_vl3, pgAll, MODE_ZEROING);
        vmax(vb16_max_vl0, vb16_src_vl0, vb16_src_vl1, preg_vl_0, MODE_ZEROING);
        vmax(vb16_max_vl2, vb16_src_vl2, vb16_src_vl3, preg_vl_1, MODE_ZEROING);
        vdintlv(vb16_max_vl0, vb16_max_vl2, vb16_max_vl0, vb16_max_vl2);
        vmax(vb16_max, vb16_max_vl0, vb16_max_vl2, preg_st, MODE_ZEROING);
        vsts(vb16_max, maxGp4Ptr, (full_loops_num + tail_loop_idx) * 128, NORM_B16, preg_st);
    }
}

// Stage 1a (strided): per-4 abs-max from a 2D BF16 source whose rows are padded
//   to srcCols. Reads row r at offset r * srcCols; writes per-4 abs-max into
//   maxGp4Ptr laid out as [validRows, DivRoundup(validCols, 4)].
PTO_INTERNAL void AbsReduceMax_gp4_Strided(
    __ubuf__ bfloat16_t* srcPtr, __ubuf__ bfloat16_t* maxGp4Ptr, uint32_t validRows, uint32_t validCols,
    uint32_t srcCols)
{
    constexpr uint32_t num_elem_per_loop = 256;    // each loop 256 elm -> 64 max gp4
    constexpr uint32_t num_elem_out_per_loop = 64; // each loop 256 elm -> 64 max gp4
    uint32_t aligned_row_size_elem = CeilDivision(srcCols, 16);
    ...
    for (uint16_t row_idx = 0; row_idx < rowLimit; ++row_idx) {
        ...
        for (uint16_t loop_idx = 0; loop_idx < loopLimit; ++loop_idx) {
            ...
            vabs((vector_f16&)vb16_src_vl1, (vector_f16&)vb16_src_vl1, pregDyn, MODE_ZEROING);
            vmax(vb16_max, vb16_src_vl0, vb16_src_vl1, pregDyn, MODE_ZEROING);
            vdintlvv2(vb16_max_dintlv_low, vb16_max, vb16_max, LOWER);
            vdintlvv2(vb16_max_dintlv_high, vb16_max, vb16_max, HIGHER);
            vmax(vb16_max, vb16_max_dintlv_low, vb16_max_dintlv_high, pregDyn, MODE_ZEROING);
            vsts(
                vb16_max, maxGp4Ptr, row_idx * aligned_row_size_elem + loop_idx * num_elem_out_per_loop, NORM_B16,
                pregDyn);
        }
    }
}

// Stage 1b (continuous): reduce per-4 max into per-8 and per-64 maxima. All
//   three max buffers are 2D [validRows, DivRoundup(validCols, k)] and live in
//   the `max` scratch tile.
PTO_INTERNAL void AbsReduceMax_gp8_gp64_Cont_Body(
    __ubuf__ bfloat16_t* maxGp4Ptr, __ubuf__ bfloat16_t* maxGp8Ptr, __ubuf__ uint8_t*& eaPtr, uint32_t num_full_loops,
    MaskReg pgAll, MaskReg pregDyn_Low8, vector_align& ureg_max)
{
    constexpr uint32_t num_elem_in = 256;
    constexpr uint32_t num_elem_out_gp8 = 128;
    constexpr bfloat16_t recp_7 = 0.1428571428571429f;
    vector_bf16 vb16_max_gp4, vb16_max_low, vb16_max_high, vb16_max_gp4_unroll;
    vector_bf16 vb16_max_gp8, vb16_max_gp64, vb16_max_gp64_unroll;
    vector_u8 vb8_Ea, vb8_Ea_unroll;
    for (uint32_t loop_idx = 0; loop_idx < num_full_loops; ++loop_idx) {
        vlds(vb16_max_low, vb16_max_high, maxGp4Ptr, loop_idx * num_elem_in, DINTLV_B16);
        vlds(vb16_max_gp4, maxGp4Ptr, loop_idx * num_elem_in, NORM);
        vlds(vb16_max_gp4_unroll, maxGp4Ptr, loop_idx * num_elem_in + 128, NORM);
        vmax(vb16_max_gp8, vb16_max_low, vb16_max_high, pgAll, MODE_ZEROING);
        vsts(vb16_max_gp8, maxGp8Ptr, loop_idx * num_elem_out_gp8, NORM_B16, pgAll);
        vcgmax((vector_f16&)vb16_max_gp64, (vector_f16&)vb16_max_gp4, pgAll);
        vcgmax((vector_f16&)vb16_max_gp64_unroll, (vector_f16&)vb16_max_gp4_unroll, pgAll);
        vmuls(vb16_max_gp64, vb16_max_gp64, recp_7, pregDyn_Low8, MODE_ZEROING);
        vmuls(vb16_max_gp64_unroll, vb16_max_gp64_unroll, recp_7, pregDyn_Low8, MODE_ZEROING);
        vcvt_bf162e6m2((vector_f8e6m2&)vb8_Ea, (vector_bf16&)vb16_max_gp64, pgAll, ROUND_R, PART_EVEN);
        vcvt_bf162e6m2((vector_f8e6m2&)vb8_Ea_unroll, (vector_bf16&)vb16_max_gp64_unroll, pgAll, ROUND_R, PART_EVEN);
        vstus(ureg_max, 8, (vector_bf16&)vb8_Ea, (__ubuf__ bfloat16_t*&)eaPtr, POST_UPDATE);
        vstus(ureg_max, 8, (vector_bf16&)vb8_Ea_unroll, (__ubuf__ bfloat16_t*&)eaPtr, POST_UPDATE);
        vstas(ureg_max, (__ubuf__ bfloat16_t*&)eaPtr, 0, POST_UPDATE);
    }
}

PTO_INTERNAL void AbsReduceMax_gp8_gp64_Cont_Tail(
    __ubuf__ bfloat16_t* maxGp4Ptr, __ubuf__ bfloat16_t* maxGp8Ptr, __ubuf__ uint8_t*& eaPtr, uint32_t num_full_loops,
    uint32_t tail_elem_count, MaskReg pregDyn_Low8, vector_align& ureg_max)
{
    constexpr uint32_t num_elem_in = 256;
    constexpr uint32_t num_elem_out_gp8 = 128;
    constexpr bfloat16_t recp_7 = 0.1428571428571429f;
    ...
    vector_bf16 vb16_max_gp8, vb16_max_gp64, vb16_max_gp64_unroll;
    vector_u8 vb8_Ea, vb8_Ea_unroll;
    uint32_t num_loops_tail = CeilDivision(tail_elem_count, 1024);
    for (uint32_t tail_idx = 0; tail_idx < num_loops_tail; ++tail_idx) {
        uint32_t rem_gps = CeilDivision(tail_elem_count, 64);
        uint32_t gp8_cnt = tail_elem_count / 2;
        MaskReg preg_gp8 = CreatePredicate<bfloat16_t>(gp8_cnt);
        MaskReg pregDyn_vl0 = CreatePredicate<bfloat16_t>(tail_elem_count);
        MaskReg pregDyn_vl1 = CreatePredicate<bfloat16_t>(tail_elem_count);
        uint32_t gp64_st0_cnt = rem_gps > 8 ? 8 : rem_gps;
        uint32_t gp64_st1_cnt = (rem_gps - gp64_st0_cnt) > 0 ? (rem_gps - gp64_st0_cnt) : 0;
        vlds(vb16_max_gp4, maxGp4Ptr, (num_full_loops + tail_idx) * num_elem_in, NORM);
        vlds(vb16_max_gp4_unroll, maxGp4Ptr, (num_full_loops + tail_idx) * num_elem_in + 128, NORM);
        vdintlvv2(vb16_max_low, vb16_max_gp4, vb16_max_gp4_unroll, LOWER);
        vdintlvv2(vb16_max_high, vb16_max_gp4, vb16_max_gp4_unroll, HIGHER);
        vmax(vb16_max_gp8, vb16_max_low, vb16_max_high, preg_gp8, MODE_ZEROING);
        vsts(vb16_max_gp8, maxGp8Ptr, (num_full_loops + tail_idx) * num_elem_out_gp8, NORM_B16, preg_gp8);
        vcgmax((vector_f16&)vb16_max_gp64, (vector_f16&)vb16_max_gp4, pregDyn_vl0);
        vcgmax((vector_f16&)vb16_max_gp64_unroll, (vector_f16&)vb16_max_gp4, pregDyn_vl1);
        vmuls(vb16_max_gp64, vb16_max_gp64, recp_7, pregDyn_vl0, MODE_ZEROING);
        vmuls(vb16_max_gp64_unroll, vb16_max_gp64_unroll, recp_7, pregDyn_vl1, MODE_ZEROING);
        vcvt_bf162e6m2((vector_f8e6m2&)vb8_Ea, (vector_bf16&)vb16_max_gp64, pregDyn_vl0, ROUND_R, PART_EVEN);
        vcvt_bf162e6m2(
            (vector_f8e6m2&)vb8_Ea_unroll, (vector_bf16&)vb16_max_gp64_unroll, pregDyn_vl1, ROUND_R, PART_EVEN);
        vstus(ureg_max, gp64_st0_cnt, (vector_bf16&)vb8_Ea, (__ubuf__ bfloat16_t*&)eaPtr, POST_UPDATE);
        vstus(ureg_max, gp64_st1_cnt, (vector_bf16&)vb8_Ea_unroll, (__ubuf__ bfloat16_t*&)eaPtr, POST_UPDATE);
        vstas(ureg_max, (__ubuf__ bfloat16_t*&)eaPtr, 0);
    }
}

PTO_INTERNAL void AbsReduceMax_gp8_gp64_Cont(
    __ubuf__ bfloat16_t* maxGp4Ptr, __ubuf__ bfloat16_t* maxGp8Ptr, __ubuf__ uint8_t* eaPtr, uint32_t validRows,
    uint32_t validCols)
{
    uint32_t total_elem_count = validRows * validCols;
    uint32_t num_full_loops = total_elem_count / 1024;
    uint32_t tail_elem_count = total_elem_count % 1024;
    MaskReg pgAll = TQuantPSetTyped<bfloat16_t>(PAT_ALL);
    MaskReg pregDyn_Low8 = pset_b16(PAT_VL8);
    vector_align ureg_max;
    AbsReduceMax_gp8_gp64_Cont_Body(maxGp4Ptr, maxGp8Ptr, eaPtr, num_full_loops, pgAll, pregDyn_Low8, ureg_max);
    AbsReduceMax_gp8_gp64_Cont_Tail(
        maxGp4Ptr, maxGp8Ptr, eaPtr, num_full_loops, tail_elem_count, pregDyn_Low8, ureg_max);
}

// Stage 1b (strided): same reduction but per-level max buffers carry their own
//   per-row group stride (gp4GroupsPerRow etc.) since validCols != srcCols.
PTO_INTERNAL void AbsReduceMax_gp8_gp64_Strided(
    __ubuf__ bfloat16_t* maxGp4Ptr, __ubuf__ bfloat16_t* maxGp8Ptr, __ubuf__ bfloat16_t* maxGp64Ptr, uint32_t validRows,
    uint32_t validCols, uint32_t gp4StaticCols, uint32_t gp8StaticCols, uint32_t gp64StaticCols)
{
    vector_bf16 vb16_max_gp4, vb16_max_low, vb16_max_high, vb16_max_gp4_unroll;
    vector_bf16 vb16_max_gp8, vb16_max_gp64, vb16_max_gp64_unroll;
    vector_f8e6m2 vb8_Ea, vb8_Ea_unroll;
    constexpr uint32_t num_elem_in = 256;
    constexpr bfloat16_t recp_7 = 0.1428571428571429f; // 1/7 for max gp64 scaling
    uint32_t gps64_per_row = CeilDivision(validCols, 64);
    vector_align ureg_max;
    uint32_t num_loops_per_row =
        CeilDivision(validCols, num_elem_in); // expectation: for usual tile size will be around 1~2
    for (uint32_t row_idx = 0; row_idx < validRows; ++row_idx) {
        uint32_t rem_cols = validCols;
        uint32_t rem_cols_gp8 = rem_cols / 2;
        uint32_t rem_grp64 = gps64_per_row;
        MaskReg pregDyn_Low8 = pset_b16(PAT_VL8);
        for (uint32_t inner_loop_idx = 0; inner_loop_idx < num_loops_per_row; ++inner_loop_idx) {
            MaskReg pregDyn_gp8 = CreatePredicate<bfloat16_t>(rem_cols_gp8);
            MaskReg pregDyn_vl0 = CreatePredicate<bfloat16_t>(rem_cols);
            MaskReg pregDyn_vl1 = CreatePredicate<bfloat16_t>(rem_cols);
            uint32_t gp64_st0_cnt = rem_grp64 > 8 ? 8 : gps64_per_row;
            uint32_t gp64_st1_cnt = (rem_grp64 - gp64_st0_cnt) > 0 ? (gps64_per_row - gp64_st0_cnt) : 0;
            vlds(
                vb16_max_low, vb16_max_high, maxGp4Ptr, row_idx * gp4StaticCols + inner_loop_idx * num_elem_in,
                DINTLV_B16); // Cheaper to have 4 lds, than 2 dintlvv2
            vlds(vb16_max_gp4, maxGp4Ptr, row_idx * gp4StaticCols + inner_loop_idx * num_elem_in, NORM);
            vlds(vb16_max_gp4_unroll, maxGp4Ptr, row_idx * gp4StaticCols + inner_loop_idx * num_elem_in + 128, NORM);
            vmax(vb16_max_gp8, vb16_max_low, vb16_max_high, pregDyn_gp8, MODE_ZEROING);
            vsts(
                vb16_max_gp8, maxGp8Ptr, row_idx * gp8StaticCols + inner_loop_idx * num_elem_in, NORM_B16, pregDyn_gp8);
            vcgmax((vector_f16&)vb16_max_gp64, (vector_f16&)vb16_max_gp4, pregDyn_vl0);
            vcgmax((vector_f16&)vb16_max_gp64_unroll, (vector_f16&)vb16_max_gp4_unroll, pregDyn_vl1);
            vmuls(vb16_max_gp64, vb16_max_gp64, recp_7, pregDyn_Low8, MODE_ZEROING);
            vmuls(vb16_max_gp64_unroll, vb16_max_gp64_unroll, recp_7, pregDyn_Low8, MODE_ZEROING);
            vcvt_bf162e6m2(vb8_Ea, vb16_max_gp64, pregDyn_Low8, ROUND_R, PART_EVEN);
            vcvt_bf162e6m2(vb8_Ea_unroll, vb16_max_gp64_unroll, pregDyn_Low8, ROUND_R, PART_EVEN);
            vstus(
                ureg_max, gp64_st0_cnt, (vector_u8&)vb8_Ea,
                (__ubuf__ uint8_t*)maxGp64Ptr + row_idx * gp64StaticCols + inner_loop_idx * 16);
            vstus(
                ureg_max, gp64_st1_cnt, (vector_u8&)vb8_Ea_unroll,
                (__ubuf__ uint8_t*)maxGp64Ptr + row_idx * gp64StaticCols + inner_loop_idx * 16 + gp64_st0_cnt);
            vstas(ureg_max, (__ubuf__ uint8_t*)maxGp64Ptr + row_idx * gp64StaticCols + inner_loop_idx * 16, 0);
            rem_grp64 -= 16;
        }
    }
}

// Stage 2 (collapsed exponent calc + merge) — continuous: read the three
//   per-level max scratch buffers, derive per-level shared exponents, and write
//   them into the pre-zeroed Ea (per-64) / Eb (per-8) / Ec (per-4) sub-regions
//   of the `exp` tile and the per-4 scaling factors into scalingPtr. Source max
//   buffers are flat-contiguous [validRows*validCols].
PTO_INTERNAL void CalcExpScale_Cont(
    __ubuf__ bfloat16_t* maxGp4Ptr, __ubuf__ bfloat16_t* maxGp8Ptr, __ubuf__ bfloat16_t* maxGp64Ptr,
    __ubuf__ uint8_t* eaPtr, __ubuf__ uint8_t* ebPtr, __ubuf__ uint8_t* ecPtr, __ubuf__ bfloat16_t* scalingPtr,
    uint32_t validRows, uint32_t validCols)
{
    vector_bf16 vb16_half, vb16_one, vb16_Ea_rec;
    vector_bf16 vb16_Ea, vb16_Mb, vb16_Mc; // max per 64, 8, 4 respectively
    vector_u8 vb8_Ea;
    ...
    for (uint16_t loop_idx = 0; loop_idx < loopLimit; ++loop_idx) {
        ...
        vmul(vb16_Ec_tmp_1, vb16_Ec_tmp_0, vb16_Eb_rec, pgAll); // Vmax16 * Ea_rec_bf16 * 2^-(Ebi)
        vcmps_ge(p_Ec, vb16_Ec_tmp_1, 2, pgAll);                // Eci = (Vmax16 * Ea_rec_bf16 * 2^-(Ebi)) > 2 ? 1:0
        psts(p_Ec, (__ubuf__ uint32_t*&)ecPtr, 16, PK, POST_UPDATE); // storing Ec predicates

        vsel(vb16_Ec_rec, vb16_half, vb16_one, p_Ec);         // 2^-(Eci): for preg 0->1, for preg 1->0.5
        vmul(vb16_Ebc_rec, vb16_Eb_rec, vb16_Ec_rec, pgAll);  // 2^-(Ebi + Eci): 2^(-Ebi) * 2^(-Eci)
        vmul(vb16_e_scale, vb16_Ea_rec, vb16_Ebc_rec, pgAll); // X_REC = Ea_REC * 2^-(Ebi + Eci)
        // Need to repeat inverse scaling 4 times (2 at store, 2 at load using US_B16)
        vsts(
            (vector_u16&)vb16_e_scale, (vector_u16&)vb16_e_scale, (__ubuf__ uint16_t*)scalingPtr, 256 * loop_idx,
            INTLV_B16, pgAll);
    }
}

// Stage 2 (collapsed exponent calc + merge) — strided: same as _Cont but each
//   per-level max buffer is 2D [validRows, gp<k>StaticCols] with 32-B-aligned
//   row starts.
PTO_INTERNAL void CalcExpScale_Strided(
    __ubuf__ bfloat16_t* maxGp4Ptr, __ubuf__ bfloat16_t* maxGp8Ptr, __ubuf__ bfloat16_t* maxGp64Ptr,
    __ubuf__ uint8_t* eaPtr, __ubuf__ uint8_t* ebPtr, __ubuf__ uint8_t* ecPtr, __ubuf__ bfloat16_t* scalingPtr,
    uint32_t validRows, uint32_t validCols, uint32_t gp4StaticCols, uint32_t gp8StaticCols, uint32_t gp64StaticCols,
    uint32_t numGp4, uint32_t numGp8, uint32_t numGp64, uint32_t numBlocks)
{
    (void)maxGp4Ptr;
    (void)maxGp8Ptr;
    (void)maxGp64Ptr;
...
// Stage 2b (exponent layout for Cube) — continuous: rearrange Ea/Eb/Ec from
//   their per-level 2D layout into the contiguous packed layout the Cube matmul
//   unit expects. Input exponents are flat-contiguous.
PTO_INTERNAL void ExpLayoutForCube_Cont(
    __ubuf__ uint8_t* eaPtr, __ubuf__ uint8_t* ebPtr, __ubuf__ uint8_t* ecPtr, __ubuf__ uint8_t* expDstPtr,
    uint32_t validRows, uint32_t validCols)
{
    uint32_t input_size = validRows * validCols / 64;  // size of Ea and Eb in bytes
    uint32_t loop_num = CeilDivision(input_size, 128); // processing 128B of each Ea and Eb per loop, not full VL
    uint32_t rem = input_size * 2;                     // interleaved Ea/Eb bytes = 2× Ea size
    vector_u8 vb8_Ea, vb8_Eb, vb8_EaEb, vb8_EaEb_hi, vb8_Ec;
    MaskReg pgAll = pset_b8(PAT_ALL);
    ...
    for (uint16_t loop_idx = 0; loop_idx < loopLimit; ++loop_idx) {
        ...
        vsstb(vb8_Ec, expDstPtr + (uint32_t)loop_idx * 512 + 32, (int32_t)cfgStride, pregDyn);
    }
}

// Stage 2b (exponent layout for Cube) — strided: same but each exponent
//   sub-region is accessed with its per-row element count (static cols).
PTO_INTERNAL void ExpLayoutForCube_Strided(
    __ubuf__ uint8_t* eaPtr, __ubuf__ uint8_t* ebPtr, __ubuf__ uint8_t* ecPtr, __ubuf__ uint8_t* expDstPtr,
    uint32_t validRows, uint32_t validCols, uint32_t gp64StaticCols, uint32_t gp8StaticCols, uint32_t gp4StaticCols)
{
    (void)eaPtr;
    (void)ebPtr;
    (void)ecPtr;
...
// Stage 3 — continuous: apply the per-4 scaling to the BF16 source and cast
//   each element to its 4-bit HiF4 code, packing two codes per storage element
//   into dstPtr. Source is flat-contiguous.
PTO_INTERNAL void CalcFp4Values_Cont(
    __ubuf__ bfloat16_t* srcPtr, __ubuf__ bfloat16_t* scalingPtr, __ubuf__ uint8_t* dstPtr, uint32_t validRows,
    uint32_t validCols)
{
    uint32_t total_elem_count = validRows * validCols;
    uint32_t loop_count = CeilDivision(total_elem_count, 128); // processing 128 elem per loop
    ...
    for (uint16_t loop_idx = 0; loop_idx < loopLimit; ++loop_idx) {
        ...
        vcvt((vector_f4e1m2x2&)vb8_fp4, vb16_input, pgAll, ROUND_R, PART_P0);
        vsts((vector_u8&)vb8_fp4, dstPtr, 64 * loop_idx, PK4_B32, pgAll);
    }
}

// Stage 3 — strided: same as _Cont but source is 2D [validRows, srcCols] with
//   each row read at offset r * srcCols.
PTO_INTERNAL void CalcFp4Values_Strided(
    __ubuf__ bfloat16_t* srcPtr, __ubuf__ bfloat16_t* scalingPtr, __ubuf__ uint8_t* dstPtr, uint32_t validRows,
    uint32_t validCols, uint32_t srcCols)
{
    (void)srcPtr;
    (void)scalingPtr;
    (void)dstPtr;
...
// Byte-level ceil-alignment: rounds `bytes` up to the next multiple of `alignBytes`.
// Uses CeilDivision to avoid division-by-zero (CeilDivision handles alignBytes==0 safely).
PTO_INTERNAL uint32_t AlignUpBytes(uint32_t bytes, uint32_t alignBytes)
{
    return CeilDivision((int32_t)bytes, (int32_t)alignBytes) * alignBytes;
}

// HiF4 ND pipeline assembler (BF16 -> HiFloat4). The caller passes the five
// tiles; this function:
//   1. zero-fills the entire `exp` workspace,
//   2. carves Ea/Eb/Ec (2D, row-padded 16/16/32 B, 32-B-aligned bases) out of
//      `exp` and per-4/8/64 max scratch (32-B-aligned) out of `max`,
//   3. routes to _Cont vs _Strided reduce variants on `validCols == srcCols`,
//   4. runs the four stages with mem_bar(VST_VLD) between reducers.
// Grouping is over axis 1 (ND). The scalar pointer arithmetic runs on the
// main Scalar unit; only ZeroFillUB and the pipeline (stages 1-3) enter
// __VEC_SCOPE__.
PTO_INTERNAL void TQuant_Hif4_Cont(
    __ubuf__ bfloat16_t* srcPtr, __ubuf__ uint8_t* expPtr, __ubuf__ uint8_t* dstPtr, __ubuf__ bfloat16_t* maxPtr,
    __ubuf__ bfloat16_t* scalingPtr, __ubuf__ uint8_t* eaPtr, __ubuf__ uint8_t* ebPtr, __ubuf__ uint8_t* ecPtr,
    __ubuf__ bfloat16_t* maxGp4Ptr, __ubuf__ bfloat16_t* maxGp8Ptr, __ubuf__ bfloat16_t* maxGp64Ptr,
    uint32_t eaDataBytes, uint32_t ebDataBytes, uint32_t ecDataBytes, uint32_t validRows, uint32_t validCols)
{
    __VEC_SCOPE__
    {
        AbsReduceMax_gp4_Cont(srcPtr, maxGp4Ptr, validRows, validCols);
        mem_bar(VST_VLD);
        __ubuf__ uint8_t* eaPtrSave = eaPtr;
        AbsReduceMax_gp8_gp64_Cont(maxGp4Ptr, maxGp8Ptr, eaPtr, validRows, validCols);
        eaPtr = eaPtrSave;
        mem_bar(VST_VLD);
        eaPtrSave = eaPtr;
        __ubuf__ uint8_t* ebPtrSave = ebPtr;
        __ubuf__ uint8_t* ecPtrSave = ecPtr;
        __ubuf__ bfloat16_t* maxGp4Save = maxGp4Ptr;
        __ubuf__ bfloat16_t* maxGp8Save = maxGp8Ptr;
        __ubuf__ bfloat16_t* scalingSave = scalingPtr;
        CalcExpScale_Cont(maxGp4Ptr, maxGp8Ptr, maxGp64Ptr, eaPtr, ebPtr, ecPtr, scalingPtr, validRows, validCols);
        eaPtr = eaPtrSave;
        ebPtr = ebPtrSave;
        ecPtr = ecPtrSave;
        maxGp4Ptr = maxGp4Save;
        maxGp8Ptr = maxGp8Save;
        scalingPtr = scalingSave;
        mem_bar(VST_VLD);
        ExpLayoutForCube_Cont(
            eaPtr, ebPtr, ecPtr, expPtr + eaDataBytes + ebDataBytes + ecDataBytes, validRows, validCols);
        mem_bar(VST_VLD);
        CalcFp4Values_Cont(srcPtr, scalingPtr, dstPtr, validRows, validCols);
    }
}

PTO_INTERNAL void TQuant_Hif4_Strided(
    __ubuf__ bfloat16_t* srcPtr, __ubuf__ uint8_t* expPtr, __ubuf__ uint8_t* dstPtr, __ubuf__ bfloat16_t* maxPtr,
    __ubuf__ bfloat16_t* scalingPtr, __ubuf__ uint8_t* eaPtr, __ubuf__ uint8_t* ebPtr, __ubuf__ uint8_t* ecPtr,
    __ubuf__ bfloat16_t* maxGp4Ptr, __ubuf__ bfloat16_t* maxGp8Ptr, __ubuf__ bfloat16_t* maxGp64Ptr,
    uint32_t expTotalBytes, uint32_t gp4StaticCols, uint32_t gp8StaticCols, uint32_t gp64StaticCols, uint32_t numGp4,
    uint32_t numGp8, uint32_t numGp64, uint32_t numBlocks, uint32_t validRows, uint32_t validCols, uint32_t srcCols)
{
    __VEC_SCOPE__
    {
        AbsReduceMax_gp4_Strided(srcPtr, maxGp4Ptr, validRows, validCols, srcCols);
        mem_bar(VST_VLD);
        AbsReduceMax_gp8_gp64_Strided(
            maxGp4Ptr, maxGp8Ptr, maxGp64Ptr, validRows, validCols, gp4StaticCols, gp8StaticCols, gp64StaticCols);
        mem_bar(VST_VLD);
        CalcExpScale_Strided(
            maxGp4Ptr, maxGp8Ptr, maxGp64Ptr, eaPtr, ebPtr, ecPtr, scalingPtr, validRows, validCols, gp4StaticCols,
            gp8StaticCols, gp64StaticCols, numGp4, numGp8, numGp64, numBlocks);
        mem_bar(VST_VLD);
        ExpLayoutForCube_Strided(
            eaPtr, ebPtr, ecPtr, expPtr + expTotalBytes, validRows, validCols, gp64StaticCols, gp8StaticCols,
            gp4StaticCols);
        mem_bar(VST_VLD);
        CalcFp4Values_Strided(srcPtr, scalingPtr, dstPtr, validRows, validCols, srcCols);
    }
}

PTO_INTERNAL void TQuant_Hif4_Impl(
    __ubuf__ bfloat16_t* srcPtr, __ubuf__ uint8_t* expPtr, __ubuf__ uint8_t* dstPtr, __ubuf__ bfloat16_t* maxPtr,
    __ubuf__ bfloat16_t* scalingPtr, uint32_t validRows, uint32_t validCols, uint32_t srcCols)
{
    constexpr uint32_t gp4Size = 4;
    constexpr uint32_t gp8Size = 8;
    constexpr uint32_t gp64Size = 64;
    constexpr uint32_t ubAlignBytes = BLOCK_BYTE_SIZE;
    const uint32_t totalElements = validRows * validCols;
    const uint32_t numGp4 = totalElements / gp4Size;
    const uint32_t numGp8 = totalElements / gp8Size;
    const uint32_t numGp64 = totalElements / gp64Size;
    const uint32_t numBlocks = numGp64;
    const uint32_t gp4PerRow = CeilDivision((int32_t)validCols, (int32_t)gp4Size);
    const uint32_t gp8PerRow = CeilDivision((int32_t)validCols, (int32_t)gp8Size);
    const uint32_t gp64PerRow = CeilDivision((int32_t)validCols, (int32_t)gp64Size);
    constexpr uint32_t maxAlignElems = ubAlignBytes / sizeof(bfloat16_t);
    const uint32_t gp4StaticCols = CeilDivision((int32_t)gp4PerRow, (int32_t)maxAlignElems) * maxAlignElems;
    const uint32_t gp8StaticCols = CeilDivision((int32_t)gp8PerRow, (int32_t)maxAlignElems) * maxAlignElems;
    const uint32_t gp64StaticCols = CeilDivision((int32_t)gp64PerRow, (int32_t)maxAlignElems) * maxAlignElems;
    const uint32_t eaDataBytes = (totalElements / gp64Size) * sizeof(bfloat16_t);
    const uint32_t ebDataBytes = ((totalElements / gp8Size) / 8) * 2;
    const uint32_t ecDataBytes = (totalElements / gp4Size) / 8;
    const uint32_t expDstBytes = (totalElements / gp64Size) * 4;
    const uint32_t expTotalBytes = AlignUpBytes(eaDataBytes + ebDataBytes + ecDataBytes + expDstBytes, ubAlignBytes);
    __ubuf__ bfloat16_t* expBf16 = reinterpret_cast<__ubuf__ bfloat16_t*>(expPtr);
    ZeroFillUB<bfloat16_t>(expBf16, expTotalBytes / sizeof(bfloat16_t));
    __ubuf__ uint8_t* eaPtr = expPtr;
    __ubuf__ uint8_t* ebPtr = eaPtr + eaDataBytes;
    __ubuf__ uint8_t* ecPtr = ebPtr + ebDataBytes;
    __ubuf__ bfloat16_t* maxGp4Ptr = maxPtr;
    __ubuf__ bfloat16_t* maxGp8Ptr = maxGp4Ptr + totalElements / gp4Size;
    __ubuf__ bfloat16_t* maxGp64Ptr = nullptr;
    if (validCols == srcCols) {
        TQuant_Hif4_Cont(
            srcPtr, expPtr, dstPtr, maxPtr, scalingPtr, eaPtr, ebPtr, ecPtr, maxGp4Ptr, maxGp8Ptr, maxGp64Ptr,
            eaDataBytes, ebDataBytes, ecDataBytes, validRows, validCols);
    } else {
        TQuant_Hif4_Strided(
            srcPtr, expPtr, dstPtr, maxPtr, scalingPtr, eaPtr, ebPtr, ecPtr, maxGp4Ptr, maxGp8Ptr, maxGp64Ptr,
            expTotalBytes, gp4StaticCols, gp8StaticCols, gp64StaticCols, numGp4, numGp8, numGp64, numBlocks, validRows,
            validCols, srcCols);
    }
}
...
// QuantScaleAlg) pair.
//
// MxQuantAlg::Hif4 is ND-only (grp_axis == 1). It dispatches
// to TQuant_Hif4_Impl, which carves the single `max` pointer
// into the per-4 / per-8 / per-64 sub-regions internally and
// runs the five HiF4 stages. The caller just passes the
// (dst, src, exp, max, scaling) tiles — no layout work.
template <
    ...
    TileDataOut& dst, TileDataSrc& src, TileDataExp* exp, TileDataMax* max, TileDataScaling* scaling)
{
    ...
    // ---- HiF4 (BF16 -> HiFloat4) path -----------------------------------
    // ND-only. TQuant_Hif4_Impl zero-fills the `exp` workspace, carves Ea/Eb/Ec
    // (2D, row-padded 16/16/32 B) out of `exp` and per-4/8/64 max scratch out of
    // `max`, and routes continuous vs strided reduces on validCols == srcCols.
    if constexpr (mx_alg == MxQuantAlg::Hif4) {
        static_assert(grp_axis == 1, "Fix: HiF4 supports ND (grp_axis==1) only.");
        CheckTQuantHif4Types<T, OutT>();
        TQuant_Hif4_Impl(
            src.data(), exp->data(), reinterpret_cast<__ubuf__ uint8_t*>(dst.data()), max->data(), scaling->data(),
            src.GetValidRow(), src.GetValidCol(), (unsigned)TileDataSrc::Cols);
        return;
    } else {
        ...
        if constexpr (grp_axis == 0) {
            if constexpr (quant_type == QuantType::MXFP8) {
                ...
                TQuant_MXFP4_E2M1_Impl_DN<
                    scale_alg, TileDataOut, TileDataSrc, TileDataExp, TileDataMax, TileDataScaling>(
                    dst.data(), exp->data(), max->data(), scaling->data(), src.data(), src.GetValidRow(),
                    src.GetValidCol());
            }
        } else {
