...
#ifndef PTO_NPU_A6_TSHR_HPP
#define PTO_NPU_A6_TSHR_HPP
...
// A6 TSHR: vector right shift with a SIGNED shift amount (a5-compatible).
//
// Symmetric to TSHL: a negative amount reverses the direction, so lanes with
// src1 < 0 must perform a LEFT shift by |src1| instead of a right shift.
//   negMask  = (src1 < 0)
//   absSrc1  = |src1|
//   tmpL     = src0 << |src1|          // vshl
//   tmpR     = src0 >> |src1|          // vshr (arith if src0 signed, logic if unsigned)
//   dst      = negMask ? tmpL : tmpR   // vsel  (opposite pick from TSHL)
template <typename T>
struct ShrOp {
        ...
        RegTensor<T>& reg_dst, RegTensor<T>& reg_src0, RegTensor<T>& reg_src1, MaskReg& preg)
    {
        ...
        vsel(reg_dst, tmpL, tmpR, negMask);
    }
};
...
__tf__ PTO_INTERNAL OP_NAME(TSHR) OP_TYPE(element_wise) void TShr(
    ...
    VFImplKind version = VFImplKind::VFIMPL_DEFAULT)
{
    ...
    if constexpr (std::is_same_v<T, int64_t> || std::is_same_v<T, uint64_t>) {
        Int64ShiftBinary<true, T, TileDataDst::Cols, TileDataSrc0::Cols, TileDataSrc1::Cols>(
...
{
    using T = typename TileDataDst::DType;
    TShrCheck<TileDataDst, TileDataSrc0, TileDataSrc1>(dst, src0, src1);
    constexpr unsigned blockSizeElem = BLOCK_BYTE_SIZE / sizeof(T);
    constexpr unsigned elementsPerRepeat = CCE_VL / sizeof(T);
    TShr<TileDataDst, TileDataSrc0, TileDataSrc1, elementsPerRepeat, blockSizeElem>(
