...
{
    using T = typename DstTile::DType;
    static_assert(std::is_same<T, typename SrcTile::DType>::value, "The data type must be same of src and dst");
    static_assert((sizeof(T) == 2) || (sizeof(T) == 4), "TREMS: Invalid data type");
    static_assert(
        !std::is_same_v<T, int32_t> && !std::is_same_v<T, uint32_t>,
        "Fix: TREMS does not support int32/uint32 on kirinDev0000: hardware lacks integer vdiv.");
    static_assert(
