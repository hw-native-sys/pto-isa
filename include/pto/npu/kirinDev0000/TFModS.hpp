...
#include "pto/npu/a5/custom/TFmodRemHp.hpp"
...
struct FModSOp {
    ...
    PTO_INTERNAL static void BinSInstr(RegTensor<T>& reg_dst, RegTensor<T>& reg_src0, T scalar, MaskReg& preg)
    {
        ...
        if constexpr (isHighPrecision) {
            ...
            RegTensor<T> reg_q, reg_prod;
            DivSoftIntImpl(reg_q, reg_src0, reg_src1, preg);
            vmul(reg_prod, reg_q, reg_src1, preg, MODE_ZEROING);
            vsub(reg_dst, reg_src0, reg_prod, preg, MODE_ZEROING);
        }
    }
};

template <
    FmodSAlgorithm PrecisionType, typename TileDataDst, typename TileDataSrc, unsigned dstRowStride,
    unsigned srcRowStride>
__tf__ PTO_INTERNAL OP_NAME(TFMODS) OP_TYPE(element_wise) void TFModS(
...
PTO_INTERNAL void TFModSCheck(unsigned srcValidRow, unsigned srcValidCol, unsigned dstValidRow, unsigned dstValidCol)
{
    ...
    static_assert(
        ...
        "Fix: TFMODS does not support int32/uint32 on kirinDev0000: hardware lacks integer vdiv.");
    static_assert(
...
PTO_INTERNAL void TFMODS_IMPL(TileDataDst& dst, TileDataSrc& src, typename TileDataSrc::DType scalar)
{
    ...
    TFModS<PrecisionType, TileDataDst, TileDataSrc, dstRowStride, srcRowStride>(
