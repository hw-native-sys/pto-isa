...
template <typename T, typename TileDataDst, typename TileDataMask, typename TileDataSrc, unsigned elementsPerRepeat>
PTO_INTERNAL void TSels_b32FullLoop(
    __ubuf__ T* dstPtr, __ubuf__ uint32_t* maskPtr, __ubuf__ T* srcPtr, RegTensor<T>& vregScalar, MaskReg& tmpMask1,
    unsigned validRow, unsigned validCol, uint16_t loopTimes, unsigned& sReg)
{
    constexpr uint32_t maskRowStride = TileDataMask::RowStride * sizeof(typename TileDataMask::DType);
    constexpr uint32_t maskBytesPerRepeat = elementsPerRepeat / 8;
    MaskReg pReg, selMask0, selMask1, tmpMask;
    RegTensor<T> vreg0, vreg2, dreg0, dreg1;
    ...
    for (uint16_t i = 0; i < (uint16_t)validRow; ++i) {
        ...
        for (uint16_t j = 0; j < (uint16_t)loopTimes; ++j) {
            ...
            plds(tmpMask, maskPtr, i * maskRowStride + 2 * maskBytesPerRepeat * j, US);
...
template <typename T, typename TileDataDst, typename TileDataMask, typename TileDataSrc, unsigned elementsPerRepeat>
PTO_INTERNAL void TSels_b32RemLoop(
    __ubuf__ T* dstPtr, __ubuf__ uint32_t* maskPtr, __ubuf__ T* srcPtr, RegTensor<T>& vregScalar, unsigned validRow,
    uint16_t loopTimes, uint32_t remain)
{
    constexpr uint32_t maskRowStride = TileDataMask::RowStride * sizeof(typename TileDataMask::DType);
    constexpr uint32_t maskBytesPerRepeat = elementsPerRepeat / 8;
    MaskReg pReg, selMask0, tmpMask;
    RegTensor<T> vreg0, dreg0;
    uint32_t colOffset0 = 2 * loopTimes * elementsPerRepeat;
    uint32_t sReg;
    constexpr auto distValue =
        std::integral_constant<::DistVST, static_cast<::DistVST>(GetDistVst<T, DistVST::DIST_NORM>())>();
    for (uint16_t i = 0; i < (uint16_t)validRow; ++i) {
        sReg = remain;
        plds(tmpMask, maskPtr, i * maskRowStride + 2 * maskBytesPerRepeat * loopTimes, US);
    ...
    unsigned validCol)
{
    ...
    __VEC_SCOPE__
    {
        ...
        RegTensor<T> vregScalar;
        uint32_t sReg = validCol;
        MaskReg pReg = CreatePredicate<T>(sReg);
        vdup(vregScalar, scalar, pReg, MODE_ZEROING);
        sReg = validCol;
        TSels_b32FullLoop<T, TileDataDst, TileDataMask, TileDataSrc, elementsPerRepeat>(
            dstPtr, maskPtr, srcPtr, vregScalar, tmpMask1, validRow, validCol, loopTimes, sReg);
        if (sReg > 0) {
            TSels_b32RemLoop<T, TileDataDst, TileDataMask, TileDataSrc, elementsPerRepeat>(
                dstPtr, maskPtr, srcPtr, vregScalar, validRow, loopTimes, sReg);
        }
    } // end of vf
}
        ...
        for (uint16_t i = 0; i < (uint16_t)validRow; ++i) {
            ...
            for (uint16_t j = 0; j < (uint16_t)repeatTimes; ++j) {
                ...
                if constexpr (sizeof(T) == 2) {
                    plds(maskreg, (__ubuf__ uint32_t*)maskPtr, i * maskRowStride + j * maskBytesPerRepeat, US);
                } else {
                    plds(maskreg, (__ubuf__ uint32_t*)maskPtr, i * maskRowStride + j * maskBytesPerRepeat, NORM);
                }
