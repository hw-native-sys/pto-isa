...
__tf__ AICORE void TExtractToA(
...
template <typename DstTileData, typename SrcTileData, bool Transpose>
__tf__ AICORE void TExtractToB(
    typename DstTileData::TileDType __out__ dst, typename SrcTileData::TileDType __in__ src, uint16_t indexRow,
    uint16_t indexCol)
{
    using SrcType = std::conditional_t<(sizeof(typename SrcTileData::DType) == 2), half, typename SrcTileData::DType>;
...
PTO_INTERNAL void TExtractToANonTransposeCompact(
    __ca__ DstType* dstAddr, __cbuf__ SrcType* srcAddr, uint16_t indexRow, uint16_t indexCol, uint16_t dstValidRowAlign,
    uint16_t dstValidColAlign)
{
    constexpr int config = srcRow | (1u << 16);
    set_fmatrix(config);
    img2colv2_cbuf_to_ca(
        dstAddr, srcAddr, dstValidColAlign, dstValidRowAlign, indexCol, indexRow, 1, 1, 1, 1, 1, 1, false, false, false,
        false, srcCol);
}

template <typename DstType, typename SrcType, int32_t srcRow, int32_t srcCol>
PTO_INTERNAL void TExtractToATransposeCompact(
    __ca__ DstType* dstAddr, __cbuf__ SrcType* srcAddr, uint16_t indexRow, uint16_t indexCol, uint16_t dstValidRowAlign,
    uint16_t dstValidColAlign)
{
    if constexpr (sizeof(SrcType) == 1) {
        ...
        set_fmatrix(config);
        img2colv2_cbuf_to_ca(
            dstAddr, srcAddr, dstValidRowAlign, dstValidColAlign, indexRow, indexCol, 1, 1, 1, 1, 1, 1, false, false,
            true, false, srcRow);
    }
}

template <typename DstTileData, typename SrcTileData, bool Transpose>
__tf__ AICORE void TExtractToACompact(
    typename DstTileData::TileDType __out__ dst, typename SrcTileData::TileDType __in__ src, uint16_t indexRow,
    uint16_t indexCol, uint16_t dstValidRow, uint16_t dstValidCol, bool isKAligned)
{
    using SrcType = std::conditional_t<(sizeof(typename SrcTileData::DType) == 2), half, typename SrcTileData::DType>;
    using DstType = std::conditional_t<(sizeof(typename DstTileData::DType) == 2), half, typename DstTileData::DType>;
    __cbuf__ SrcType* srcAddr = (__cbuf__ SrcType*)__cce_get_tile_ptr(src);
...
template <typename DstType, typename SrcType, int32_t srcRow, int32_t srcCol>
PTO_INTERNAL void TExtractToBNonTransposeCompact(
    __cb__ DstType* dstAddr, __cbuf__ SrcType* srcAddr, uint16_t indexRow, uint16_t indexCol, uint16_t dstValidRowAlign,
    uint16_t dstValidColAlign)
{
    uint16_t dstGap = 0;
    constexpr int32_t c0Size = BLOCK_BYTE_SIZE / sizeof(SrcType);
...
template <typename DstType, typename SrcType, int32_t srcRow, int32_t srcCol>
PTO_INTERNAL void TExtractToBTransposeCompact(
    __cb__ DstType* dstAddr, __cbuf__ SrcType* srcAddr, uint16_t indexRow, uint16_t indexCol, uint16_t dstValidRowAlign,
    uint16_t dstValidColAlign, uint16_t dstValidCol)
{
    if constexpr (sizeof(SrcType) == 1) {
        constexpr uint16_t fractNum = 2;
        constexpr uint16_t srcColNum = srcCol * sizeof(SrcType) >> SHIFT_BLOCK_BYTE;
        ...
        set_fmatrix_b(config);
        img2colv2_cbuf_to_cb(
            dstAddr, srcAddr, dstValidColAlign, dstValidRowAlign, indexCol, indexRow, 1, 1, 1, 1, 1, 1, false, false,
            false, true, srcCol);
    }
}

template <typename DstTileData, typename SrcTileData, bool Transpose>
__tf__ AICORE void TExtractToBCompact(
    typename DstTileData::TileDType __out__ dst, typename SrcTileData::TileDType __in__ src, uint16_t indexRow,
    uint16_t indexCol, uint16_t dstValidRow, uint16_t dstValidCol)
{
    using SrcType = std::conditional_t<(sizeof(typename SrcTileData::DType) == 2), half, typename SrcTileData::DType>;
    using DstType = std::conditional_t<(sizeof(typename DstTileData::DType) == 2), half, typename DstTileData::DType>;
    constexpr int32_t c0Size = BLOCK_BYTE_SIZE / sizeof(SrcType);
...
template <typename DstTileData, typename SrcTileData>
__tf__ AICORE void TExtractToBConv(
    typename DstTileData::TileDType __out__ dst, typename SrcTileData::TileDType __in__ src, uint16_t srcCol,
...
PTO_INTERNAL void TEXTRACT_CONVTILE_IMPL(DstTileData& dst, SrcTileData& src, uint16_t indexRow, uint16_t indexCol)
{
    ...
    static_assert(
        sizeof(typename DstTileData::DType) == sizeof(typename SrcTileData::DType),
        "Fix: Source dtype must be same with dst dtype!");

    static_assert(
        (SrcTileData::layout == Layout::FRACTAL_Z) || (SrcTileData::layout == Layout::FRACTAL_Z_3D),
        "TExtract: Source layout only support FRACTAL_Z or FRACTAL_Z_3D.");
    static_assert(
        DstTileData::SFractal == SLayout::ColMajor && DstTileData::isRowMajor,
        "TExtract: Destination layout only support SLayout is ColMajor ang BLayout is RowMajor.");
    static_assert(
        std::is_same<typename DstTileData::DType, int8_t>::value ||
...
AICORE void TExtractToLeft(DstTileData& dst, SrcTileData& src, uint16_t indexRow, uint16_t indexCol)
{
    ...
    static_assert(
        DstTileData::SFractal == SLayout::RowMajor && DstTileData::isRowMajor, "TExtract: LeftTile Invalid Fractal.");
...
AICORE void TExtractToRight(DstTileData& dst, SrcTileData& src, uint16_t indexRow, uint16_t indexCol)
{
    static_assert(
        ...
        "TExtract: SrcTile Invalid Fractal");
    static_assert(
        DstTileData::SFractal == SLayout::ColMajor && DstTileData::isRowMajor, "TExtract: RightTile Invalid Fractal.");
...
__tf__ AICORE void TExtractVecToVecNDScalar(
...
template <typename T, typename DstTileData, typename SrcTileData>
__tf__ AICORE void TExtractVecToVecNZScalar(
    typename DstTileData::TileDType __out__ dst, typename SrcTileData::TileDType __in__ src, uint32_t indexRow,
    uint32_t indexCol)
{
    __ubuf__ T* dstAddr = (__ubuf__ T*)__cce_get_tile_ptr(dst);
...
PTO_INTERNAL void CheckTExtractVecToVecCommon()
{
    ...
    static_assert(
        std::is_same<DstT, int8_t>::value || std::is_same<DstT, uint8_t>::value || std::is_same<DstT, int16_t>::value ||
            std::is_same<DstT, uint16_t>::value || std::is_same<DstT, half>::value ||
            std::is_same<DstT, bfloat16_t>::value || std::is_same<DstT, float>::value ||
            std::is_same<DstT, int32_t>::value || std::is_same<DstT, uint32_t>::value,
...
PTO_INTERNAL void CheckTExtractVecToVecND()
{
    ...
    static_assert(
        DstTileData::Rows <= SrcTileData::Rows, "TEXTRACT ND Vec->Vec : Destination Rows must not exceed source Rows.");
    static_assert(
        DstTileData::Cols <= SrcTileData::Cols, "TEXTRACT ND Vec->Vec : Destination Cols must not exceed source Cols.");
    static_assert(
    ...
    if constexpr (!(DstTileData::ValidRow == 1 && DstTileData::ValidCol == 1)) {
        static_assert(
            (DstTileData::ValidCol * sizeof(T)) % sizeof(uint16_t) == 0,
...
PTO_INTERNAL void CheckTExtractVecToVecNZ()
{
    ...
    static_assert(
        DstTileData::Rows <= SrcTileData::Rows, "TEXTRACT NZ Vec->Vec : Destination Rows must not exceed source Rows.");
    static_assert(
        DstTileData::Cols <= SrcTileData::Cols, "TEXTRACT NZ Vec->Vec : Destination Cols must not exceed source Cols.");
