...
PTO_INTERNAL void TStoreMat2GmInstr(
    ...
    int gShape2, int gStride0, int gStride1, int gStride2, int64_t srcStride2, uint16_t nBurst, uint16_t lenBurst,
...
template <typename GlobalData, typename TileData>
PTO_INTERNAL void TStoreMat2GmNd2Nd(
    typename GlobalData::DType* dstAddr, __cbuf__ typename TileData::DType* srcAddr, int gShape0, int gShape1,
...
template <typename GlobalData, typename TileData>
PTO_INTERNAL void TStoreMat2GmDn2Dn(
    typename GlobalData::DType* dstAddr, __cbuf__ typename TileData::DType* srcAddr, int gShape0, int gShape1,
    int gShape2, int gShape3, int gShape4, int gStride0, int gStride1, int gStride2, int gStride3, int gStride4,
    int validRow, int validCol)
{
...
template <typename GlobalData, typename TileData>
PTO_INTERNAL void TStoreMat2GmNz2Nz(
    typename GlobalData::DType* dstAddr, __cbuf__ typename TileData::DType* srcAddr, int gShape0, int gShape1,
    int gShape2, int gShape3, int gShape4, int gStride0, int gStride1, int gStride2, int gStride3, int gStride4,
    int validRow, int validCol)
{
    ...
    int validRow, int validCol)
{
    ...
    if constexpr (TileData::isRowMajor & (TileData::SFractal == SLayout::NoneBox)) {
        TStoreMat2GmNd2Nd<GlobalData, TileData>(
        ...
        TStoreMat2GmDn2Dn<GlobalData, TileData>(
            dstAddr, srcAddr, gShape0, gShape1, gShape2, gShape3, gShape4, gStride0, gStride1, gStride2, gStride3,
            gStride4, validRow, validCol);
    } else if constexpr (!TileData::isRowMajor & (TileData::SFractal == SLayout::RowMajor)) {
        TStoreMat2GmNz2Nz<GlobalData, TileData>(
            dstAddr, srcAddr, gShape0, gShape1, gShape2, gShape3, gShape4, gStride0, gStride1, gStride2, gStride3,
            gStride4, validRow, validCol);
    }
}
...
{
    static_assert(
        (std::is_same<T, __gm__ half>::value) || (std::is_same<T, __gm__ float>::value) ||
...
PTO_INTERNAL void SetAtomicNone() { set_atomic_none(); }

template <
    typename GlobalData, typename TileData, QuantMode_t quantizationMode = QuantMode_t::NoQuant,
    ReluPreMode reluPreMode = ReluPreMode::NoRelu, STPhase Phase = STPhase::Unspecified>
PTO_INTERNAL void TStoreAccNz2nd(
    typename GlobalData::DType* dstAddr, __cc__ typename TileData::DType* srcAddr, int gShape0, int gShape1,
    int gShape2, int gShape3, int gShape4, int gStride0, int gStride1, int gStride2, int gStride3, int gStride4,
    int validRow, int validCol)
{
...
template <
    typename GlobalData, typename TileData, QuantMode_t quantizationMode = QuantMode_t::NoQuant,
    ReluPreMode reluPreMode = ReluPreMode::NoRelu, STPhase Phase = STPhase::Unspecified>
PTO_INTERNAL void TStoreAccNz2NC1HWC0(
    typename GlobalData::DType* dstAddr, __cc__ typename TileData::DType* srcAddr, int gShape0, int gShape1,
    int gShape2, int gShape3, int gShape4, int gStride1, int gStride3, int validRow, int validCol)
{
    PTO_ASSERT(
        validRow == gShape0 * gShape2 * gShape3,
        "The validRow of TileData must be equal to gShape0 * Shape2 * Shape3 of NC1HWC0 shape!");
    PTO_ASSERT(
        validCol == gShape1 * gShape4, "The validCol of TileData must be equal to Shape1 * Shape4 of NC1HWC0 shape!");
    PTO_ASSERT(
        validRow >= 1 && validRow <= 65535, "When GlobalData is NC1HWC0 format, the range of validRow is [1, 65535].");
...
template <
    typename GlobalData, typename TileData, QuantMode_t quantizationMode = QuantMode_t::NoQuant,
    ReluPreMode reluPreMode = ReluPreMode::NoRelu, STPhase Phase = STPhase::Unspecified>
PTO_INTERNAL void TStoreAccNz2NDC1HWC0(
    typename GlobalData::DType* dstAddr, __cc__ typename TileData::DType* srcAddr, int gShape0, int gShape1,
    int gShape2, int gShape3, int gShape4, int gStride2, int gStride4, int validRow, int validCol)
{
...
template <typename GlobalData, typename TileData>
PTO_INTERNAL void TStoreUb2gmNd2nd(
