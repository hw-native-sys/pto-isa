...
def make_inputs(shape: PaShape = PaShape(batch=1), device="npu:0", deterministic=True):
    ...
    if deterministic:
        ...
        values = (((batch * 13 + token * 17 + kv_head * 31 + dim * 7).remainder(257) / 128.0) - 1.0) * 0.25
        v_dense.copy_(values.reshape(shape.batch, shape.seq_len, shape.num_kv_heads * shape.head_dim).to(shape.dtype))
    else:
...
def main():
    ...
    ref = golden_uniform(v, block_table, shape)
