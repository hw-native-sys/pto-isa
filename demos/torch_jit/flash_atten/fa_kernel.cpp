...
template <
    ...
    bool INTERMEDIATE_CHECK, typename TileMatPData, typename TileMatVData, typename TilePVData, typename TSyncSM2PV,
    typename TSyncPV2GU>
AICORE inline void compute_pv(
    ...
    TSyncSM2PV& sm2pvSync, TSyncPV2GU& pv2guSync)
{
    constexpr uint32_t Cube_S0 = CUBE_S0;
    constexpr uint32_t Cube_S1 = CUBE_S1;
    constexpr uint32_t Tile_S1 = TILE_S1;
...
template <
    ...
    typename TileOutT, typename ReduceTileF_T, typename TSyncPV2GU>
...
void LaunchTFA(
    ...
    float* pv_tile_fifo, uint8_t* profile_data, aclrtStream stream, uint8_t* cv_comm_buf, int s0, int s1)
{
    uint32_t block_rows = s0 / CUBE_S0;
    ...
    runTFA<
        HEAD_SIZE, CUBE_S0, CUBE_S1, TILE_S1, QK_PRELOAD, CV_FIFO_SIZE, INTERMEDIATE_CHECK, CAUSAL_MASK,
...
#define LAUNCH_TFA(S0_, S1_, CAUSAL_)                                                              \
    do {                                                                                           \
        runTFA<                                                                                    \
            128, CUBE_S0, CUBE_S1, TILE_S1, QK_PRELOAD, CV_FIFO_SIZE, INTERMEDIATE_CHECK, CAUSAL_, \
